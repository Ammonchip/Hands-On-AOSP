 ...
 * administrator</a>. Additionally, a device administrator may be registered as
 * either a profile or
 * device owner. A given method is accessible to all device administrators
 * unless the documentation
 * for that method specifies that it is restricted to either device or profile
 * owners. Any
 * application calling an api may only pass as an argument a device
 * administrator component it
 * owns. Otherwise, a {@link SecurityException} will be thrown.
 *
 * <p>
 * <b>Note: </b>on
 * {@link android.content.pm.PackageManager#FEATURE_AUTOMOTIVE automotive
 * builds}, some methods can
 * throw an {@link UnsafeStateException} exception (for example, if the vehicle
 * is moving), so
 * callers running on automotive builds should always check for that exception,
 * otherwise they
 * might crash.
 *
 ...
 * <p>
 * For more information about managing policies for device administration, read
 * the <a href=
...
public class DevicePolicyManager {
	...
	/**
	 * Activity action: Starts the provisioning flow which sets up a managed
	 * profile.
	 *
	 * <p>
	 * A managed profile allows data separation for example for the usage of a
	 * device as a personal and corporate device. The user which provisioning is
	 * started from and
	 * the managed profile share a launcher.
	 *
	 * <p>
	 * This intent will typically be sent by a mobile device management application
	 * (MDM).
	 * Provisioning adds a managed profile and sets the MDM as the profile owner who
	 * has full
	 * control over the profile.
	 *
	 * <p>
	 * It is possible to check if provisioning is allowed or not by querying the
	 * method
	 * {@link #isProvisioningAllowed(String)}.
	 *
	 * <p>
	 * In version {@link android.os.Build.VERSION_CODES#LOLLIPOP}, this intent must
	 * contain the
	 ...
	 *
	 * <p>
	 * When managed provisioning has completed, broadcasts are sent to the
	 * application specified
	 * in the provisioning intent. The
	 * {@link DeviceAdminReceiver#ACTION_PROFILE_PROVISIONING_COMPLETE} broadcast is
	 * sent in the
	 * managed profile and the {@link #ACTION_MANAGED_PROFILE_PROVISIONED} broadcast
	 * is sent in
	 * the primary profile.
	 *
	 * <p>
	 ...
	 * {@link #ACTION_PROVISIONING_SUCCESSFUL} will also be sent to the profile
	 * owner.
	 *
	 * <p>
	 * If provisioning fails, the managedProfile is removed so the device returns to
	 * its
	 * previous state.
	 *
	 * <p>
	 * If launched with
	 * {@link android.app.Activity#startActivityForResult(Intent, int)} a
	 * result code of {@link android.app.Activity#RESULT_OK} implies that the
	 * synchronous part of
	 * the provisioning flow was successful, although this doesn't guarantee the
	 * full flow will
	 * succeed. Conversely a result code of
	 * {@link android.app.Activity#RESULT_CANCELED} implies
	 * that the user backed-out of provisioning, or some precondition for
	 * provisioning wasn't met.
	 *
	 * <p>
	 * If a device policy management role holder (DPMRH) updater is present on the
	 * device, an
	 * internet connection attempt must be made prior to launching this intent. If
	 * internet
	 * connection could not be established, provisioning will fail unless {@link
	 * #EXTRA_PROVISIONING_ALLOW_OFFLINE} is explicitly set to {@code true}, in
	 * which case
	 * provisioning will continue without using the DPMRH. If an internet connection
	 * has been
	 * established, the DPMRH updater will be launched, which will update the DPMRH
	 * if it's not
	 * present on the device, or if it's present and not valid.
	 *
	 * <p>
	 * If a DPMRH is present on the device and valid, the provisioning flow will be
	 * deferred to
	 * it.
	 */
	 ...
	 * This intent will typically be sent by a mobile device management application
	 * (MDM).
	 * Provisioning configures the user as managed user and sets the MDM as the
	 * profile
	 * owner who has full control over the user. Provisioning can only happen before
	 * user setup has
	 * been completed. Use {@link #isProvisioningAllowed(String)} to check if
	 * provisioning is
	 * allowed.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * During device owner provisioning a device admin app is set as the owner of
	 * the device.
	 * A device owner has full control over the device. The device owner can not be
	 * modified by the
	 * user.
	 *
	 * <p>
	 * A typical use case would be a device that is owned by a company, but used by
	 * either an
	 * employee or client.
	 *
	 * <p>
	 ...
	 * {@link DeviceAdminReceiver#ACTION_PROFILE_PROVISIONING_COMPLETE} is broadcast
	 * to the
	 * device owner.
	 *
	 * <p>
	 * From version {@link android.os.Build.VERSION_CODES#O}, when device owner
	 * provisioning has
	 * completed, along with the above broadcast, activity intent
	 * {@link #ACTION_PROVISIONING_SUCCESSFUL} will also be sent to the device
	 * owner.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * A result code of {@link android.app.Activity#RESULT_OK} implies that the
	 * synchronous part
	 * of the provisioning flow was successful, although this doesn't guarantee the
	 * full flow will
	 * succeed. Conversely a result code of
	 * {@link android.app.Activity#RESULT_CANCELED} implies
	 * that the user backed-out of provisioning, or some precondition for
	 * provisioning wasn't met.
	 *
	 * @deprecated to support {@link android.os.Build.VERSION_CODES#S} and later,
	 *             admin apps must
	 *             implement activities with intent filters for the
	 *             {@link #ACTION_GET_PROVISIONING_MODE} and
	 *             {@link #ACTION_ADMIN_POLICY_COMPLIANCE} intent actions; using
	 *             {@link
	 *             #ACTION_PROVISION_MANAGED_DEVICE} to start provisioning will
	 *             cause the provisioning to fail;
	 *             to additionally support
	 *             pre-{@link android.os.Build.VERSION_CODES#S}, admin apps must
	 *             also
	 ...
	 *
	 * <p>
	 * Please note that the API behavior is not necessarily consistent across
	 * various releases,
	 * and devices, as it's contract between SetupWizard and ManagedProvisioning.
	 * The default
	 * implementation is that ManagedProvisioning launches SetupWizard in NFC
	 * provisioning only.
	 *
	 * <p>
	 * The activity must be protected by permission
	 * {@link android.Manifest.permission#BIND_DEVICE_ADMIN}, and the process must
	 * hold
	 * {@link android.Manifest.permission#DISPATCH_PROVISIONING_MESSAGE} to be
	 * launched.
	 * Only one {@link ComponentName} in the entire system should be enabled, and
	 * the rest of the
	 * components are not started by this intent.
	 *
	 * @deprecated Starting from Android 13, the system no longer launches an intent
	 *             with this
	 ...
	 *
	 * <p>
	 * During device owner provisioning, a device admin app is downloaded and set as
	 * the owner of
	 * the device. A device owner has full control over the device. The device owner
	 * can not be
	 * modified by the user and the only way of resetting the device is via factory
	 * reset.
	 *
	 * <p>
	 * From version {@link android.os.Build.VERSION_CODES#Q}, the admin app can
	 * choose
	 * whether to set up a fully managed device or a managed profile. For the admin
	 * app to support
	 * this, it must have an activity with intent filter
	 * {@link #ACTION_GET_PROVISIONING_MODE} and
	 * another one with intent filter {@link #ACTION_ADMIN_POLICY_COMPLIANCE}. For
	 * example:
	 * 
	 * <pre>
	 * &lt;activity
	 ...
	 *
	 * <p>
	 * Unlike {@link #ACTION_PROVISION_MANAGED_DEVICE}, the provisioning message can
	 * only be sent
	 ...
	 * #ACTION_UPDATE_DEVICE_MANAGEMENT_ROLE_HOLDER}, which will update the DPMRH if
	 * it's not
	 * present on the device, or if it's present and not valid.
	 *
	 * <p>
	 * A DPMRH is considered valid if it has intent filters for {@link
	 ...
	 * deferred to
	 * it via the
	 * {@link #ACTION_ROLE_HOLDER_PROVISION_MANAGED_DEVICE_FROM_TRUSTED_SOURCE}
	 * intent.
	 *
	 * <p>
	 ...
	 * <ul>
	 * <li>{@link #EXTRA_PROVISIONING_DEVICE_ADMIN_COMPONENT_NAME}</li>
	 * <li>{@link #EXTRA_PROVISIONING_DEVICE_ADMIN_PACKAGE_DOWNLOAD_LOCATION},
	 * optional</li>
	 * <li>{@link #EXTRA_PROVISIONING_DEVICE_ADMIN_PACKAGE_DOWNLOAD_COOKIE_HEADER},
	 * optional</li>
	 ...
	 * <li>{@link #EXTRA_PROVISIONING_WIFI_HIDDEN} (convert to String),
	 * optional</li>
	 ...
	 * <li>{@link #EXTRA_PROVISIONING_WIFI_PROXY_PORT} (convert to String),
	 * optional</li>
	 ...
	 * <li>{@link #EXTRA_PROVISIONING_SKIP_EDUCATION_SCREENS}, optional - when not
	 * used for
	...
	/**
	 * Activity action: Starts the provisioning flow which sets up a financed
	 * device.
	 *
	 * <p>
	 * During financed device provisioning, a device admin app is downloaded and set
	 * as the owner
	 * of the device. A device owner has full control over the device. The device
	 * owner can not be
	 * modified by the user.
	 *
	 * <p>
	 * A typical use case would be a device that is bought from the reseller through
	 * financing
	 * program.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * Once the device admin app is set as the device owner, the following APIs are
	 * available for
	 ...
	 * <li>{@link #setLockTaskFeatures(ComponentName, int)}, where the following
	 * lock task features
	 ...
	 * <li>{@link #clearPackagePersistentPreferredActivities(ComponentName, String)}
	 * </li>
	 * <li>{@link #wipeData(int)}</li>
	 * <li>{@link #isDeviceOwnerApp(String)}</li>
	 * <li>{@link #clearDeviceOwnerApp(String)}</li>
	 * <li>{@link #setPermissionGrantState(ComponentName, String, String, int)},
	 * where
	 * {@link permission#READ_PHONE_STATE} is the <b>only</b> permission that can be
	 * {@link #PERMISSION_GRANT_STATE_GRANTED},
	 * {@link #PERMISSION_GRANT_STATE_DENIED}, or
	 * {@link #PERMISSION_GRANT_STATE_DEFAULT} and can <b>only</b> be applied to the
	 * device admin
	 ...
	 * {@link permission#READ_PHONE_STATE} is the <b>only</b> permission that can be
	 * used and device admin app is the only package that can be used to retrieve
	 * the permission
	 * permission grant state for (otherwise a {@link SecurityException} will be
	 * thrown)</li>
	 * <li>{@link #addUserRestriction(ComponentName, String)}, where the following
	 * user restrictions
	 ...
	 * <li>{@link #clearUserRestriction(ComponentName, String)}, where the following
	 * user
	 * restrictions are permitted (otherwise a {@link SecurityException} will be
	 * thrown):</li>
	 * <ul>
	 * <li>{@link UserManager#DISALLOW_ADD_USER}</li>
	 * <li>{@link UserManager#DISALLOW_DEBUGGING_FEATURES}</li>
	...
	/**
	 * Activity action: Finalizes management provisioning, should be used after
	 * user-setup
	 ...
	 *
	 * <p>
	 * If a device policy management role holder (DPMRH) is present on the device
	 * and
	...
	/**
	 * Activity action: starts the managed profile provisioning flow inside the
	 * device policy
	 * management role holder.
	 *
	 * <p>
	 * During the managed profile provisioning flow, the platform-provided
	 * provisioning handler
	 * will delegate provisioning to the device policy management role holder, by
	 * firing this
	 * intent. Third-party mobile device management applications attempting to fire
	 * this intent will
	 * receive a {@link SecurityException}.
	 *
	 * <p>
	 * Device policy management role holders are required to have a handler for this
	 * intent
	 * action.
	 *
	 * <p>
	 * If {@link #EXTRA_ROLE_HOLDER_STATE} is supplied to this intent, it is the
	 * responsibility
	 * of the role holder to restore its state from this extra. This is the same
	 * {@link Bundle}
	 * which the role holder returns alongside {@link #RESULT_UPDATE_ROLE_HOLDER}.
	 *
	 * <p>
	 * A result code of {@link Activity#RESULT_OK} implies that managed profile
	 * provisioning
	 * finished successfully. If it did not, a result code of
	...
	/**
	 * Activity action: starts the trusted source provisioning flow inside the
	 * device policy
	 * management role holder.
	 *
	 * <p>
	 * During the trusted source provisioning flow, the platform-provided
	 * provisioning handler
	 * will delegate provisioning to the device policy management role holder, by
	 * firing this
	 * intent. Third-party mobile device management applications attempting to fire
	 ...
	 *
	 * <p>
	 * The result codes can be either {@link #RESULT_WORK_PROFILE_CREATED}, {@link
	 * #RESULT_DEVICE_OWNER_SET} or {@link Activity#RESULT_CANCELED} if provisioning
	 * failed.
	 *
	...
	/**
	 * Activity action: starts the provisioning finalization flow inside the device
	 * policy
	 * management role holder.
	 *
	 * <p>
	 * During the provisioning finalization flow, the platform-provided provisioning
	 * handler
	 * will delegate provisioning to the device policy management role holder, by
	 * firing this
	 * intent. Third-party mobile device management applications attempting to fire
	 ...
	 *
	 * <p>
	 * This handler forwards the result from the admin app's {@link
	 * #ACTION_ADMIN_POLICY_COMPLIANCE} handler. Result code
	 * {@link Activity#RESULT_CANCELED}
	 ...
	 * #ACTION_ROLE_HOLDER_PROVISION_MANAGED_PROFILE} and {@link
	 * #ACTION_ROLE_HOLDER_PROVISION_MANAGED_DEVICE_FROM_TRUSTED_SOURCE} to signal
	 * that an update
	 * to the role holder is required.
	 *
	 * <p>
	 * This result code can be accompanied by {@link #EXTRA_ROLE_HOLDER_STATE}.
	 *
	 * @hide
	 */
	@SystemApi
	...
	/**
	 * A {@link PersistableBundle} extra which the role holder can use to describe
	 * its own state
	 * when it returns {@link #RESULT_UPDATE_ROLE_HOLDER}.
	 *
	 * <p>
	 * If {@link #RESULT_UPDATE_ROLE_HOLDER} was accompanied by this extra, after
	 * the update
	 * completes, the role holder's
	 * {@link #ACTION_ROLE_HOLDER_PROVISION_MANAGED_PROFILE} or {@link
	 * #ACTION_ROLE_HOLDER_PROVISION_MANAGED_DEVICE_FROM_TRUSTED_SOURCE} intent will
	 * be relaunched,
	 * which will contain this extra. It is the role holder's responsibility to
	 * restore its
	 * state from this extra.
	 *
	 * <p>
	 * The content of this {@link PersistableBundle} is entirely up to the role
	 * holder. It
	 * should contain anything the role holder needs to restore its original state
	 * when it gets
	 * restarted.
	 *
	 * @hide
	...
	/**
	 * A {@code boolean} extra which determines whether to force a role holder
	 * update, regardless
	 * of any internal conditions
	 * {@link #ACTION_UPDATE_DEVICE_POLICY_MANAGEMENT_ROLE_HOLDER} might
	 * have.
	 *
	 * <p>
	 * This extra can be provided to intents with action {@link
	 ...
	 *
	 * <p>
	 * For the online provisioning flow, there will be an attempt to download and
	 * install
	 * the latest version of the device policy management role holder. The platform
	 * will then
	 * delegate provisioning to the device policy management role holder via role
	 * holder-specific
	 * provisioning actions.
	 *
	 * <p>
	 * For the offline provisioning flow, the provisioning flow will always be
	 * handled by
	 * the platform.
	 *
	 * <p>
	 * If this extra is set to {@code false}, the provisioning flow will enforce
	 * that an
	 * internet connection is established, which will start the online provisioning
	 * flow. If an
	 * internet connection cannot be established, provisioning will fail.
	 *
	 * <p>
	 * If this extra is set to {@code true}, the provisioning flow will still try to
	 * connect to
	 * the internet, but if it fails it will start the offline provisioning flow.
	 *
	 * <p>
	 * For T if this extra is set to {@code true}, the provisioning flow will be
	 * forced through
	 * the platform and there will be no attempt to download and install the device
	 * policy
	 * management role holder.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * This extra is respected when provided via the provisioning intent actions
	 * such as {@link
	...
	/**
	 * A String extra holding a url that specifies the download location of the
	 * device policy
	 * management role holder package.
	 *
	 * <p>
	 * This is only meant to be used in cases when a specific variant of the role
	 * holder package
	 * is needed (such as a debug variant). If not provided, the default variant of
	 * the device
	 * manager role holder package is downloaded.
	 *
	 * <p>
	 * Use in an intent with action
	 * {@link #ACTION_PROVISION_MANAGED_DEVICE_FROM_TRUSTED_SOURCE}
	 * or in an NFC record with {@link #MIME_TYPE_PROVISIONING_NFC} that starts
	 * device owner
	...
	/**
	 * A String extra holding the URL-safe base64 encoded SHA-256 checksum of any
	 * signature of the
	 ...
	 *
	 * <p>
	 * If {@link #EXTRA_PROVISIONING_ROLE_HOLDER_PACKAGE_DOWNLOAD_LOCATION} is
	 * provided, it must
	 * be accompanied by this extra. The provided checksum must match the checksum
	 * of any signature
	 * of the file at the download location. If the checksum does not match an error
	 * will be shown
	 ...
	 * request to the
	 * url specified in
	 * {@link #EXTRA_PROVISIONING_ROLE_HOLDER_PACKAGE_DOWNLOAD_LOCATION}.
	 *
	 * <p>
	 * Use in an intent with action
	 * {@link #ACTION_PROVISION_MANAGED_DEVICE_FROM_TRUSTED_SOURCE}
	...
	/**
	 * An extra of type {@link android.os.PersistableBundle} that allows the
	 * provisioning initiator
	 * to pass data to the device policy management role holder.
	 *
	 * <p>
	 * The device policy management role holder will receive this extra via the
	 * {@link
	 * #ACTION_ROLE_HOLDER_PROVISION_MANAGED_DEVICE_FROM_TRUSTED_SOURCE} intent.
	 *
	 * <p>
	 * The contents of this extra are up to the contract between the provisioning
	 * initiator
	 ...
	 *
	 * <p>
	 * Use in an intent with action {@link
	...
	/**
	 * An {@link Intent} result extra specifying the {@link Intent} to be launched
	 * after
	 * provisioning is finalized.
	 *
	 * <p>
	 * If {@link #EXTRA_PROVISIONING_SHOULD_LAUNCH_RESULT_INTENT} is set to
	 * {@code false},
	 * this result will be supplied as part of the result {@link Intent} for
	 * provisioning actions
	 * such as {@link #ACTION_PROVISION_MANAGED_PROFILE}. This result will also be
	 * supplied as
	 * part of the result {@link Intent} for the device policy management role
	 * holder provisioning
	 * actions.
	 */
	...
	/**
	 * A boolean extra that determines whether the provisioning flow should launch
	 * the resulting
	 * launch intent, if one is supplied by the device policy management role holder
	 * via {@link
	 * #EXTRA_RESULT_LAUNCH_INTENT}. Default value is {@code false}.
	 *
	 * <p>
	 * If {@code true}, the resulting intent will be launched by the provisioning
	 * flow, if one
	 * is supplied by the device policy management role holder.
	 *
	 * <p>
	 * If {@code false}, the resulting intent will be returned as {@link
	 * #EXTRA_RESULT_LAUNCH_INTENT} to the provisioning initiator, if one is
	 * supplied by the device
	 * manager role holder. It will be the responsibility of the provisioning
	 * initiator to launch
	...
	/**
	 * Notification type for a bugreport that has already been accepted to be
	 * shared, but is still
	 * being taken.
	 *
	 * @hide
	...
	/**
	 * Notification type for a bugreport that has been taken and can be shared or
	 * declined.
	 *
	 * @hide
	 */
	...
	/**
	 * Default and maximum timeout in milliseconds after which unlocking with weak
	 * auth times out,
	 * i.e. the user has to use a strong authentication method like password, PIN or
	 * pattern.
	 *
	 * @hide
	 */
	...
	/**
	 * A {@link android.os.Parcelable} extra of type
	 * {@link android.os.PersistableBundle} that
	 * allows a mobile device management application or NFC programmer application
	 * which starts
	 * managed provisioning to pass data to the management application instance
	 * after provisioning.
	 * <p>
	 * If used with {@link #ACTION_PROVISION_MANAGED_PROFILE} it can be used by the
	 * application that
	 * sends the intent to pass data to itself on the newly created profile.
	 * If used with {@link #ACTION_PROVISION_MANAGED_DEVICE} it allows passing data
	 * to the same
	 * instance of the app on the primary user.
	 * Starting from {@link android.os.Build.VERSION_CODES#M}, if used with
	 * {@link #MIME_TYPE_PROVISIONING_NFC} as part of NFC managed device
	 * provisioning, the NFC
	 * message should contain a stringified {@link java.util.Properties} instance,
	 * whose string
	 * properties will be converted into a {@link android.os.PersistableBundle} and
	 * passed to the
	 * management application after provisioning.
	 *
	 * <p>
	 * Admin apps will receive this extra in their
	 * {@link #ACTION_GET_PROVISIONING_MODE} and
	 * {@link #ACTION_ADMIN_POLICY_COMPLIANCE} intent handlers. Additionally, {@link
	 * #ACTION_GET_PROVISIONING_MODE} may also return this extra which will then be
	 * sent over to
	 * {@link #ACTION_ADMIN_POLICY_COMPLIANCE}, alongside the original values that
	 * were passed to
	 * {@link #ACTION_GET_PROVISIONING_MODE}.
	 *
	 * <p>
	 * In both cases the application receives the data in
	 * {@link DeviceAdminReceiver#onProfileProvisioningComplete} via an intent with
	 * the action
	 * {@link DeviceAdminReceiver#ACTION_PROFILE_PROVISIONING_COMPLETE}. The bundle
	 * is not changed
	...
	/**
	 * A String extra holding the package name of the mobile device management
	 * application that
	 * will be set as the profile owner or device owner.
	 *
	 * <p>
	 * If an application starts provisioning directly via an intent with action
	 * {@link #ACTION_PROVISION_MANAGED_PROFILE} this package has to match the
	 * package name of the
	 * application that started provisioning. The package will be set as profile
	 * owner in that case.
	 *
	 * <p>
	 * This package is set as device owner when device owner provisioning is started
	 * by an NFC
	 * message containing an NFC record with MIME type
	 * {@link #MIME_TYPE_PROVISIONING_NFC}.
	 *
	 * <p>
	 * When this extra is set, the application must have exactly one device admin
	 * receiver.
	 * This receiver will be set as the profile or device owner and active admin.
	 *
	 * @see DeviceAdminReceiver
	 * @deprecated Use {@link #EXTRA_PROVISIONING_DEVICE_ADMIN_COMPONENT_NAME}. This
	 *             extra is still
	 *             supported, but only if there is only one device admin receiver in
	 *             the package that requires
	 *             the permission
	 *             {@link android.Manifest.permission#BIND_DEVICE_ADMIN}.
	 */
	@Deprecated
	...
	/**
	 * A ComponentName extra indicating the device admin receiver of the mobile
	 * device management
	 * application that will be set as the profile owner or device owner and active
	 * admin.
	 *
	 * <p>
	 * If an application starts provisioning directly via an intent with action
	 ...
	 * component has to match the package name of the application that started
	 * provisioning.
	 *
	 * <p>
	 * This component is set as device owner and active admin when device owner
	 * provisioning is
	 * started by an intent with action {@link #ACTION_PROVISION_MANAGED_DEVICE} or
	 * by an NFC
	 * message containing an NFC record with MIME type
	 * {@link #MIME_TYPE_PROVISIONING_NFC}. For the NFC record, the component name
	 * must be
	...
	/**
	 * An {@link android.accounts.Account} extra holding the account to migrate
	 * during managed
	 * profile provisioning. If the account supplied is present in the primary user,
	 * it will be
	 * copied, along with its credentials to the managed profile and removed from
	 * the primary user.
	 *
	 * Use with {@link #ACTION_PROVISION_MANAGED_PROFILE}, with managed account
	 * provisioning, or
	 * return as an extra to the intent result from the
	 * {@link #ACTION_GET_PROVISIONING_MODE}
	 * activity.
	 */
	...
	/**
	 * Boolean extra to indicate that the migrated account should be kept. This is
	 * used in
	 * conjunction with {@link #EXTRA_PROVISIONING_ACCOUNT_TO_MIGRATE}. If it's set
	 * to {@code true},
	 * the account will not be removed from the primary user after it is migrated to
	 * the newly
	 ...
	 *
	 * <p>
	 * Use with {@link #ACTION_PROVISION_MANAGED_PROFILE} or set as an extra to the
	...
	/**
	 * @deprecated From {@link android.os.Build.VERSION_CODES#O}, never used while
	 *             provisioning the
	 *             device.
	 */
	@Deprecated
	...
	/**
	 * A integer extra indicating the predominant color to show during the
	 * provisioning.
	 * Refer to {@link android.graphics.Color} for how the color is represented.
	 *
	 * <p>
	 * Use with {@link #ACTION_PROVISION_MANAGED_PROFILE} or
	 * {@link #ACTION_PROVISION_MANAGED_DEVICE}.
	 *
	 * @deprecated Color customization is no longer supported in the provisioning
	 *             flow.
	 */
	@Deprecated
	...
	/**
	 * A Boolean extra that can be used by the mobile device management application
	 * to skip the
	 * disabling of system apps during provisioning when set to {@code true}.
	 *
	 * <p>
	 * Use in an NFC record with {@link #MIME_TYPE_PROVISIONING_NFC}, an intent with
	 * action
	 * {@link #ACTION_PROVISION_MANAGED_PROFILE} that starts profile owner
	 * provisioning or
	 * set as an extra to the intent result of the
	...
	/**
	 * A String extra holding the time zone {@link android.app.AlarmManager} that
	 * the device
	 * will be set to.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * Use in an NFC record with {@link #MIME_TYPE_PROVISIONING_NFC} that starts
	 * device owner
	...
	/**
	 * A Long extra holding the wall clock time (in milliseconds) to be set on the
	 * device's
	...
	/**
	 * A String extra holding the {@link java.util.Locale} that the device will be
	 * set to.
	...
	/**
	 * A String extra holding the ssid of the wifi network that should be used
	 * during nfc device
	 ...
	 * {@link #EXTRA_PROVISIONING_WIFI_SSID} and could be one of {@code NONE},
	 * {@code WPA},
	 ...
	 * {@link #EXTRA_PROVISIONING_WIFI_SSID}
	 * and could be one of {@code NONE}, {@code PAP}, {@code MSCHAP},
	 * {@code MSCHAPV2}, {@code GTC},
	 ...
	 * {@link #EXTRA_PROVISIONING_WIFI_SSID}. This should
	 * be an X.509 certificate Base64 encoded DER format, ie. PEM representation of
	 * a certificate
	 ...
	 * {@link #EXTRA_PROVISIONING_WIFI_SSID}. This
	 * should be an X.509 certificate and private key Base64 encoded DER format, ie.
	 * PEM
	 * representation of a certificate and key without header, footer and line
	 * breaks. <a href=
	 * "https://tools.ietf.org/html/rfc7468"> More information</a> This is only
	...
	/**
	 * The identity of the wifi network in {@link #EXTRA_PROVISIONING_WIFI_SSID}.
	 * This is only used
	 ...
	 * {@link #EXTRA_PROVISIONING_WIFI_SSID}. This is
	 * only used if the {@link #EXTRA_PROVISIONING_WIFI_SECURITY_TYPE} is
	 * {@code EAP}.
	 *
	 * <p>
	 * Use in an NFC record with {@link #MIME_TYPE_PROVISIONING_NFC} that starts
	...
	/**
	 * The domain of the wifi network in {@link #EXTRA_PROVISIONING_WIFI_SSID}. This
	 * is only used if
	...
	/**
	 * A String extra holding the proxy auto-config (PAC) URL for the wifi network
	 * in
	 * {@link #EXTRA_PROVISIONING_WIFI_SSID}.
	 *
	 * <p>
	 ...
	 * A String extra holding a url that specifies the download location of the
	 * device admin
	 * package. When not provided it is assumed that the device admin package is
	 * already installed.
	 *
	 * <p>
	 * Use in an NFC record with {@link #MIME_TYPE_PROVISIONING_NFC} that starts
	...
	/**
	 * A String extra holding the localized name of the organization under
	 * management.
	 *
	 * The name is displayed only during provisioning.
	 *
	 * <p>
	 * Use in an intent with action {@link #ACTION_PROVISION_FINANCED_DEVICE}
	 *
	 * @hide
	 */
	@SystemApi
	...
	/**
	 * A String extra holding a url to the website of the device provider so the
	 * user can open it
	...
	/**
	 * A String extra holding the localized name of the device admin package. It
	 * should be the same
	...
	/**
	 * A {@link Uri} extra pointing to the app icon of device admin package. This
	 * image will be
	 ...
	 * <li>android.resource
	 * ({@link android.content.ContentResolver#SCHEME_ANDROID_RESOURCE})</li>
	 * </ul>
	 *
	 * <p>
	 ...
	 * {@link Intent#FLAG_GRANT_READ_URI_PERMISSION} and the uri should be added to
	 * the
	...
	/**
	 * An int extra holding a minimum required version code for the device admin
	 * package. If the
	 * device admin is already installed on the device, it will only be
	 * re-downloaded from
	 * {@link #EXTRA_PROVISIONING_DEVICE_ADMIN_PACKAGE_DOWNLOAD_LOCATION} if the
	 * version of the
	...
	/**
	 * A String extra holding the URL-safe base64 encoded SHA-256 hash of the file
	 * at download
	 * location specified in
	 * {@link #EXTRA_PROVISIONING_DEVICE_ADMIN_PACKAGE_DOWNLOAD_LOCATION}.
	 *
	 * <p>
	 * Either this extra or
	 * {@link #EXTRA_PROVISIONING_DEVICE_ADMIN_SIGNATURE_CHECKSUM} must be
	 * present. The provided checksum must match the checksum of the file at the
	 * download
	 * location. If the checksum doesn't match an error will be shown to the user
	 * and the user will
	 ...
	 *
	 * <p>
	 * <strong>Note:</strong> for devices running
	 * {@link android.os.Build.VERSION_CODES#LOLLIPOP}
	 * and {@link android.os.Build.VERSION_CODES#LOLLIPOP_MR1} only SHA-1 hash is
	 * supported.
	 * Starting from {@link android.os.Build.VERSION_CODES#M}, this parameter
	 * accepts SHA-256 in
	 * addition to SHA-1. From {@link android.os.Build.VERSION_CODES#Q}, only
	 * SHA-256 hash is
	 * supported.
	 */
	...
	/**
	 * A boolean extra indicating the admin of a fully-managed device opts out of
	 * controlling
	 * permission grants for sensor-related permissions,
	 * see {@link #setPermissionGrantState(ComponentName, String, String, int)}.
	 *
	 * The default for this extra is {@code false} - by default, the admin of a
	 * fully-managed
	 * device has the ability to grant sensors-related permissions.
	 *
	 * <p>
	 * Use only for device owner provisioning. This extra can be returned by the
	 * admin app when performing the admin-integrated provisioning flow as a result
	 * of the
	 * {@link #ACTION_GET_PROVISIONING_MODE} activity.
	 *
	 * <p>
	 ...
	 * <p>
	 * Either this extra or
	 * {@link #EXTRA_PROVISIONING_DEVICE_ADMIN_PACKAGE_CHECKSUM} must be
	 * present. The provided checksum must match the checksum of any signature of
	 * the file at
	 * the download location. If the checksum does not match an error will be shown
	 * to the user and
	...
	/**
	 * Broadcast Action: This broadcast is sent to indicate that provisioning of a
	 * managed profile
	 * has completed successfully.
	 *
	 * <p>
	 * The broadcast is limited to the primary profile, to the app specified in the
	 * provisioning
	 ...
	 * <ul>
	 * <li>{@link Intent#EXTRA_USER}, corresponds to the {@link UserHandle} of the
	 * managed
	 * profile.</li>
	 * <li>{@link #EXTRA_PROVISIONING_ACCOUNT_TO_MIGRATE}, corresponds to the
	 * account requested to
	...
	/**
	 * Activity action: This activity action is sent to indicate that provisioning
	 * of a managed
	 * profile or managed device has completed successfully. It'll be sent at the
	 * same time as
	 * {@link DeviceAdminReceiver#ACTION_PROFILE_PROVISIONING_COMPLETE} broadcast
	 * but this will be
	...
	/**
	 * A boolean extra indicating whether device encryption can be skipped as part
	 * of device owner
	 * or managed profile provisioning.
	 *
	 * <p>
	 * Use in an NFC record with {@link #MIME_TYPE_PROVISIONING_NFC} or an intent
	 * with action
	 * {@link #ACTION_PROVISION_MANAGED_DEVICE} that starts device owner
	 * provisioning.
	 *
	 * <p>
	 * From {@link android.os.Build.VERSION_CODES#N} onwards, this is also supported
	 * for an
	 * intent with action {@link #ACTION_PROVISION_MANAGED_PROFILE}.
	 *
	 * <p>
	 * This extra can also be returned by the admin app when performing the
	 * admin-integrated
	 * provisioning flow as a result of the {@link #ACTION_GET_PROVISIONING_MODE}
	 * activity.
	 */
	public static final String EXTRA_PROVISIONING_SKIP_ENCRYPTION = "android.app.extra.PROVISIONING_SKIP_ENCRYPTION";

	/**
	 * A {@link Uri} extra pointing to a logo image. This image will be shown during
	 * the
	 ...
	 *
	 * <p>
	 * Use in an intent with action {@link #ACTION_PROVISION_MANAGED_PROFILE} or
	 * {@link #ACTION_PROVISION_MANAGED_DEVICE}
	 *
	 * @deprecated Logo customization is no longer supported in the provisioning
	 *             flow.
	 */
	@Deprecated
	...
	/**
	 * A {@link Bundle}[] extra consisting of list of disclaimer headers and
	 * disclaimer contents.
	 * Each {@link Bundle} must have both
	 * {@link #EXTRA_PROVISIONING_DISCLAIMER_HEADER}
	 * as disclaimer header, and {@link #EXTRA_PROVISIONING_DISCLAIMER_CONTENT} as
	 * disclaimer
	 * content.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * Call {@link Bundle#putParcelableArray(String, Parcelable[])} to put the
	 * {@link Bundle}[]
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * Can be used in an intent with action
	 * {@link #ACTION_PROVISION_MANAGED_PROFILE}. This
	 * extra can also be returned by the admin app when performing the
	 * admin-integrated
	 * provisioning flow as a result of the {@link #ACTION_GET_PROVISIONING_MODE}
	 * activity.
	 */
	 ...
	 *
	 * <p>
	 * The extra is typically the company name of mobile device management
	 * application (MDM)
	 ...
	 *
	 * <p>
	 * System app, i.e. application with {@link ApplicationInfo#FLAG_SYSTEM}, can
	 * also insert a
	 * disclaimer by declaring an application-level meta-data in
	 * {@code AndroidManifest.xml}.
	 * Must use it with {@link #EXTRA_PROVISIONING_DISCLAIMER_CONTENT}. Here is the
	 * example:
	 *
	 * <pre>
	 *  &lt;meta-data
	 ...
	 *
	 * <p>
	 * If a <code>content:</code> URI is passed, URI is passed, the intent should
	 * have the flag
	 * {@link Intent#FLAG_GRANT_READ_URI_PERMISSION} and the uri should be added to
	 * the
	 * {@link android.content.ClipData} of the intent too.
	 ...
	 * disclaimer by declaring an application-level meta-data in
	 * {@code AndroidManifest.xml}.
	 * Must use it with {@link #EXTRA_PROVISIONING_DISCLAIMER_HEADER}. Here is the
	 * example:
	 *
	 * <pre>
	 *  &lt;meta-data
	...
	/**
	 * A boolean extra indicating if the user consent steps from the provisioning
	 * flow should be
	 * skipped. If unspecified, defaults to {@code false}.
	 *
	 * It can only be used by an existing device owner trying to create a managed
	 * profile via
	 * {@link #ACTION_PROVISION_MANAGED_PROFILE}. Otherwise it is ignored.
	 *
	 * @deprecated this extra is no longer relevant as device owners cannot create
	 *             managed profiles
	 */
	@Deprecated
	...
	/**
	 * A boolean extra indicating if the education screens from the provisioning
	 * flow should be
	 * skipped. If unspecified, defaults to {@code false}.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * If the education screens are skipped, it is the admin application's
	 * responsibility
	...
	/**
	 * A boolean extra indicating if mobile data should be used during the
	 * provisioning flow
	 * for downloading the admin app. If {@link #EXTRA_PROVISIONING_WIFI_SSID} is
	 * also specified,
	 ...
	 *
	 * <p>
	 * If this extra is set to {@code true} and
	 * {@link #EXTRA_PROVISIONING_WIFI_SSID} is not
	 * specified, this extra has different behaviour depending on the way
	 * provisioning is triggered:
	 * <ul>
	 * <li>
	 * For provisioning started via a QR code or an NFC tag, mobile data is always
	 * used for
	 * downloading the admin app.
	 * </li>
	 * <li>
	 * For all other provisioning methods, a mobile data connection check is made at
	 * the start
	 * of provisioning. If mobile data is connected at that point, the admin app
	 * download will
	 * happen using mobile data. If mobile data is not connected at that point, the
	 * end-user
	 * will be asked to pick a wifi network and the admin app download will proceed
	 * over wifi.
	 * </li>
	 * </ul>
	 */
	 ...
	 * {@link #PROVISIONING_TRIGGER_CLOUD_ENROLLMENT},
	 * {@link #PROVISIONING_TRIGGER_QR_CODE},
	...
	/**
	 * A value for {@link #EXTRA_PROVISIONING_TRIGGER} indicating that the
	 * provisioning
	 ...
	 * <p>
	 * This constant is meant to represent a specific type of managed account
	 * provisioning which
	 * provisions a device to a device owner by invoking the standard provisioning
	 * flow (where
	 * the ManagedProvisioning component downloads and installs the admin app), as
	 * opposed to
	 * relying on the provisioning trigger to handle download and install of the
	 * admin app.
	 * <p>
	 * As of {@link android.os.Build.VERSION_CODES#S}, this constant is no longer
	 * used in favor
	 * of the more general {@link #PROVISIONING_TRIGGER_MANAGED_ACCOUNT} which
	 * handles all managed
	 * account provisioning types.
	 * 
	 * @deprecated Use the broader {@link #PROVISIONING_TRIGGER_MANAGED_ACCOUNT}
	 *             instead
	 * @see #PROVISIONING_TRIGGER_CLOUD_ENROLLMENT
	 * @see #PROVISIONING_TRIGGER_QR_CODE
	 * @see #PROVISIONING_TRIGGER_UNSPECIFIED
	...
	/**
	 * Flag for {@link #EXTRA_PROVISIONING_SUPPORTED_MODES} indicating that
	 * provisioning is
	 * organization-owned.
	 *
	 * <p>
	 * Using this value indicates the admin app can only be provisioned in either a
	 * fully-managed device or a corporate-owned work profile. This will cause the
	 * admin app's
	 ...
	 * #PROVISIONING_MODE_MANAGED_PROFILE} and
	 * {@link #PROVISIONING_MODE_FULLY_MANAGED_DEVICE}.
	 *
	 * <p>
	 * This flag can be combined with
	 * {@link #FLAG_SUPPORTED_MODES_PERSONALLY_OWNED}. In
	 * that case, the admin app's {@link #ACTION_GET_PROVISIONING_MODE} activity
	 * will have
	 * the {@link #EXTRA_PROVISIONING_ALLOWED_PROVISIONING_MODES} array extra
	 * contain {@link
	 * #PROVISIONING_MODE_MANAGED_PROFILE},
	 * {@link #PROVISIONING_MODE_FULLY_MANAGED_DEVICE} and
	 ...
	 *
	 * <p>
	 * Using this flag will cause the admin app's
	 * {@link #ACTION_GET_PROVISIONING_MODE}
	 * activity to have the {@link #EXTRA_PROVISIONING_ALLOWED_PROVISIONING_MODES}
	 * array extra
	 * contain only {@link #PROVISIONING_MODE_MANAGED_PROFILE}.
	 *
	 * <p>
	 * Also, if this flag is set, the admin app's
	 * {@link #ACTION_GET_PROVISIONING_MODE} activity
	 ...
	 * <p>
	 * This flag can be combined with
	 * {@link #FLAG_SUPPORTED_MODES_ORGANIZATION_OWNED}. In
	 * that case, the admin app's {@link #ACTION_GET_PROVISIONING_MODE} activity
	 * will have the
	 * {@link #EXTRA_PROVISIONING_ALLOWED_PROVISIONING_MODES} array extra contain
	 * {@link
	 * #PROVISIONING_MODE_MANAGED_PROFILE},
	 * {@link #PROVISIONING_MODE_FULLY_MANAGED_DEVICE} and
	 * {@link #PROVISIONING_MODE_MANAGED_PROFILE_ON_PERSONAL_DEVICE}.
	 ...
	 *
	 * <p>
	 * When returned from {@link #getMinimumRequiredWifiSecurityLevel()}, the
	 * constant
	...
	/**
	 * Constant for {@link #getMinimumRequiredWifiSecurityLevel()} and
	 * {@link #setMinimumRequiredWifiSecurityLevel(int)}: personal network such as
	 * WEP, WPA2-PSK.
	 *
	 * <p>
	 * When returned from {@link #getMinimumRequiredWifiSecurityLevel()}, the
	...
	/**
	 * Constant for {@link #getMinimumRequiredWifiSecurityLevel()} and
	 * {@link #setMinimumRequiredWifiSecurityLevel(int)}: enterprise 192 bit
	 * network.
	 *
	 * <p>
	 * When returned from {@link #getMinimumRequiredWifiSecurityLevel()}, the
	 ...
	 * A device owner has full control over the device. The device owner can not be
	 * modified by the
	 * user and the only way of resetting the device is if the device owner app
	 * calls a factory
	 * reset.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * The NFC record must contain a serialized {@link java.util.Properties} object
	 * which
	 ...
	 * <li>{@link #EXTRA_PROVISIONING_WIFI_EAP_METHOD}, optional, supported from
	 * {@link
	 * android.os.Build.VERSION_CODES#Q}</li>
	 * <li>{@link #EXTRA_PROVISIONING_WIFI_PHASE2_AUTH}, optional, supported from
	 * {@link
	 * android.os.Build.VERSION_CODES#Q}</li>
	 * <li>{@link #EXTRA_PROVISIONING_WIFI_CA_CERTIFICATE}, optional, supported from
	 * {@link
	 * android.os.Build.VERSION_CODES#Q}</li>
	 * <li>{@link #EXTRA_PROVISIONING_WIFI_USER_CERTIFICATE}, optional, supported
	 * from {@link
	 * android.os.Build.VERSION_CODES#Q}</li>
	 * <li>{@link #EXTRA_PROVISIONING_WIFI_IDENTITY}, optional, supported from
	 * {@link
	 * android.os.Build.VERSION_CODES#Q}</li>
	 * <li>{@link #EXTRA_PROVISIONING_WIFI_ANONYMOUS_IDENTITY}, optional, supported
	 * from {@link
	 * android.os.Build.VERSION_CODES#Q}</li>
	 ...
	 * {@link #EXTRA_PROVISIONING_DEVICE_ADMIN_PACKAGE_NAME}, (although specifying
	 * only
	 * {@link #EXTRA_PROVISIONING_DEVICE_ADMIN_PACKAGE_NAME} is still supported).
	 */
	...
	/**
	 * Activity action: ask the user to add a new device administrator to the
	 * system.
	 ...
	 * immediately (without
	 * user intervention). However, if your administrator has been updated and is
	 * requesting
	 * additional uses-policy flags, the user will be presented with the new list.
	 * New policies
	 * will not be available to the updated administrator until the user has
	...
	/**
	 * @hide
	 *       Activity action: ask the user to add a new device administrator as the
	 *       profile owner
	 *       for this user. Only system apps can launch this intent.
	 *
	 *       <p>
	 *       The ComponentName of the profile owner admin is passed in the
	 *       {@link #EXTRA_DEVICE_ADMIN}
	 *       extra field. This will invoke a UI to bring the user through adding the
	 *       profile owner admin
	 *       to remotely control restrictions on the user.
	 *
	 *       <p>
	 *       The intent must be invoked via {@link Activity#startActivityForResult}
	 *       to receive the
	 *       result of whether or not the user approved the action. If approved, the
	 *       result will
	 *       be {@link Activity#RESULT_OK} and the component will be set as an
	 *       active admin as well
	 *       as a profile owner.
	 *
	 *       <p>
	 *       You can optionally include the {@link #EXTRA_ADD_EXPLANATION}
	 ...
	 *
	 *       <p>
	 *       If there is already a profile owner active or the caller is not a
	 *       system app, the
	 ...
	 *
	 * If the profile owner of an organization-owned managed profile changes some
	 * user
	 * restriction explicitly on the parent user, this broadcast will <em>not</em>
	 * be
	 ...
	 * 
	 * @see #ACTION_PROVISION_MANAGED_DEVICE
	 * @see DevicePolicyManager#transferOwnership(ComponentName, ComponentName,
	 *      PersistableBundle)
	 */
	...
	/**
	 * Broadcast action: sent when the factory reset protection (FRP) policy is
	 * changed.
	 *
	...
	/**
	 * Broadcast action: sent when there is a location update on a device in lost
	 * mode. This
	...
	/**
	 * Extra used with {@link #ACTION_LOST_MODE_LOCATION_UPDATE} to send the
	...
	/**
	 * Constant to indicate the feature of disabling screen captures. Used as
	 * argument to
	 * {@link #createAdminSupportIntent(String)}.
	 * 
	...
	/**
	 * A String indicating a specific restricted feature. Can be a user restriction
	 * from the
	 * {@link UserManager}, e.g. {@link UserManager#DISALLOW_ADJUST_VOLUME}, or one
	 * of the values
	 ...
	 *
	 * <p>
	 * For admin apps, this activity should be launched after using {@link
	 * #setPasswordQuality(ComponentName, int)}, or {@link
	 * #setPasswordMinimumLength(ComponentName, int)} to have the user enter a new
	 * password that
	 * meets the current requirements. You can use
	 * {@link #isActivePasswordSufficient()} to
	 * determine whether you need to have the user select a new password in order to
	 * meet the
	 * current constraints. Upon being resumed from this activity, you can check the
	 * new
	 * password characteristics to see if they are sufficient.
	 *
	 * <p>
	 * Non-admin apps can use {@link #getPasswordComplexity()} to check the current
	 * screen lock
	 * complexity, and use this activity with extra
	 * {@link #EXTRA_PASSWORD_COMPLEXITY} to suggest
	 * to users how complex the app wants the new screen lock to be. Note that both
	 * {@link
	 * #getPasswordComplexity()} and the extra {@link #EXTRA_PASSWORD_COMPLEXITY}
	 * require the
	 * calling app to have the permission
	...
	/**
	 * An integer indicating the complexity level of the new password an app would
	 * like the user to
	 ...
	 *
	 * <p>
	 * When returned from {@link #getPasswordComplexity()}, the constant represents
	 * the exact complexity band the password is in.
	 * When passed to {@link #setRequiredPasswordComplexity(int), it sets the
	 * minimum complexity
	 ...
	 * <ul>
	 * <li>PIN with <b>no</b> repeating (4444) or ordered (1234, 4321, 2468)
	 * sequences, length at
	 * least 4
	...
	/**
	 * A boolean extra for {@link #ACTION_SET_NEW_PARENT_PROFILE_PASSWORD}
	 * requesting that only
	 * device password requirement is enforced during the parent profile password
	 * enrolment flow.
	 * <p>
	 * Normally when enrolling password for the parent profile, both the device-wide
	 * password
	 * requirement (requirement set via
	 * {@link #getParentProfileInstance(ComponentName)} instance)
	 * and the profile password requirement are enforced, if the profile currently
	 * does not have a
	 * separate work challenge. By setting this to {@code true}, profile password
	 * requirement is
	...
	/**
	 * Indicates that nearby streaming is not controlled by policy, which means
	 * nearby streaming is
	 * allowed.
	 */
	...
	/**
	 * Indicates that nearby streaming is enabled only to devices offering a
	 * comparable level of
	 ...
	 * entering a new password for the parent of the profile. The caller can
	 * optionally
	 * set {@link #EXTRA_DEVICE_PASSWORD_REQUIREMENT_ONLY} to only enforce
	 * device-wide
	...
	/**
	 * Broadcast action: Tell the status bar to open the device monitoring dialog,
	 * e.g. when
	...
	/**
	 * Broadcast Action: Sent after application delegation scopes are changed. The
	 * new delegation
	...
	/**
	 * An {@code ArrayList<String>} corresponding to the delegation scopes given to
	 * an app in the
	...
	/**
	 * Broadcast action: notify that a new local system update policy has been set
	 * by the device
	 ...
	 * {@link UserManager#DISALLOW_SHARE_INTO_MANAGED_PROFILE} restriction has been
	 * successfully
	...
	/**
	 * Broadcast action: notify that a value of
	 * {@link Settings.Global#DEVICE_POLICY_CONSTANTS}
	 * has been changed.
	 * 
	 * @hide
	 */
	...
	/**
	 * Permission policy to prompt user for new permission requests for runtime
	 * permissions.
	...
	/**
	 * Permission policy to always grant new permission requests for runtime
	 * permissions.
	 * Already granted or denied permissions are not affected by this.
	 */
	...
	/**
	 * Permission policy to always deny new permission requests for runtime
	 * permissions.
	 * Already granted or denied permissions are not affected by this.
	 */
	...
	/**
	 * Delegation of certificate installation and management. This scope grants
	 * access to the
	 * {@link #getInstalledCaCerts}, {@link #hasCaCertInstalled},
	 * {@link #installCaCert},
	 * {@link #uninstallCaCert}, {@link #uninstallAllUserCaCerts} and
	 * {@link #installKeyPair} APIs.
	 * This scope also grants the ability to read identifiers that the delegating
	 * device owner or
	...
	/**
	 * Delegation of application restrictions management. This scope grants access
	 * to the
	 * {@link #setApplicationRestrictions} and {@link #getApplicationRestrictions}
	 * APIs.
	 */
	...
	/**
	 * Delegation of permission policy and permission grant state. This scope grants
	 * access to the
	 ...
	 * {@link #isApplicationHidden}, {@link #setApplicationHidden},
	 * {@link #isPackageSuspended}, and
	...
	/**
	 * Delegation of management of uninstalled packages. This scope grants access to
	 * the
	 * {@link #setKeepUninstalledPackages} and {@link #getKeepUninstalledPackages}
	 * APIs.
	 */
	public static final String DELEGATION_KEEP_UNINSTALLED_PACKAGES = "delegation-keep-uninstalled-packages";

	/**
	 * Grants access to {@link #setNetworkLoggingEnabled},
	 * {@link #isNetworkLoggingEnabled} and
	 * {@link #retrieveNetworkLogs}. Once granted the delegated app will start
	 * receiving
	 * DelegatedAdminReceiver.onNetworkLogsAvailable() callback, and Device owner or
	 * Profile Owner
	 * will no longer receive the DeviceAdminReceiver.onNetworkLogsAvailable()
	 * callback.
	 ...
	 *
	 * <p>
	 * Device Owner can grant this access since Android 10. Profile Owner of a
	 * managed profile
	 ...
	 * {@link DelegatedAdminReceiver#onChoosePrivateKeyAlias}. The caller (PO/DO)
	 * will
	 ...
	 * it will lose the delegation when a new app is delegated.
	 * <p>
	 * The delegaetd app can also call {@link #grantKeyPairToApp} and
	...
	/**
	 * Grants access to {@link #setSecurityLoggingEnabled},
	 * {@link #isSecurityLoggingEnabled},
	 * {@link #retrieveSecurityLogs}, and {@link #retrievePreRebootSecurityLogs}.
	 * Once granted the
	 * delegated app will start receiving
	 * {@link DelegatedAdminReceiver#onSecurityLogsAvailable}
	 * callback, and Device owner or Profile Owner will no longer receive the
	 * {@link DeviceAdminReceiver#onSecurityLogsAvailable} callback. There can be at
	 * most one app
	 * that has this delegation. If another app already had delegated security
	 * logging access, it
	 * will lose the delegation when a new app is delegated.
	 *
	 * <p>
	 * Can only be granted by Device Owner or Profile Owner of an organization-owned
	 * managed profile.
	 */
	 ...
	 * <p>
	 * This is used as an intermediate state after
	 * {@link #STATE_USER_PROFILE_COMPLETE} once the
	 ...
	 *
	 * <p>
	 * Unknown error code returned for {@link #ACTION_PROVISION_MANAGED_DEVICE},
	 * {@link #ACTION_PROVISION_MANAGED_PROFILE} and
	 ...
	 *
	 * <p>
	 * Returned for {@link #ACTION_PROVISION_MANAGED_DEVICE},
	 * {@link #ACTION_PROVISION_MANAGED_PROFILE} and
	 * {@link #ACTION_PROVISION_MANAGED_USER}
	 ...
	 *
	 * <p>
	 * Returned for {@link #ACTION_PROVISION_MANAGED_DEVICE} when the device already
	 * has a
	 * device owner.
	 *
	 * @hide
	 ...
	 *
	 * <p>
	 * Returned for {@link #ACTION_PROVISION_MANAGED_DEVICE} when the user has a
	 * profile owner
	 * and for {@link #ACTION_PROVISION_MANAGED_PROFILE} when the profile owner is
	 * already set.
	 *
	 * @hide
	 */
	 ...
	 *
	 * <p>
	 * Returned for {@link #ACTION_PROVISION_MANAGED_DEVICE} when the user isn't
	 * running.
	 *
	 * @hide
	 */
	 ...
	 *
	 * <p>
	 * Returned for {@link #ACTION_PROVISION_MANAGED_DEVICE} if the device has
	 * already been
	 * setup and for {@link #ACTION_PROVISION_MANAGED_USER} if the user has already
	 * been setup.
	 *
	 * @hide
	 */
	...
	/**
	 * Code used to indicate that the device also has a user other than the system
	 * user.
	 *
	 * @hide
	 */
	 ...
	 *
	 * <p>
	 * Returned for {@link #ACTION_PROVISION_MANAGED_DEVICE} if the user is not a
	 * system user.
	 *
	 * @hide
	 */
	 ...
	 *
	 * <p>
	 * Returned for {@link #ACTION_PROVISION_MANAGED_DEVICE} and
	 * {@link #ACTION_PROVISION_MANAGED_USER} when the device is a watch and is
	 * already paired.
	 *
	 * @hide
	 */
	 ...
	 *
	 * <p>
	 * Returned for {@link #ACTION_PROVISION_MANAGED_PROFILE} and
	 * {@link #ACTION_PROVISION_MANAGED_USER} on devices which do not support
	 * managed users.
	 *
	 ...
	 *
	 * <p>
	 * Returned for {@link #ACTION_PROVISION_MANAGED_USER} if the user is a system
	 * user and
	 * for {@link #ACTION_PROVISION_MANAGED_DEVICE} on devices running headless
	 * system user mode
	 ...
	 *
	 * <p>
	 * Returned for {@link #ACTION_PROVISION_MANAGED_PROFILE} when the user cannot
	 * have more
	 * managed profiles.
	 *
	 * @hide
	 ...
	 * <p>
	 * Returned for {@link #ACTION_PROVISION_MANAGED_DEVICE},
	 * {@link #ACTION_PROVISION_MANAGED_PROFILE} on devices which do not support
	 * device
	 * admins.
	 *
	 * @hide
	 ...
	 *
	 * <p>
	 * Returned for {@link #ACTION_PROVISION_MANAGED_DEVICE} when provisioning a DPC
	 * which does
	...
	/**
	 * Result codes for {@link #checkProvisioningPrecondition} indicating all the
	 * provisioning pre
	 * conditions.
	 *
	 * @hide
	...
	/**
	 * Disable all configurable SystemUI features during LockTask mode. This
	 * includes,
	 * <ul>
	...
	/**
	 * Enable the system info area in the status bar during LockTask mode. The
	 * system info area
	 * usually occupies the right side of the status bar (although this can differ
	 * across OEMs). It
	 * includes all system information indicators, such as date and time,
	...
	/**
	 * Enable notifications during LockTask mode. This includes notification icons
	 * on the status
	 * bar, heads-up notifications, and the expandable notification shade. Note that
	 * the Quick
	 * Settings panel remains disabled. This feature flag can only be used in
	 * combination with
	 * {@link #LOCK_TASK_FEATURE_HOME}.
	 * {@link #setLockTaskFeatures(ComponentName, int)}
	 * throws an {@link IllegalArgumentException} if this feature flag is defined
	 * without
	 * {@link #LOCK_TASK_FEATURE_HOME}.
	 *
	 * @see #setLockTaskFeatures(ComponentName, int)
	...
	/**
	 * Enable the Home button during LockTask mode. Note that if a custom launcher
	 * is used, it has
	 * to be registered as the default launcher with
	 * {@link #addPersistentPreferredActivity(ComponentName, IntentFilter, ComponentName)},
	 * and its
	...
	/**
	 * Enable the Overview button and the Overview screen during LockTask mode. This
	 * feature flag
	...
	/**
	 * Enable the global actions dialog during LockTask mode. This is the dialog
	 * that shows up when
	 * the user long-presses the power button, for example. Note that the user may
	 * not be able to
	 * power off the device if this flag is not set.
	 *
	 * <p>
	 * This flag is enabled by default until
	 * {@link #setLockTaskFeatures(ComponentName, int)} is
	...
	/**
	 * Enable the keyguard during LockTask mode. Note that if the keyguard is
	 * already disabled with
	 * {@link #setKeyguardDisabled(ComponentName, boolean)}, setting this flag will
	 * have no effect.
	 * If this flag is not set, the keyguard will not be shown even if the user has
	 * a lock screen
	 * credential.
	 *
	 * @see #setLockTaskFeatures(ComponentName, int)
	...
	/**
	 * Service action: Action for a service that device owner and profile owner can
	 * optionally
	 * own. If a device owner or a profile owner has such a service, the system
	 * tries to keep
	 * a bound connection to it, in order to keep their process always running.
	 * The service must be protected with the
	 * {@link android.Manifest.permission#BIND_DEVICE_ADMIN}
	 * permission.
	 */
	...
	/**
	 * Specifies that the device should attest its manufacturer details. For use
	 * with
	...
	/**
	 * Specifies that the device should attest using an individual attestation
	 * certificate.
	...
	/**
	 * Specifies that the calling app should be granted access to the installed
	 * credentials
	 * immediately. Otherwise, access to the credentials will be gated by user
	 * approval.
	 * For use with
	 * {@link #installKeyPair(ComponentName, PrivateKey, Certificate[], String, int)}
	 *
	...
	/**
	 * Specifies that a user can select the key via the Certificate Selection
	 * prompt.
	 * If this flag is not set when calling {@link #installKeyPair}, the key can
	 * only be granted
	...
	/**
	 * Specifies that the device owner configured a specific host to use for Private
	 * DNS.
	 */
	public static final int PRIVATE_DNS_MODE_PROVIDER_HOSTNAME = 3;

	/**
	 * Callback used in {@link #installSystemUpdate} to indicate that there was an
	 * error while
	...
	public abstract static class InstallSystemUpdateCallback {
		...
		/**
		 * Represents the update file being wrong; e.g. payloads are mismatched, or the
		 * wrong
		 ...
		 *
		 * <p>
		 * The given error message is not intended to be user-facing. It is intended to
		 * be
	 ...
	 * {@code PRIVATE_DNS_MODE_PROVIDER_HOSTNAME} then it implies the supplied host
	 * is valid
	 * and reachable.
	 */
	...
	/**
	 * If the {@code privateDnsHost} provided was of a valid hostname but that host
	 * was found
	...
	/**
	 * Activity action: Starts the administrator to get the mode for the
	 * provisioning.
	 ...
	 *
	 * <p>
	 * If performing fully-managed device provisioning and the admin app desires to
	 * show its
	 ...
	 *
	 * <p>
	 * The target activity may also return the account that needs to be migrated
	 * from primary
	 * user to managed profile in case of a profile owner provisioning in
	 * {@link #EXTRA_PROVISIONING_ACCOUNT_TO_MIGRATE} as result.
	 *
	 * <p>
	 * The target activity may also include the
	 * {@link #EXTRA_PROVISIONING_ADMIN_EXTRAS_BUNDLE}
	 * extra in the intent result. The values of this
	 * {@link android.os.PersistableBundle} will be
	 * sent as an intent extra of the same name to the
	 * {@link #ACTION_ADMIN_POLICY_COMPLIANCE}
	 * activity, along with the values of the
	 * {@link #EXTRA_PROVISIONING_ADMIN_EXTRAS_BUNDLE} extra
	 ...
	 * provisioning</li>
	 * <li>{@link #EXTRA_PROVISIONING_LEAVE_ALL_SYSTEM_APPS_ENABLED} for work
	 * profile
	 * provisioning</li>
	 * <li>{@link #EXTRA_PROVISIONING_SENSORS_PERMISSION_GRANT_OPT_OUT} for
	 * fully-managed
	 * device provisioning</li>
	 * <li>{@link #EXTRA_PROVISIONING_LOCALE} for fully-managed device
	 * provisioning</li>
	 * <li>{@link #EXTRA_PROVISIONING_LOCAL_TIME} for fully-managed device
	 * provisioning</li>
	 * <li>{@link #EXTRA_PROVISIONING_TIME_ZONE} for fully-managed device
	 * provisioning</li>
	 * </ul>
	 *
	 ...
	 * #EXTRA_PROVISIONING_ALLOWED_PROVISIONING_MODES}, which is provided as an
	 * intent extra to
	...
	/**
	 * An integer extra indication what provisioning modes should be available for
	 * the admin app
	 * to pick.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * The value of this extra will determine the contents of the {@link
	 * #EXTRA_PROVISIONING_ALLOWED_PROVISIONING_MODES} array that is passed to the
	 * admin app as an
	 * extra to its {@link #ACTION_GET_PROVISIONING_MODE} activity.
	 *
	 * <p>
	 * If one of the possible admin app choices is a personally-owned work profile,
	 * then the
	 * IMEI and serial number will not be passed to the admin app's {@link
	 * #ACTION_GET_PROVISIONING_MODE} activity via the
	 * {@link #EXTRA_PROVISIONING_IMEI} and {@link
	 ...
	 *
	 * <p>
	 * This extra is only respected when provided alongside the {@link
	...
	/**
	 * A boolean extra which determines whether to skip the ownership disclaimer
	 * screen during the
	 * provisioning flow. The default value is {@code false}.
	 *
	 * If the value is {@code true}, the provisioning initiator must display a
	 * device ownership
	...
	/**
	 * An {@link ArrayList} of {@link Integer} extra specifying the allowed
	 * provisioning modes.
	 * <p>
	 * This extra will be passed to the admin app's
	 * {@link #ACTION_GET_PROVISIONING_MODE}
	 * activity, whose result intent must contain {@link #EXTRA_PROVISIONING_MODE}
	 * set to one of
	 * the values in this array.
	 * <p>
	 * If the value set to {@link #EXTRA_PROVISIONING_MODE} is not in the array,
	 ...
	 * <p>
	 * This mode is only available when the provisioning initiator has explicitly
	 * instructed the
	 * provisioning flow to support managed profile on a personal device
	 * provisioning. In that case,
	 * {@link #PROVISIONING_MODE_MANAGED_PROFILE} corresponds to an
	 * organization-owned managed
	 * profile, whereas this constant corresponds to a personally-owned managed
	 * profile.
	 *
	...
	/**
	 * A {@code boolean} flag that indicates whether the provisioning flow should
	 * return before
	 * starting the admin app's {@link #ACTION_ADMIN_POLICY_COMPLIANCE} handler. The
	 * default value
	 * is {@code true}.
	 *
	 * <p>
	 * If this extra is set to {@code true}, then when the provisioning flow returns
	 * back to the
	 * provisioning initiator, provisioning will not be complete. The provisioning
	 * initiator can
	 * use this opportunity to do its own preparatory steps prior to the launch of
	 * the admin app's
	 * {@link #ACTION_ADMIN_POLICY_COMPLIANCE} handler. It is the responsibility of
	 * the
	 * provisioning initiator to ensure that the provisioning flow is then resumed
	 * and completed.
	 *
	 * <p>
	 * If this extra is set to {@code false}, then when the provisioning flow
	 * returns back to
	 * the provisioning initiator, provisioning will be complete. Note that device
	 * owner
	...
	/**
	 * A {@code boolean} flag that indicates whether the screen should be on
	 * throughout the
	 * provisioning flow.
	 *
	 * <p>
	 * This extra can either be passed as an extra to the {@link
	 * #ACTION_PROVISION_MANAGED_PROFILE} intent, or it can be returned by the
	 * admin app when performing the admin-integrated provisioning flow as a result
	 * of the
	 * {@link #ACTION_GET_PROVISIONING_MODE} activity.
	 *
	 * @deprecated from {@link android.os.Build.VERSION_CODES#UPSIDE_DOWN_CAKE}, the
	 *             flag wouldn't
	 *             be functional. The screen is kept on throughout the provisioning
	 *             flow.
	 */
	...
	/**
	 * Activity action: Starts the administrator to show policy compliance for the
	 * provisioning.
	 * This action is used any time that the administrator has an opportunity to
	 * show policy
	 * compliance before the end of setup wizard. This could happen as part of the
	 * admin-integrated
	 * provisioning flow (in which case this gets sent after
	 * {@link #ACTION_GET_PROVISIONING_MODE}),
	 * or it could happen during provisioning finalization if the administrator
	 * supports
	 * finalization during setup wizard.
	 *
	 * <p>
	 * Intents with this action may also be supplied with the {@link
	 ...
	 *
	 * <p>
	 * The activity must handle the device policy management role holder update and
	 * set the
	 * intent result. This can include {@link Activity#RESULT_OK} if the update was
	 * successful,
	 * {@link #RESULT_UPDATE_DEVICE_POLICY_MANAGEMENT_ROLE_HOLDER_RECOVERABLE_ERROR}
	 * if
	 * it encounters a problem that may be solved by relaunching it again, {@link
	 * #RESULT_UPDATE_DEVICE_POLICY_MANAGEMENT_ROLE_HOLDER_PROVISIONING_DISABLED} if
	 * role holder
	 * provisioning is disabled, or {@link
	 * #RESULT_UPDATE_DEVICE_POLICY_MANAGEMENT_ROLE_HOLDER_UNRECOVERABLE_ERROR} if
	 * it encounters
	 * any other problem that will not be solved by relaunching it again.
	 *
	 * <p>
	 * If this activity has additional internal conditions which are not met, it
	 * should return
	...
	/**
	 * Result code that can be returned by the {@link
	 * #ACTION_UPDATE_DEVICE_POLICY_MANAGEMENT_ROLE_HOLDER} handler if it encounters
	 * a problem
	...
	/**
	 * Result code that can be returned by the {@link
	 * #ACTION_UPDATE_DEVICE_POLICY_MANAGEMENT_ROLE_HOLDER} handler if role holder
	 * provisioning
	 * is disabled.
	 *
	 * @hide
	 ...
	 *
	 * <p>
	 * The failure strategy specifies how the platform should handle a failed device
	 * policy
	 ...
	 *
	 * <p>
	 * This extra may be supplied as part of the {@link
	 * #ACTION_UPDATE_DEVICE_POLICY_MANAGEMENT_ROLE_HOLDER} result intent.
	 *
	 * <p>
	 * Default value is
	 * {@link #ROLE_HOLDER_UPDATE_FAILURE_STRATEGY_FAIL_PROVISIONING}.
	 *
	 * @hide
	 */
	...
	/**
	 * A value for {@link #EXTRA_ROLE_HOLDER_UPDATE_FAILURE_STRATEGY} indicating
	 * that upon
	 ...
	 * A value for {@link #EXTRA_ROLE_HOLDER_UPDATE_FAILURE_STRATEGY} indicating
	 * that upon
	 * failure to update the role holder, provisioning should fallback to be
	 * platform-driven.
	 *
	 * @hide
	 */
	...
	/**
	 * An {@code int} extra which contains the result code of the last attempt to
	 * update
	 * the device policy management role holder via {@link
	 * #ACTION_UPDATE_DEVICE_POLICY_MANAGEMENT_ROLE_HOLDER}.
	 *
	 * <p>
	 * This extra is provided to the device policy management role holder via either
	 * {@link
	 * #ACTION_ROLE_HOLDER_PROVISION_MANAGED_DEVICE_FROM_TRUSTED_SOURCE} or {@link
	 * #ACTION_ROLE_HOLDER_PROVISION_MANAGED_PROFILE} when started after the role
	 * holder
	 * had previously returned {@link #RESULT_UPDATE_ROLE_HOLDER}.
	 *
	 * <p>
	 * If the role holder update had failed, the role holder can use the value of
	 * this extra to
	 * make a decision whether to fail the provisioning flow or to carry on with the
	 * older version
	 * of the role holder.
	 *
	 * <p>
	 ...
	 * <li>{@link #RESULT_UPDATE_DEVICE_POLICY_MANAGEMENT_ROLE_HOLDER_RECOVERABLE_ERROR}
	 * if it
	 * encounters a problem that may be solved by relaunching it again.
	 * <li>{@link #RESULT_UPDATE_DEVICE_POLICY_MANAGEMENT_ROLE_HOLDER_UNRECOVERABLE_ERROR}
	 * if
	 ...
	 *
	 * <p>
	 * If this extra is provided to the device policy management role holder via
	 * either {@link
	 * #ACTION_ROLE_HOLDER_PROVISION_MANAGED_DEVICE_FROM_TRUSTED_SOURCE} or {@link
	 * #ACTION_ROLE_HOLDER_PROVISION_MANAGED_PROFILE}, the device policy management
	 * role holder must
	 * launch this intent which shows the custom user consent screen, replacing its
	 * own standard
	 * consent screen.
	 *
	 * <p>
	 * If this extra is provided, it is the responsibility of the intent handler to
	 * show the
	 * list of disclaimers which are normally shown by the standard consent screen:
	 * <ul>
	 * <li>Disclaimers set by the IT admin via the
	 * {@link #EXTRA_PROVISIONING_DISCLAIMERS}
	 * provisioning extra</li>
	 * <li>For fully-managed device provisioning, disclaimers defined in system apps
	 * via the
	 ...
	 *
	 * <p>
	 * When this {@link Intent} is started, the following intent extras will be
	 * supplied:
	 * <ul>
	 * <li>{@link #EXTRA_PROVISIONING_DISCLAIMERS}</li>
	 ...
	 *
	 * <p>
	 * If the custom consent screens are granted by the user
	 * {@link Activity#RESULT_OK} is
	 * returned, otherwise {@link Activity#RESULT_CANCELED} is returned. The device
	 * policy
	 * management role holder should ensure that the provisioning flow terminates
	 * immediately if
	 ...
	 *
	 * <p>
	 * This intent can be accompanied by any of the relevant provisioning extras
	 * related to
	 ...
	 *
	 * <p>
	 * If there are provisioning extras related to network connectivity, this
	 * activity
	 * attempts to connect to the specified network. Otherwise it prompts the
	 * end-user to connect.
	 *
	 * <p>
	 * This activity is meant to be started by the provisioning initiator prior to
	 * starting
	 * {@link #ACTION_PROVISION_MANAGED_PROFILE} or {@link
	 * #ACTION_PROVISION_MANAGED_DEVICE_FROM_TRUSTED_SOURCE}.
	 *
	 * <p>
	 * Note that network connectivity is still also handled when provisioning via
	 * {@link
	 * #ACTION_PROVISION_MANAGED_PROFILE} or {@link
	 * #ACTION_PROVISION_MANAGED_DEVICE_FROM_TRUSTED_SOURCE}. {@link
	 * #ACTION_ESTABLISH_NETWORK_CONNECTION} should only be used in cases when the
	 * provisioning
	 * initiator would like to do some additional logic after the network
	 * connectivity step and
	 * before the start of provisioning.
	 *
	 * If network connection is established, {@link Activity#RESULT_OK} will be
	 * returned. Otherwise
	...
	/**
	 * Service Action: Service implemented by a device owner or profile owner
	 * supervision app to
	...
	/**
	 * Return value for {@link #getPersonalAppsSuspendedReasons} when personal apps
	 * are not
	 * suspended.
	 */
	...
	/**
	 * Flag for {@link #getPersonalAppsSuspendedReasons} return value. Set when
	 * personal
	 * apps are suspended by an admin explicitly via
	 * {@link #setPersonalAppsSuspended}.
	 */
	 ...
	 * Flag for {@link #getPersonalAppsSuspendedReasons} return value. Set when
	 * personal apps are
	 * suspended by framework because managed profile was off for longer than
	 * allowed by policy.
	 * 
	...
	/**
	 * Indicates that a {@link UnsafeStateException} was thrown because the
	 * operation would distract
	...
	/**
	 * Prevent an app from dismissible notifications. Starting from Android U,
	 * notifications with
	 * the ongoing parameter can be dismissed by a user on an unlocked device. An
	 * app with
	...
	/**
	 * Exempt an app from all power-related restrictions, including app standby and
	 * doze.
	 * In addition, the app will be able to start foreground services from the
	 * background,
	...
	/**
	 * Broadcast action: notify system apps (e.g. settings, SysUI, etc) that the
	 * device management
	 * resources with IDs {@link #EXTRA_RESOURCE_IDS} has been updated, the updated
	 * resources can be
	 ...
	 *
	 * <p>
	 * {@link #EXTRA_RESOURCE_TYPE} will be included to identify the type of
	 * resource being
	 * updated.
	 */
	@SdkConstant(SdkConstantType.BROADCAST_INTENT_ACTION)
	...
	/**
	 * An {@code int} extra for {@link #ACTION_DEVICE_POLICY_RESOURCE_UPDATED} to
	 * indicate the type
	 * of the resource being updated, the type can be
	 * {@link #EXTRA_RESOURCE_TYPE_DRAWABLE} or
	 * {@link #EXTRA_RESOURCE_TYPE_STRING}
	 */
	public static final String EXTRA_RESOURCE_TYPE = "android.app.extra.RESOURCE_TYPE";

	/**
	 * A {@code int} value for {@link #EXTRA_RESOURCE_TYPE} to indicate that a
	 * resource of type
	...
	/**
	 * An integer array extra for {@link #ACTION_DEVICE_POLICY_RESOURCE_UPDATED} to
	 * indicate which
	...
	/**
	 * Broadcast Action: Broadcast sent to indicate that the device financing state
	 * has changed.
	 *
	 * <p>
	 * This occurs when, for example, a financing kiosk app has been added or
	 * removed.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * This will be delivered to the following apps if they include a receiver for
	 * this action
	 * in their manifest:
	 * <ul>
	...
	/**
	 * Require that MTE be enabled on the device, if supported. Can be set by a
	 * device owner or a
	...
	/** @hide */
	@IntDef(prefix = { "MTE_" }, value = { MTE_ENABLED, MTE_DISABLED, MTE_NOT_CONTROLLED_BY_POLICY })
	@Retention(RetentionPolicy.SOURCE)
	 ...
	 * set the Memory
	 * Tagging Extension (MTE) policy. MTE is a CPU extension that allows to protect
	 * against certain
	 * classes of security problems at a small runtime performance cost overhead.
	 *
	 * <p>
	 * The MTE policy can only be set to {@link #MTE_DISABLED} if called by a device
	 * owner.
	 ...
	 *
	 * @throws SecurityException             if caller is not permitted to set Mte
	 *                                       policy
	 ...
	 * binder caches in this class derive from this Config, so all can be
	 * invalidated or
	 * disabled through this Config.
	 */
	private static final IpcDataCache.Config sDpmCaches = new IpcDataCache.Config(8, IpcDataCache.MODULE_SYSTEM,
			"DevicePolicyManagerCaches");
	 ...
	 *
	 * <p>
	 * <b>Note:</b> notice that the operation safety state might change between the
	 * time this
	 * method returns and the operation's method is called, so calls to the latter
	 * could still throw
	 ...
	 *
	 * @return whether it's safe to run operations that can be affected by the given
	 *         {@code reason}.
	 */
	...
	/**
	 * Acknoledges that the new managed user disclaimer was viewed by the (human)
	 * user
	 * so that {@link #ACTION_SHOW_NEW_USER_DISCLAIMER broadcast} is not sent again
	 * the next time
	...
	/**
	 * Return true if the given administrator component is currently active
	 * (enabled) in the system.
	 *
	 * @param admin The administrator component to check for.
	 * @return {@code true} if {@code admin} is currently enabled in the system,
	 *         {@code false}
	 *         otherwise
	 */
	 ...
	 *
	 * <p>
	 * Note that the operation is not synchronous and the admin might still be
	 * active (as
	 * indicated by {@link #getActiveAdmins()}) by the time this method returns.
	 *
	 * @param admin The administration compononent to remove.
	 * @throws SecurityException if the caller is not in the owner application of
	 *                           {@code admin}.
	 */
	 ...
	 * This can be
	 * used to check whether the administrator was activated under an earlier set of
	 * policies, but
	 * requires additional policies after an upgrade.
	 *
	 * @param admin      Which {@link DeviceAdminReceiver} this request is
	 *                   associated with. Must be an
	 *                   active administrator, or an exception will be thrown.
	 * @param usesPolicy Which uses-policy to check, as defined in
	 *                   {@link DeviceAdminInfo}.
	...
	/**
	 * Constant for {@link #setPasswordQuality}: the policy allows for low-security
	 * biometric
	 * recognition technology. This implies technologies that can recognize the
	 * identity of
	 * an individual to about a 3 digit PIN (false detection is less than 1 in
	 * 1,000).
	 ...
	 * of password or pattern, but doesn't care what it is. Note that quality
	 * constants
	...
	/**
	 * Constant for {@link #setPasswordQuality}: the policy requires the DevTitans
	 * pattern option. Note that quality constants
	 * are ordered so that higher values are more restrictive.
	 */

	public static final int DEV_TITANS = 0x15000;
	/**
	...
	/**
	 * Constant for {@link #setPasswordQuality}: allows the admin to set precisely
	 * how many
	 * characters of various types the password should contain to satisfy the
	 * policy. The admin
	 ...
	 * @hide
	 *
	 *       adb shell dpm set-{device,profile}-owner will normally not allow
	 *       installing an owner to
	 *       a user with accounts.
	 *       {@link #ACCOUNT_FEATURE_DEVICE_OR_PROFILE_OWNER_ALLOWED}
	 *       and {@link #ACCOUNT_FEATURE_DEVICE_OR_PROFILE_OWNER_DISALLOWED} are the
	 *       account features
	 *       used by authenticator to exempt their accounts from this:
	 *
	 *       <ul>
	 *       <li>Non-test-only DO/PO still can't be installed when there are
	 *       accounts.
	 *       <p>
	 *       In order to make an apk test-only, add android:testOnly="true" to the
	 *       &lt;application&gt; tag in the manifest.
	 *
	 *       <li>Test-only DO/PO can be installed even when there are accounts, as
	 *       long as all the
	 *       accounts have the
	 *       {@link #ACCOUNT_FEATURE_DEVICE_OR_PROFILE_OWNER_ALLOWED} feature.
	 *       Some authenticators claim to have any features, so to detect it, we
	 *       also check
	 *       {@link #ACCOUNT_FEATURE_DEVICE_OR_PROFILE_OWNER_DISALLOWED} and
	 *       disallow installing
	...
	/**
	 * A {@code boolean} metadata to be included in a mainline module's
	 * {@code <application>}
	 * manifest element, which declares that the module should be considered a
	 * required app for
	 * managed users.
	 * <p>
	 ...
	 * restrictions it
	 * is imposing. After setting this, the user will not be able to enter a new
	 * password that is
	 ...
	 * thus the highest
	 * requested quality constant (between the policy set here, the user's
	 * preference, and any other
	 * considerations) is the one that is in effect.
	 * <p>
	 * On devices not supporting {@link PackageManager#FEATURE_SECURE_LOCK_SCREEN}
	 * feature, the
	 ...
	 * <p>
	 * Apps targeting {@link android.os.Build.VERSION_CODES#R} and below can call
	 * this method on the
	 * {@link DevicePolicyManager} instance returned by
	 * {@link #getParentProfileInstance(ComponentName)} in order to set restrictions
	 * on the parent
	 * profile. Apps targeting {@link android.os.Build.VERSION_CODES#S} and above,
	 * with the
	 * exception of a profile owner on an organization-owned device (as can be
	 * identified by
	 ...
	 *
	 * <p>
	 * <strong>Note:</strong> Specifying password requirements using this method
	 * clears the
	 * password complexity requirements set using
	 * {@link #setRequiredPasswordComplexity(int)}.
	 * If this method is called on the {@link DevicePolicyManager} instance returned
	 * by
	 * {@link #getParentProfileInstance(ComponentName)}, then password complexity
	 * requirements
	 * set on the primary {@link DevicePolicyManager} must be cleared first by
	 * calling
	 * {@link #setRequiredPasswordComplexity} with {@link #PASSWORD_COMPLEXITY_NONE)
	 * first.
	 *
	 * <p>
	 * <string>Note:</strong> this method is ignored on
	 * {PackageManager#FEATURE_AUTOMOTIVE automotive builds}.
	 *
	 * @deprecated Prefer using {@link #setRequiredPasswordComplexity(int)}, to
	 *             require a password
	 *             that satisfies a complexity level defined by the platform, rather
	 *             than specifying custom
	 *             password requirement.
	 *             Setting custom, overly-complicated password requirements leads to
	 *             passwords that are hard
	 *             for users to remember and may not provide any security benefits
	 *             given as Android uses
	 *             hardware-backed throttling to thwart online and offline
	 *             brute-forcing of the device's
	 *             screen lock. Company-owned devices (fully-managed and
	 *             organization-owned managed profile
	 *             devices) are able to continue using this method, though it is
	 *             recommended that
	 *             {@link #setRequiredPasswordComplexity(int)} should be used
	 *             instead.
	 *
	 * @param admin   Which {@link DeviceAdminReceiver} this request is associated
	 *                with.
	 * @param quality The new desired quality. One of
	 *                {@link #PASSWORD_QUALITY_UNSPECIFIED},
	 *                {@link #PASSWORD_QUALITY_BIOMETRIC_WEAK},
	 *                {@link #PASSWORD_QUALITY_SOMETHING},
	 *                {@link #PASSWORD_QUALITY_NUMERIC},
	 *                {@link #PASSWORD_QUALITY_NUMERIC_COMPLEX},
	 *                {@link #PASSWORD_QUALITY_ALPHABETIC},
	 *                {@link #PASSWORD_QUALITY_ALPHANUMERIC} or
	 *                {@link #PASSWORD_QUALITY_COMPLEX}.
	 * @throws SecurityException     if {@code admin} is not an active
	 *                               administrator, if {@code admin}
	 *                               does not use
	 *                               {@link DeviceAdminInfo#USES_POLICY_LIMIT_PASSWORD}
	 *                               or if the
	 *                               calling app is targeting
	 *                               {@link android.os.Build.VERSION_CODES#S} and
	 *                               above,
	 *                               and is calling the method the
	 *                               {@link DevicePolicyManager} instance returned
	 *                               by
	 *                               {@link #getParentProfileInstance(ComponentName)}.
	 * @throws IllegalStateException if the caller is trying to set password quality
	 *                               on the parent
	 *                               {@link DevicePolicyManager} instance while
	 *                               password complexity was set on the
	...
	/**
	 * Retrieve the current minimum password quality for a particular admin or all
	 * admins that set
	 * restrictions on this user and its participating profiles. Restrictions on
	 * profiles that have
	 * a separate challenge are not taken into account.
	 *
	 * <p>
	 * This method can be called on the {@link DevicePolicyManager} instance
	 * returned by {@link #getParentProfileInstance(ComponentName)} in order to
	 * retrieve
	 * restrictions on the parent profile.
	 *
	 * <p>
	 * Note: on devices not supporting
	 ...
	 * allowed password
	 * length. After setting this, the user will not be able to enter a new password
	 * that is not at
	 * least as restrictive as what has been set. Note that the current password
	 * will remain until
	 * the user has set a new one, so the change does not take place immediately. To
	 * prompt the user
	 ...
	 * This constraint is
	 * only imposed if the administrator has also requested either
	 * {@link #PASSWORD_QUALITY_NUMERIC}
	 * , {@link #PASSWORD_QUALITY_NUMERIC_COMPLEX},
	 * {@link #PASSWORD_QUALITY_ALPHABETIC},
	 ...
	 * {@link android.os.Build.VERSION_CODES#R} and above enforces this constraint
	 * without settings
	 ...
	 * @param admin  Which {@link DeviceAdminReceiver} this request is associated
	 *               with.
	 * @param length The new desired minimum password length. A value of 0 means
	 *               there is no
	 *               restriction.
	 * @throws SecurityException     if {@code admin} is not an active administrator
	 *                               or {@code admin}
	 ...
	 *                               {@link android.os.Build.VERSION_CODES#R} and
	 *                               above and didn't set a sufficient password
	 *                               quality requirement prior to calling this
	 *                               method.
	 */
	@Deprecated
	...
	/**
	 * Retrieve the current minimum password length for a particular admin or all
	 * admins that set
	 * restrictions on this user and its participating profiles. Restrictions on
	 * profiles that have
	 * a separate challenge are not taken into account.
	 ...
	 * Called by an application that is administering the device to set the minimum
	 * number of upper
	 * case letters required in the password. After setting this, the user will not
	 * be able to enter
	 ...
	 * setting this value. This constraint is only imposed if the administrator has
	 * also requested
	 * {@link #PASSWORD_QUALITY_COMPLEX} with {@link #setPasswordQuality}. If an app
	 * targeting
	 * SDK level {@link android.os.Build.VERSION_CODES#R} and above enforces this
	 * constraint without
	 * settings password quality to {@link #PASSWORD_QUALITY_COMPLEX} first, this
	 * method will throw
	 ...
	 * @param admin  Which {@link DeviceAdminReceiver} this request is associated
	 *               with.
	 * @param length The new desired minimum number of upper case letters required
	 *               in the password.
	 ...
	 * its participating profiles. Restrictions on profiles that have a separate
	 * challenge
	 ...
	 * @param admin  Which {@link DeviceAdminReceiver} this request is associated
	 *               with.
	 * @param length The new desired minimum number of lower case letters required
	 *               in the password.
	 *               A value of 0 means there is no restriction.
	 * @throws SecurityException     if {@code admin} is not an active administrator
	 *                               or {@code admin}
	 ...
	 * Called by an application that is administering the device to set the minimum
	 * number of
	 * letters required in the password. After setting this, the user will not be
	 * able to enter a
	 * new password that is not at least as restrictive as what has been set. Note
	 * that the current
	 ...
	 * {@link #PASSWORD_QUALITY_COMPLEX} with
	 * {@link #setPasswordQuality}. If an app targeting SDK level
	 * {@link android.os.Build.VERSION_CODES#R} and above enforces this constraint
	 * without settings
	 * password quality to {@link #PASSWORD_QUALITY_COMPLEX} first, this method will
	 * throw
	 ...
	 * Called by an application that is administering the device to set the minimum
	 * number of
	 * numerical digits required in the password. After setting this, the user will
	 * not be able to
	 * enter a new password that is not at least as restrictive as what has been
	 * set. Note that the
	 * current password will remain until the user has set a new one, so the change
	 * does not take
	 * place immediately. To prompt the user for a new password, use
	 ...
	 * @param admin  Which {@link DeviceAdminReceiver} this request is associated
	 *               with.
	 * @param length The new desired minimum number of numerical digits required in
	 *               the password. A
	 ...
	 * Called by an application that is administering the device to set the minimum
	 * number of
	 * symbols required in the password. After setting this, the user will not be
	 * able to enter a
	 * new password that is not at least as restrictive as what has been set. Note
	 * that the current
	 * password will remain until the user has set a new one, so the change does not
	 ...
	 * After setting
	 * this, the user will not be able to enter a new password that is not at least
	 * as restrictive
	 * as what has been set. Note that the current password will remain until the
	 * user has set a new
	 * one, so the change does not take place immediately. To prompt the user for a
	 * new password,
	 * use {@link #ACTION_SET_NEW_PASSWORD} or
	 * {@link #ACTION_SET_NEW_PARENT_PROFILE_PASSWORD} after
	 * setting this value. This constraint is only imposed if the administrator has
	 * also requested
	 * {@link #PASSWORD_QUALITY_COMPLEX} with {@link #setPasswordQuality}. If an app
	...
	/**
	 * Returns minimum PasswordMetrics that satisfies all admin policies.
	 * If requested, only consider device-wide admin policies and ignore policies
	 * set on the
	 ...
	 * of the password
	 * history. After setting this, the user will not be able to enter a new
	 * password that is the
	 * same as any password in the history. Note that the current password will
	 * remain until the
	 * user has set a new one, so the change does not take place immediately. To
	 * prompt the user for
	 ...
	 * @param admin  Which {@link DeviceAdminReceiver} this request is associated
	 *               with.
	 * @param length The new desired length of password history. A value of 0 means
	 *               there is no
	 *               restriction.
	 * @throws SecurityException if {@code admin} is not an active administrator or
	 *                           {@code admin}
	 *                           does not use
	 *                           {@link DeviceAdminInfo#USES_POLICY_LIMIT_PASSWORD}
	 */
	 ...
	 * method will
	 * restart the countdown for password expiration for the given admin, as will
	 * changing the
	 * device password (for all admins).
	 * <p>
	 * The provided timeout is the time delta in ms and will be added to the current
	 * time. For
	 * example, to have the password expire 5 days from now, timeout would be 5 *
	 * 86400 * 1000 =
	 ...
	 * for all active
	 * admins. Active admins do not need to explicitly call this method in that
	 * case.
	 * <p>
	 * This method can be called on the {@link DevicePolicyManager} instance
	 * returned by
	 ...
	 * @param timeout The limit (in ms) that a password can remain in effect. A
	 *                value of 0 means
	 *                there is no restriction (unlimited).
	 * @throws SecurityException if {@code admin} is not an active administrator or
	 *                           {@code admin}
	 *                           does not use
	 *                           {@link DeviceAdminInfo#USES_POLICY_EXPIRE_PASSWORD}
	 */
	@RequiresFeature(PackageManager.FEATURE_SECURE_LOCK_SCREEN)
	...
	/**
	 * Get the password expiration timeout for the given admin. The expiration
	 * timeout is the
	 * recurring expiration timeout provided in the call to
	 * {@link #setPasswordExpirationTimeout(ComponentName, long)} for the given
	 * admin or the
	 * aggregate of all participating policy administrators if {@code admin} is
	 * null. Admins that
	 * have set restrictions on profiles that have a separate challenge are not
	 * taken into account.
	 *
	 * <p>
	 * This method can be called on the {@link DevicePolicyManager} instance
	...
	/**
	 * Get the current password expiration time for a particular admin or all admins
	 * that set
	 * restrictions on this user and its participating profiles. Restrictions on
	 * profiles that have
	 * a separate challenge are not taken into account. If admin is {@code null},
	 * then a composite
	 * of all expiration times is returned - which will be the minimum of all of
	 * them.
	 *
	 * <p>
	 * This method can be called on the {@link DevicePolicyManager} instance
	 ...
	 * admins that
	 * set restrictions on this user and its participating profiles. Restrictions on
	 * profiles that
	...
	/**
	 * Determines whether the calling user's current password meets policy
	 * requirements
	 * (e.g. quality, minimum length). The user must be unlocked to perform this
	 * check.
	 *
	 * <p>
	 * Policy requirements which affect this check can be set by admins of the user,
	 * but also
	 * by the admin of a managed profile associated with the calling user (when the
	 * managed profile
	 * doesn't have a separate work challenge). When a managed profile has a
	 * separate work
	 * challenge, its policy requirements only affect the managed profile.
	 *
	 * <p>
	 * Depending on the user, this method checks the policy requirement against one
	 * of the
	 * following passwords:
	 * <ul>
	 * <li>For the primary user or secondary users: the personal keyguard password.
	 * <li>For managed profiles: a work challenge if set, otherwise the parent
	 * user's personal
	 * keyguard password.
	 * <ul/>
	 * In other words, it's always checking the requirement against the password
	 * that is protecting
	 * the calling user.
	 *
	 * <p>
	 * Note that this method considers all policy requirements targeting the
	 * password in
	 * question. For example a profile owner might set a requirement on the parent
	 * profile i.e.
	 * personal keyguard but not on the profile itself. When the device has a weak
	 * personal keyguard
	 * password and no separate work challenge, calling this method will return
	 * {@code false}
	 * despite the profile owner not setting a policy on the profile itself. This is
	 * because the
	 * profile's current password is the personal keyguard password, and it does not
	 * meet all policy
	 * requirements.
	 *
	 * <p>
	 * Device admins must request {@link DeviceAdminInfo#USES_POLICY_LIMIT_PASSWORD}
	 * before
	 * calling this method. Note, this policy type is deprecated for device admins
	 * in Android 9.0
	 ...
	 * This method can be called on the {@link DevicePolicyManager} instance
	 * returned by
	 * {@link #getParentProfileInstance(ComponentName)} in order to determine if the
	 * password set on
	 ...
	 * On devices not supporting {@link PackageManager#FEATURE_SECURE_LOCK_SCREEN}
	 * feature, the
	 * password is always treated as empty - i.e. this method will always return
	 * false on such
	 * devices, provided any password requirements were set.
	 *
	 * @return {@code true} if the password meets the policy requirements,
	 *         {@code false} otherwise
	 * @throws SecurityException     if the calling application isn't an active
	 *                               admin that uses
	 *                               {@link DeviceAdminInfo#USES_POLICY_LIMIT_PASSWORD}
	...
	/**
	 * Called by profile owner of a managed profile to determine whether the current
	 * device password
	 * meets policy requirements set explicitly device-wide.
	 * <p>
	 * This API is similar to {@link #isActivePasswordSufficient()}, with two
	 * notable
	 * differences:
	 * <ul>
	 * <li>this API always targets the device password. As a result it should always
	 * be called on
	 * the {@link #getParentProfileInstance(ComponentName)} instance.</li>
	 * <li>password policy requirement set on the managed profile is not taken into
	 * consideration
	 * by this API, even if the device currently does not have a separate work
	 * challenge set.</li>
	 * </ul>
	 *
	 * <p>
	 * This API is designed to facilite progressive password enrollment flows when
	 * the DPC
	 * imposes both device and profile password policies. DPC applies profile
	 * password policy by
	 * calling {@link #setPasswordQuality(ComponentName, int)} or
	 * {@link #setRequiredPasswordComplexity} on the regular
	 * {@link DevicePolicyManager} instance,
	 * while it applies device-wide policy by calling
	 * {@link #setRequiredPasswordComplexity} on the
	 * {@link #getParentProfileInstance(ComponentName)} instance. The DPC can
	 * utilize this check to
	 * guide the user to set a device password first taking into consideration the
	 * device-wide
	 * policy only, and then prompt the user to either upgrade it to be fully
	 * compliant, or enroll a
	 * separate work challenge to satisfy the profile password policy only.
	 *
	 * <p>
	 * The device user must be unlocked (@link
	 * {@link UserManager#isUserUnlocked(UserHandle)})
	 * to perform this check.
	 *
	 * @return {@code true} if the device password meets explicit requirement set on
	 *         it,
	 *         {@code false} otherwise.
	 * @throws SecurityException     if the calling application is not a profile
	 *                               owner of a managed
	 *                               profile, or if this API is not called on the
	 *                               parent DevicePolicyManager instance.
	 * @throws IllegalStateException if the user isn't unlocked
	 ...
	 *
	 * <p>
	 * Note that when called from a profile which uses an unified challenge with its
	 * parent, the
	 * screen lock complexity of the parent will be returned.
	 *
	 * <p>
	 * Apps need the {@link permission#REQUEST_PASSWORD_COMPLEXITY} permission to
	 * call this
	 * method. On Android {@link android.os.Build.VERSION_CODES#S} and above, the
	 * calling
	 * application does not need this permission if it is a device owner or a
	 * profile owner.
	 *
	 * <p>
	 * This method can be called on the {@link DevicePolicyManager} instance
	 ...
	 */
	@PasswordComplexity
	@RequiresPermission(anyOf = { MANAGE_DEVICE_POLICY_LOCK_CREDENTIALS,
			REQUEST_PASSWORD_COMPLEXITY }, conditional = true)
	 ...
	 * The complexity level is one of the pre-defined levels, and the user is unable
	 * to set a
	 * password with a lower complexity level.
	 *
	 * <p>
	 * Note that when called on a profile which uses an unified challenge with its
	 * parent, the
	 ...
	 * <strong>Note:</strong> Specifying password requirements using this method
	 * clears any
	 * password requirements set using the obsolete
	 * {@link #setPasswordQuality(ComponentName, int)}
	 * and any of its associated methods.
	 * Additionally, if there are password requirements set using the obsolete
	 * {@link #setPasswordQuality(ComponentName, int)} on the parent
	 * {@code DevicePolicyManager}
	 * instance, they must be cleared by calling
	 * {@link #setPasswordQuality(ComponentName, int)}
	 * with {@link #PASSWORD_QUALITY_UNSPECIFIED} on that instance prior to setting
	 * complexity
	 * requirement for the managed profile.
	 *
	 * @throws SecurityException        if the calling application is not a device
	 *                                  owner or a profile
	 *                                  owner.
	 * @throws IllegalArgumentException if the complexity level is not one of the
	 *                                  four above.
	 * @throws IllegalStateException    if the caller is trying to set password
	 *                                  complexity while there
	 *                                  are password requirements specified using
	 *                                  {@link #setPasswordQuality(ComponentName, int)}
	 *                                  on the parent {@code DevicePolicyManager}
	 *                                  instance.
	 */
	@RequiresPermission(value = MANAGE_DEVICE_POLICY_LOCK_CREDENTIALS, conditional = true)
	 ...
	 *
	 * <p>
	 * The difference between this method and {@link #getPasswordComplexity()} is
	 * that this
	 * method simply returns the value set by
	 * {@link #setRequiredPasswordComplexity(int)} while
	 * {@link #getPasswordComplexity()} returns the complexity of the actual
	 * password.
	 *
	 * <p>
	 * This method can be called on the {@link DevicePolicyManager} instance
	 ...
	 * restrictions on the parent profile.
	 *
	 * @throws SecurityException if the calling application is not a device owner or
	 *                           a profile
	 *                           owner.
	 */
	@PasswordComplexity
	...
	/**
	 * Returns the password complexity that applies to this user, aggregated from
	 * other users if
	 * necessary (for example, if the DPC has set password complexity requirements
	 * on the parent
	 * profile DPM instance of a managed profile user, they would apply to the
	 * primary user on the
	 * device).
	 * 
	 * @hide
	 ...
	 * profile DPM instance of a managed profile user, they would apply to the
	 * primary user on the
	 * device). If {@code deviceWideOnly} is {@code true}, ignore policies set on
	 * the
	 * managed profile DPM instance (as if the managed profile had separate work
	 * challenge).
	 * 
	 * @hide
	 */
	...
	/**
	 * When called by a profile owner of a managed profile returns true if the
	 * profile uses unified
	 * challenge with its parent user.
	 *
	 * <strong>Note</strong>: This method is not concerned with password quality and
	 * will return
	 * false if the profile has empty password as a separate challenge.
	 *
	 * @param admin Which {@link DeviceAdminReceiver} this request is associated
	 *              with.
	 * @throws SecurityException if {@code admin} is not a profile owner of a
	 *                           managed profile.
	...
	/**
	 * Returns whether the given user's credential will be sufficient for all
	 * password policy
	 * requirement, once the user's profile has switched to unified challenge.
	 *
	 * <p>
	 * This is different from {@link #isActivePasswordSufficient()} since once the
	 * profile
	 * switches to unified challenge, policies set explicitly on the profile will
	 * start to affect
	 * the parent user.
	 * 
	 * @param userHandle  the user whose password requirement will be checked
	 * @param profileUser the profile user whose lockscreen challenge will be
	 *                    unified.
	 * @hide
	 */
	 ...
	 * This method can be called on the {@link DevicePolicyManager} instance
	 * returned by
	 * {@link #getParentProfileInstance(ComponentName)} in order to retrieve the
	 * number of failed
	 * password attemts for the parent user.
	 * <p>
	 * The calling device admin must have requested
	 * {@link DeviceAdminInfo#USES_POLICY_WATCH_LOGIN}
	 * to be able to call this method; if it has not, a security exception will be
	 * thrown.
	 * <p>
	 * On devices not supporting {@link PackageManager#FEATURE_SECURE_LOCK_SCREEN}
	 * feature, the
	 * password is always empty and this method always returns 0.
	 *
	 * @return The number of times user has entered an incorrect password since the
	 *         last correct
	 *         password entry.
	 ...
	 * <p>
	 * The calling device admin must have requested
	 * {@link DeviceAdminInfo#USES_POLICY_WATCH_LOGIN} to be able to call this
	 * method; if it has
	...
	/**
	 * Queries whether {@link #RESET_PASSWORD_DO_NOT_ASK_CREDENTIALS_ON_BOOT} flag
	 * is set.
	 *
	...
	/**
	 * Setting this to a value greater than zero enables a policy that will perform
	 * a
	 * device or profile wipe after too many incorrect device-unlock passwords have
	 * been entered.
	 ...
	 * <p>
	 * When this policy is set on the system or the main user, the device will be
	 * factory reset
	 * after too many incorrect password attempts. When set on any other user, only
	 * the
	 * corresponding user or profile will be wiped.
	 * <p>
	 * To implement any other policy (e.g. wiping data for a particular application
	 * only, erasing or
	 * revoking credentials, or reporting the failure to a server), you should
	 * implement
	 * {@link DeviceAdminReceiver#onPasswordFailed(Context, android.content.Intent)}
	 * instead. Do not
	 * use this API, because if the maximum count is reached, the device or profile
	 * will be wiped
	 * immediately, and your callback will not be invoked.
	 * <p>
	 * This method can be called on the {@link DevicePolicyManager} instance
	 * returned by
	 * {@link #getParentProfileInstance(ComponentName)} in order to set a value on
	 * the parent
	 * profile.
	 * <p>
	 * On devices not supporting {@link PackageManager#FEATURE_SECURE_LOCK_SCREEN}
	 * feature, the
	 * password is always empty and this method has no effect - i.e. the policy is
	 * not set.
	 *
	 * @param admin Which {@link DeviceAdminReceiver} this request is associated
	 *              with. Null if the
	 *              caller is not a device admin.
	 * @param num   The number of failed password attempts at which point the device
	 *              or profile will
	 *              be wiped.
	 * @throws SecurityException if {@code admin} is not null, and {@code admin} is
	 *                           not an active
	 *                           administrator or does not use both
	 *                           {@link DeviceAdminInfo#USES_POLICY_WATCH_LOGIN} and
	 *                           {@link DeviceAdminInfo#USES_POLICY_WIPE_DATA}, or
	 *                           if {@code admin} is null and the
	 ...
	 * the device
	 * or profile is wiped, for a particular admin or all admins that set
	 * restrictions on this user
	 * and its participating profiles. Restrictions on profiles that have a separate
	 * challenge are
	 ...
	 * On devices not supporting {@link PackageManager#FEATURE_SECURE_LOCK_SCREEN}
	 * feature, the
	 * password is always empty and this method returns a default value (0)
	 * indicating that the
	 * policy is not set.
	 *
	 * @param admin The name of the admin component to check, or {@code null} to
	...
	/**
	 * Returns the user that will be wiped first when too many failed attempts are
	 * made to unlock
	 * user {@code userHandle}. That user is either the same as {@code userHandle}
	 * or belongs to the
	 * same profile group. When there is no such policy, returns
	 * {@code UserHandle.USER_NULL}.
	 * E.g. managed profile user may be wiped as a result of failed primary profile
	 * password
	 * attempts when using unified challenge. Primary user may be wiped as a result
	 * of failed
	...
	/**
	 * Flag for {@link #resetPasswordWithToken} and {@link #resetPassword}: don't
	 * allow other admins
	 ...
	 * If the flag is set, the device can be booted without asking for user
	 * password.
	 * The absence of this flag does not change the current boot requirements. This
	 * flag
	 * can be set by the device owner only. If the app is not the device owner, the
	 * flag
	 * is ignored. Once the flag is set, it cannot be reverted back without
	 * resetting the
	 ...
	 * entire device) or
	 * the work profile challenge on the current user. This takes effect
	 * immediately.
	 *
	 * <p>
	 * Before {@link android.os.Build.VERSION_CODES#N}, this API is available to
	 * device admin,
	 * profile owner and device owner. Starting from
	 * {@link android.os.Build.VERSION_CODES#N},
	 * legacy device admin (who is not also profile owner or device owner) can only
	 * call this
	 * API to set a new password if there is currently no password set. Profile
	 * owner and device
	 * owner can continue to force change an existing password as long as the target
	 * user is
	 * unlocked, although device owner will not be able to call this API at all if
	 * there is also a
	 * managed profile on the device.
	 *
	 * <p>
	 * Between {@link android.os.Build.VERSION_CODES#O},
	 * {@link android.os.Build.VERSION_CODES#P} and
	 * {@link android.os.Build.VERSION_CODES#Q},
	 * profile owner and devices owner targeting SDK level
	 * {@link android.os.Build.VERSION_CODES#O}
	 * or above who attempt to call this API will receive {@link SecurityException};
	 * they are
	 * encouraged to migrate to the new {@link #resetPasswordWithToken} API instead.
	 * Profile owner and device owner targeting older SDK levels are not affected:
	 * they continue
	 * to experience the existing behaviour described in the previous paragraph.
	 *
	 * <p>
	 * <em>Starting from {@link android.os.Build.VERSION_CODES#R}, this API is no
	 * longer
	 ...
	 * {@link android.os.Build.VERSION_CODES#O} or above, or they will receive a
	 * silent failure
	 * (API returning {@code false}) if they target lower SDK level.
	 * For legacy device admins, this API throws {@link SecurityException} if they
	 * target SDK level
	 * {@link android.os.Build.VERSION_CODES#N} or above, and returns {@code false}
	 * otherwise. Only
	 * privileged apps holding RESET_PASSWORD permission which are part of
	 * the system factory image can still call this API to set a new password if
	 * there is currently
	 * no password set. In this case, if the device already has a password, this API
	 * will throw
	 * {@link SecurityException}.
	 *
	 * <p>
	 * The given password must be sufficient for the current password quality and
	 * length constraints
	 * as returned by {@link #getPasswordQuality(ComponentName)} and
	 * {@link #getPasswordMinimumLength(ComponentName)}; if it does not meet these
	 * constraints, then
	 * it will be rejected and false returned. Note that the password may be a
	 * stronger quality
	 * (containing alphanumeric characters when the requested quality is only
	 * numeric), in which
	 ...
	 * <p>
	 * The calling device admin must have requested
	 * {@link DeviceAdminInfo#USES_POLICY_RESET_PASSWORD} to be able to call this
	 * method; if it has
	 * not, a security exception will be thrown.
	 *
	 * @param password The new password for the user. Null or empty clears the
	 *                 password.
	 * @param flags    May be 0 or combination of
	 *                 {@link #RESET_PASSWORD_REQUIRE_ENTRY} and
	 *                 {@link #RESET_PASSWORD_DO_NOT_ASK_CREDENTIALS_ON_BOOT}.
	 * @return Returns true if the password was applied, or false if it is not
	 *         acceptable for the
	 *         current constraints.
	 * @throws SecurityException     if the calling application does not own an
	 *                               active administrator
	 *                               that uses
	 *                               {@link DeviceAdminInfo#USES_POLICY_RESET_PASSWORD}
	 * @throws IllegalStateException if the calling user is locked or has a managed
	 *                               profile.
	...
	/**
	 * Called by a profile or device owner to provision a token which can later be
	 * used to reset the
	 * device lockscreen password (if called by device owner), or managed profile
	 * challenge (if
	 * called by profile owner), via {@link #resetPasswordWithToken}.
	 * <p>
	 * If the user currently has a lockscreen password, the provisioned token will
	 * not be
	 * immediately usable; it only becomes active after the user performs a confirm
	 * credential
	 * operation, which can be triggered by
	 * {@link KeyguardManager#createConfirmDeviceCredentialIntent}.
	 * If the user has no lockscreen password, the token is activated immediately.
	 * In all cases,
	 * the active state of the current token can be checked by
	 * {@link #isResetPasswordTokenActive}.
	 * For security reasons, un-activated tokens are only stored in memory and will
	 * be lost once
	 * the device reboots. In this case a new token needs to be provisioned again.
	 * <p>
	 * Once provisioned and activated, the token will remain effective even if the
	 * user changes
	 * or clears the lockscreen password.
	 * <p>
	 * <em>This token is highly sensitive and should be treated at the same level as
	 * user
	 * credentials. In particular, NEVER store this token on device in plaintext. Do
	 * not store
	 * the plaintext token in device-encrypted storage if it will be needed to reset
	 * password on
	 * file-based encryption devices before user unlocks. Consider carefully how any
	 * password token
	 * will be stored on your server and who will need access to them. Tokens may be
	 * the subject of
	 ...
	 *              with. Null if the
	 *              caller is not a device admin.
	 * @param token a secure token a least 32-byte long, which must be generated by
	 *              a
	 ...
	 * On devices not supporting {@link PackageManager#FEATURE_SECURE_LOCK_SCREEN}
	 * feature, this
	 * method has no effect - the reset token should not have been set in the first
	 * place - and
	 * false is returned.
	 *
	 * @param admin Which {@link DeviceAdminReceiver} this request is associated
	 ...
	 *              caller is not a device admin.
	 * @return true if the operation is successful, false otherwise.
	 * @throws SecurityException if admin is not a device or profile owner and if
	 *                           the caller does
	 *                           not the permission
	 *                           {@link android.Manifest.permission#MANAGE_DEVICE_POLICY_RESET_PASSWORD}.
	 */
	 ...
	 * @throws SecurityException     if admin is not a device or profile owner and
	 *                               not a holder of the
	 *                               permission
	 *                               {@link android.Manifest.permission#MANAGE_DEVICE_POLICY_RESET_PASSWORD}
	...
	/**
	 * Called by device or profile owner to force set a new device unlock password
	 * or a managed
	 * profile challenge on current user. This takes effect immediately.
	 * <p>
	 * Unlike {@link #resetPassword}, this API can change the password even before
	 * the user or
	 * device is unlocked or decrypted. The supplied token must have been previously
	 * provisioned via
	 * {@link #setResetPasswordToken}, and in active state
	 * {@link #isResetPasswordTokenActive}.
	 * <p>
	 * The given password must be sufficient for the current password quality and
	 * length constraints
	 ...
	 * constraints, then
	 * it will be rejected and false returned. Note that the password may be a
	 * stronger quality, for
	 * example, a password containing alphanumeric characters when the requested
	 * quality is only
	 * numeric.
	 * <p>
	 * Calling with a {@code null} or empty password will clear any existing PIN,
	 * pattern or
	 * password if the current password constraints allow it.
	 * <p>
	 * On devices not supporting {@link PackageManager#FEATURE_SECURE_LOCK_SCREEN}
	 * feature,
	 * calling this methods has no effect - the password is always empty - and false
	 * is returned.
	 *
	 * @param admin    Which {@link DeviceAdminReceiver} this request is associated
	 *                 with. Null if the
	 *                 caller is not a device admin.
	 * @param password The new password for the user. {@code null} or empty clears
	 *                 the password.
	 ...
	 * time for user
	 * activity until the device will lock. This limits the length that the user can
	 * set. It takes
	 * effect immediately.
	 * <p>
	 * A calling device admin must have requested
	 ...
	 *               with. Null if the
	 *               caller is not a device admin
	 * @param timeMs The new desired maximum time to lock in milliseconds. A value
	 *               of 0 means there
	 *               is no restriction.
	 * @throws SecurityException if {@code admin} is not an active administrator or
	 *                           it does not use
	...
	public void setMaximumTimeToLock(@Nullable ComponentName admin, long timeMs) {
		if (mService != null) {
			try {
				mService.setMaximumTimeToLock(admin, mContext.getPackageName(), timeMs,
						mParentInstance);
			} catch (RemoteException e) {
				throw e.rethrowFromSystemServer();
			}
	 ...
	 *              aggregate
	 *              all admins.
	 * @return time in milliseconds for the given admin or the minimum value
	 *         (strictest) of
	...
	/**
	 * Called by a device/profile owner to set the timeout after which unlocking
	 * with secondary, non
	 * strong auth (e.g. fingerprint, face, trust agents) times out, i.e. the user
	 * has to use a
	 * strong authentication method like password, pin or pattern.
	 *
	 * <p>
	 * This timeout is used internally to reset the timer to require strong auth
	 * again after
	 * specified timeout each time it has been successfully used.
	 *
	 * <p>
	 * Fingerprint can also be disabled altogether using
	 * {@link #KEYGUARD_DISABLE_FINGERPRINT}.
	 *
	 * <p>
	 * Trust agents can also be disabled altogether using
	 * {@link #KEYGUARD_DISABLE_TRUST_AGENTS}.
	 *
	 * <p>
	 * A calling device admin can verify the value it has set by calling
	 * {@link #getRequiredStrongAuthTimeout(ComponentName)} and passing in its
	 * instance.
	 *
	 * <p>
	 * This method can be called on the {@link DevicePolicyManager} instance
	 ...
	 *                  with. Null if the
	 *                  caller is not a device admin
	 * @param timeoutMs The new timeout in milliseconds, after which the user will
	 *                  have to unlock
	 *                  with strong authentication method. A value of 0 means the
	 *                  admin is not participating
	 *                  in controlling the timeout.
	 *                  The minimum and maximum timeouts are platform-defined and
	 *                  are typically 1 hour and
	 *                  72 hours, respectively. Though discouraged, the admin may
	 *                  choose to require strong
	 *                  auth at all times using
	 *                  {@link #KEYGUARD_DISABLE_FINGERPRINT} and/or
	 *                  {@link #KEYGUARD_DISABLE_TRUST_AGENTS}.
	 *
	...
	/**
	 * Determine for how long the user will be able to use secondary, non strong
	 * auth for
	 * authentication, since last strong method authentication (password, pin or
	 * pattern) was used.
	 * After the returned timeout the user is required to use strong authentication
	 * method.
	 *
	 * <p>
	 * This method can be called on the {@link DevicePolicyManager} instance
	 ...
	 * @return The timeout in milliseconds or 0 if not configured for the provided
	 *         admin.
	 */
	@RequiresFeature(PackageManager.FEATURE_SECURE_LOCK_SCREEN)
	...
	/**
	 * Flag for {@link #lockNow(int)}: also evict the user's credential encryption
	 * key from the
	 * keyring. The user's credential will need to be entered again in order to
	 * derive the
	 * credential encryption key that will be stored back in the keyring for future
	 * use.
	 * <p>
	 * This flag can only be used by a profile owner when locking a managed profile
	 * when
	 * {@link #getStorageEncryptionStatus} returns
	 * {@link #ENCRYPTION_STATUS_ACTIVE_PER_USER}.
	 * <p>
	 * In order to secure user data, the user will be stopped and restarted so apps
	 * should wait
	...
	/**
	 * Make the device lock immediately, as if the lock screen timeout has expired
	 * at the point of
	 * this call.
	 * <p>
	 * This method secures the device in response to an urgent situation, such as a
	 * lost or stolen
	 * device. After this method is called, the device must be unlocked using strong
	 * authentication
	 * (PIN, pattern, or password). This API is intended for use only by device
	 * admins.
	 * <p>
	 * From version {@link android.os.Build.VERSION_CODES#R} onwards, the caller
	 * must either have
	 * the LOCK_DEVICE permission or the device must have the device admin feature;
	 * if neither is
	 * true, then the method will return without completing any action. Before
	 * version
	 * {@link android.os.Build.VERSION_CODES#R}, the device needed the device admin
	 * feature,
	 ...
	 * thrown.
	 * <p>
	 * If there's no lock type set, this method forces the device to go to sleep but
	 * doesn't lock
	 * the device. Device admins who find the device in this state can lock an
	 * otherwise-insecure
	 * device by first calling {@link #resetPassword} to set the password and then
	 * lock the device.
	 * <p>
	 * This method can be called on the {@link DevicePolicyManager} instance
	 * returned by
	 * {@link #getParentProfileInstance(ComponentName)} in order to lock the parent
	 * profile.
	 * <p>
	 * NOTE: on {@link android.content.pm.PackageManager#FEATURE_AUTOMOTIVE
	 * automotive builds}, this
	 * method doesn't turn off the screen as it would be a driving safety
	 * distraction.
	 * <p>
	 ...
	 * device. After this method is called, the device must be unlocked using strong
	 * authentication
	 * (PIN, pattern, or password). This API is for use only by device admins and
	 * holders of the
	 ...
	 * <p>
	 * NOTE: In order to lock the parent profile and evict the encryption key of the
	 * managed
	 * profile, {@link #lockNow()} must be called twice: First, {@link #lockNow()}
	 * should be called
	 * on the {@link DevicePolicyManager} instance returned by
	 * {@link #getParentProfileInstance(ComponentName)}, then {@link #lockNow(int)}
	 * should be
	 * called on the {@link DevicePolicyManager} instance associated with the
	 * managed profile,
	 * with the {@link #FLAG_EVICT_CREDENTIAL_ENCRYPTION_KEY} flag.
	 * Calling the method twice in this order ensures that all users are locked and
	 * does not
	 * stop the device admin on the managed profile from issuing a second call to
	 * lock its own
	 * profile.
	 * <p>
	 * NOTE: on {@link android.content.pm.PackageManager#FEATURE_AUTOMOTIVE
	 ...
	 * @throws SecurityException             if the calling application does not own
	 *                                       an active administrator
	 *                                       that uses
	 *                                       {@link DeviceAdminInfo#USES_POLICY_FORCE_LOCK}
	 *                                       and the does not hold
	 *                                       the
	 *                                       {@link android.Manifest.permission#MANAGE_DEVICE_POLICY_LOCK}
	 *                                       permission, or
	 *                                       the
	 *                                       {@link #FLAG_EVICT_CREDENTIAL_ENCRYPTION_KEY}
	 *                                       flag is passed by an
	 *                                       application that is not a profile owner
	 *                                       of a managed profile.
	 ...
	 *                               and is
	 *                               not granted the
	 *                               {@link android.Manifest.permission#MASTER_CLEAR}
	 *                               or
	 ...
	 *
	 * <p>
	 * If called as a secondary user or managed profile, the user itself and its
	 * associated user
	 * data will be wiped. In particular, If the caller is a profile owner of an
	 * organization-owned managed profile, calling this method will relinquish the
	 * device for
	 * personal use, removing the managed profile and all policies set by the
	 * profile owner.
	 * </p>
	 *
	 * <p>
	 * Calling this method from the primary user will only work if the calling app
	 * is
	 * targeting SDK level {@link Build.VERSION_CODES#TIRAMISU} or below, in which
	 * case it will
	 * cause the device to reboot, erasing all device data - including all the
	 * secondary users
	 * and their data - while booting up. If an app targeting SDK level
	 * {@link Build.VERSION_CODES#UPSIDE_DOWN_CAKE} and above is calling this method
	 * from the
	 * primary user or last full user, {@link IllegalStateException} will be thrown.
	 * </p>
	 *
	 ...
	 * @param flags  Bit mask of additional options: currently supported flags are
	 *               {@link #WIPE_EXTERNAL_STORAGE},
	 *               {@link #WIPE_RESET_PROTECTION_DATA} and
	 *               {@link #WIPE_EUICC}.
	 ...
	 * requested
	 * {@link DeviceAdminInfo#USES_POLICY_WIPE_DATA} to be able to call this method;
	 * if it has
	 * not, a security exception will be thrown.
	 *
	 * @param flags Bit mask of additional options: currently supported flags are
	 ...
	 *                           administrator
	 *                           that uses
	 *                           {@link DeviceAdminInfo#USES_POLICY_WIPE_DATA} and
	 *                           is not
	 *                           granted the
	 *                           {@link android.Manifest.permission#MASTER_CLEAR}
	 *                           or both the
	...
	/**
	 * Internal function for {@link #wipeData(int)},
	 * {@link #wipeData(int, CharSequence)}
	...
	/**
	 * Callable by device owner or profile owner of an organization-owned device, to
	 * set a
	 ...
	 *               with. Null if the
	 *               caller is not a device admin
	 * @param policy the new FRP policy, or {@code null} to clear the current
	 *               policy.
	 * @throws SecurityException             if {@code admin} is not a device owner
	 *                                       or a profile owner of
	 ...
	 * the permission
	 * {@link android.Manifest.permission#MASTER_CLEAR}, in which case, it can pass
	 * {@code null}
	 ...
	 * @throws SecurityException             if {@code admin} is not a device owner,
	 *                                       a profile owner of
	 *                                       an organization-owned device or the FRP
	 *                                       management agent.
	 * @throws UnsupportedOperationException if factory reset protection is not
	 *                                       supported on the device.
	 */
	...
	/**
	 * Send a lost mode location update to the admin. This API is limited to
	 * organization-owned
	 * devices, which includes devices with a device owner or devices with a profile
	 * owner on an
	 * organization-owned managed profile.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * Register a broadcast receiver to receive lost mode location updates. This
	 * receiver should
	 * subscribe to the {@link #ACTION_LOST_MODE_LOCATION_UPDATE} action and receive
	 * the location
	 ...
	 * @param callback A callback object that will inform the caller whether a lost
	 *                 mode location
	 ...
	 * to set the proxy, the {@link ComponentName} of the admin originally setting
	 * the
	 ...
	 * @return {@code null} if the proxy was successfully set, or otherwise a
	 *         {@link ComponentName}
		...
		if (mService != null) {
			try {
				...
				if (proxySpec.equals(Proxy.NO_PROXY)) {
					...
					final Pair<String, String> proxyParams = getProxyParameters(proxySpec,
							exclusionList);
	 ...
	 * want for typical
	 * HTTP proxies - they are generally network dependent. However if you're doing
	 * something
	 * unusual like general internal filtering this may be useful. On a private
	 * network where the
	 ...
	 * ignore it.
	 * <p>
	 * Note: The device owner won't be able to set a global HTTP proxy if there are
	 * unaffiliated
	 * secondary users or profiles on the device. It's recommended that affiliation
	 * ids are set for
	 ...
	 * @param admin     Which {@link DeviceAdminReceiver} this request is associated
	 *                  with.
	 * @param proxyInfo The a {@link ProxyInfo} object defining the new global HTTP
	 *                  proxy. A
	 *                  {@code null} value will clear the global HTTP proxy.
	 * @throws SecurityException if {@code admin} is not the device owner.
	 */
	public void setRecommendedGlobalProxy(@NonNull ComponentName admin, @Nullable ProxyInfo proxyInfo) {
	 ...
	 * 
	 * @return ComponentName object of the device admin that set the global proxy,
	 *         or {@code null}
	...
	/**
	 * Result code for {@link #setStorageEncryption} and
	 * {@link #getStorageEncryptionStatus}:
	 ...
	 * <p>
	 * {@link #getStorageEncryptionStatus} can only return this value on devices
	 * that use Full Disk
	 * Encryption. Support for Full Disk Encryption was entirely removed in API
	 * level 33, having
	 * been replaced by File Based Encryption. Devices that use File Based
	 * Encryption always
	 * automatically activate their encryption on first boot.
	 * <p>
	 * {@link #setStorageEncryption} can still return this value for an unrelated
	 * reason, but {@link
	...
	/**
	 * Result code for {@link #getStorageEncryptionStatus}: indicating that
	 * encryption is not
	 * currently active, but is currently being activated.
	 * <p>
	 * 
	 * @deprecated This result code has never actually been used, so there is no
	 *             reason for apps to
	 *             check for it.
	 */
	@Deprecated
	 ...
	 * <p>
	 * {@link #getStorageEncryptionStatus} can only return this value for apps
	 * targeting API level
	 * 23 or lower, or on devices that use Full Disk Encryption. Support for Full
	 * Disk Encryption
	 * was entirely removed in API level 33, having been replaced by File Based
	 * Encryption. The
	 * result code {@link #ENCRYPTION_STATUS_ACTIVE_PER_USER} is used on devices
	 * that use File Based
	 ...
	 * but the encryption key is not cryptographically protected by the user's
	 * credentials.
	 * <p>
	 * This value can only be returned on devices that use Full Disk Encryption.
	 * Support for Full
	 * Disk Encryption was entirely removed in API level 33, having been replaced by
	 * File Based
	 * Encryption. With File Based Encryption, each user's credential-encrypted
	 * storage is always
	 ...
	 * indicating that encryption is active and the encryption key is tied to the
	 * user or profile.
	 * <p>
	 * This value is only returned to apps targeting API level 24 and above. For
	 * apps targeting
	 * earlier API levels, {@link #ENCRYPTION_STATUS_ACTIVE} is returned, even if
	 * the
	 ...
	 * activity should
	 * be launched after using {@link #setStorageEncryption} to request encryption
	 * be activated.
	 * After resuming from this activity, use {@link #getStorageEncryption}
	 * to check encryption status. However, on some devices this activity may never
	 * return, as
	...
	/**
	 * Activity action: launch the DPC to check policy compliance. This intent is
	 * launched when
	 * the user taps on the notification about personal apps suspension. When
	 * handling this intent
	 * the DPC must check if personal apps should still be suspended and either
	 * unsuspend them or
	...
	/**
	 * Broadcast action: notify managed provisioning that PO/DO provisioning has
	 * completed.
	 *
	 * @hide
	 */
	...
	/**
	 * Extra for {@link #ACTION_PROVISIONING_COMPLETED} to indicate the provisioning
	 * action that has
	 * been completed, this can either be {@link #ACTION_PROVISION_MANAGED_PROFILE},
	 * {@link #ACTION_PROVISION_MANAGED_DEVICE}, or
	 * {@link #ACTION_PROVISION_MANAGED_USER}.
	 *
	 * @hide
	 */
	...
	/**
	 * Broadcast action: notify system that a new (Android) user was added when the
	 * device is
	 * managed by a device owner, so receivers can show the proper disclaimer to the
	 * (human) user.
	 *
	 * @hide
	 */
	 ...
	 * {@link android.os.Build.VERSION_CODES#LOLLIPOP} since keyguard widget is only
	 * supported
	 ...
	 * By setting this flag alone, all trust agents are disabled. If the admin then
	 * wants to
	 * allowlist specific features of some trust agent,
	 * {@link #setTrustAgentConfiguration} can be
	 ...
	 * PIN/Pattern/Password).
	 * 
	 * @deprecated This flag was added in version
	 *             {@link android.os.Build.VERSION_CODES#N}, but it
	...
	/**
	 * NOTE: Please remember to update the DevicePolicyManagerTest's
	 * testKeyguardDisabledFeatures
	 ...
	 * PIN/Pattern/Password).
	 */
	public static final int KEYGUARD_DISABLE_BIOMETRICS = DevicePolicyManager.KEYGUARD_DISABLE_FACE
	...
	/**
	 * Keyguard features that when set on a non-organization-owned managed profile
	 * that doesn't
	 * have its own challenge will affect the profile's parent user. These can also
	 * be set on the
	 * managed profile's parent {@link DevicePolicyManager} instance to explicitly
	 * control the
	 * parent user.
	 *
	 * <p>
	 * Organization-owned managed profile supports disabling additional keyguard
	 * features on the
	 * parent user as defined in
	 * {@link #ORG_OWNED_PROFILE_KEYGUARD_FEATURES_PARENT_ONLY}.
	 *
	 * @hide
	 */
	public static final int NON_ORG_OWNED_PROFILE_KEYGUARD_FEATURES_AFFECT_OWNER = DevicePolicyManager.KEYGUARD_DISABLE_TRUST_AGENTS
			| DevicePolicyManager.KEYGUARD_DISABLE_BIOMETRICS;

	/**
	 * Keyguard features that when set by the profile owner of an organization-owned
	 * managed
	 * profile will affect the profile's parent user if set on the managed profile's
	 * parent
	 * {@link DevicePolicyManager} instance.
	 *
	 * @hide
	 */
	public static final int ORG_OWNED_PROFILE_KEYGUARD_FEATURES_PARENT_ONLY = DevicePolicyManager.KEYGUARD_DISABLE_SECURE_CAMERA
			| DevicePolicyManager.KEYGUARD_DISABLE_SECURE_NOTIFICATIONS
			| DevicePolicyManager.KEYGUARD_DISABLE_SHORTCUTS_ALL;

	/**
	 * Keyguard features that when set on a normal or organization-owned managed
	 * profile, have
	 * the potential to affect the profile's parent user.
	 *
	 * @hide
	 */
	public static final int PROFILE_KEYGUARD_FEATURES_AFFECT_OWNER = DevicePolicyManager.NON_ORG_OWNED_PROFILE_KEYGUARD_FEATURES_AFFECT_OWNER
			| DevicePolicyManager.ORG_OWNED_PROFILE_KEYGUARD_FEATURES_PARENT_ONLY;

	/**
	 * @deprecated This method does not actually modify the storage encryption of
	 *             the device.
	 *             It has never affected the encryption status of a device.
	 *
	 *             Called by an application that is administering the device to
	 *             request that the storage system
	 *             be encrypted. Does nothing if the caller is on a secondary user
	 *             or a managed profile.
	 *             <p>
	 *             When multiple device administrators attempt to control device
	 *             encryption, the most secure,
	 *             supported setting will always be used. If any device
	 *             administrator requests device
	 *             encryption, it will be enabled; Conversely, if a device
	 *             administrator attempts to disable
	 *             device encryption while another device administrator has enabled
	 *             it, the call to disable will
	 *             fail (most commonly returning {@link #ENCRYPTION_STATUS_ACTIVE}).
	 *             <p>
	 *             This policy controls encryption of the secure (application data)
	 *             storage area. Data written
	 *             to other storage areas may or may not be encrypted, and this
	 *             policy does not require or
	 *             control the encryption of any other storage areas. There is one
	 *             exception: If
	 *             {@link android.os.Environment#isExternalStorageEmulated()} is
	 *             {@code true}, then the
	 *             directory returned by
	 *             {@link android.os.Environment#getExternalStorageDirectory()} must
	 *             be
	 *             written to disk within the encrypted storage area.
	 *             <p>
	 *             Important Note: On some devices, it is possible to encrypt
	 *             storage without requiring the user
	 *             to create a device PIN or Password. In this case, the storage is
	 *             encrypted, but the
	 *             encryption key may not be fully secured. For maximum security,
	 *             the administrator should also
	 *             require (and check for) a pattern, PIN, or password.
	 *
	 * @param admin   Which {@link DeviceAdminReceiver} this request is associated
	 *                with.
	 * @param encrypt true to request encryption, false to release any previous
	 *                request
	 * @return the new total request status (for all active admins), or {@link
	 *         DevicePolicyManager#ENCRYPTION_STATUS_UNSUPPORTED} if called for a
	 *         non-system user.
	 *         Will be one of {@link #ENCRYPTION_STATUS_UNSUPPORTED}, {@link
	 *         #ENCRYPTION_STATUS_INACTIVE}, or {@link #ENCRYPTION_STATUS_ACTIVE}.
	 *         This is the value
	 *         of the requests; use {@link #getStorageEncryptionStatus()} to query
	 *         the actual device
	 *         state.
	 *
	 * @throws SecurityException if {@code admin} is not an active administrator or
	...
	/**
	 * @deprecated This method only returns the value set by
	 *             {@link #setStorageEncryption}.
	 ...
	 * @param admin Which {@link DeviceAdminReceiver} this request is associated
	 *              with. If null,
	 *              this will return the requested encryption setting as an
	 *              aggregate of all active
	 *              administrators.
	 ...
	 * #ACTION_START_ENCRYPTION} to begin the process of encrypting or decrypting
	 * the
	 * storage. If the result is {@link #ENCRYPTION_STATUS_ACTIVE_DEFAULT_KEY}, the
	 * storage system has enabled encryption but no password is set so further
	 * action
	 * may be required. If the result is {@link #ENCRYPTION_STATUS_ACTIVATING},
	 * {@link #ENCRYPTION_STATUS_ACTIVE} or
	 * {@link #ENCRYPTION_STATUS_ACTIVE_PER_USER},
	 * no further action is required.
	 *
	 * @return current status of encryption. The value will be one of
	 *         {@link #ENCRYPTION_STATUS_UNSUPPORTED},
	 *         {@link #ENCRYPTION_STATUS_INACTIVE},
	 *         {@link #ENCRYPTION_STATUS_ACTIVATING},
	...
	/**
	 * Mark a CA certificate as approved by the device user. This means that they
	 * have been notified
	 * of the installation, were made aware of the risks, viewed the certificate and
	 * still wanted to
	 * keep the certificate on the device.
	 *
	 * Calling with {@param approval} as {@code true} will cancel any ongoing
	 * warnings related to
	 * this certificate.
	 *
	 * @hide
	 ...
	 * <p>
	 * Inserted user CAs aren't automatically trusted by apps in Android 7.0 (API
	 * level 24) and
	 * higher. App developers can change the default behavior for an app by adding a
	 * <a href="
	 * {@docRoot}training/articles/security-config.html">Security Configuration
	 * File</a> to the app manifest file.
	 *
	 * The caller must be a profile or device owner on that user, or a delegate
	 * package given the
	 * {@link #DELEGATION_CERT_INSTALL} scope via {@link #setDelegatedScopes};
	 * otherwise a
	 * security exception will be thrown.
	 *
	 * @param admin      Which {@link DeviceAdminReceiver} this request is
	 *                   associated with, or
	 *                   {@code null} if calling from a delegated certificate
	 *                   installer.
	 * @param certBuffer encoded form of the certificate to install.
	 *
	 * @return false if the certBuffer cannot be parsed or installation is
	 *         interrupted, true otherwise.
	 * @throws SecurityException if {@code admin} is not {@code null} and not a
	 *                           device or profile
	 *                           owner.
	...
	/**
	 * Returns all CA certificates that are currently trusted, excluding system CA
	 * certificates.
	 * If a user has installed any certificates by other means than device policy
	 * these will be
	 * included too.
	 *
	 * @param admin Which {@link DeviceAdminReceiver} this request is associated
	...
	/**
	 * Uninstalls all custom trusted CA certificates from the profile. Certificates
	 * installed by
	 * means other than device policy will also be removed, except for system CA
	 * certificates.
	 *
	 * @param admin Which {@link DeviceAdminReceiver} this request is associated
	 *              with, or
	...
	/**
	 * This API can be called by the following to install a certificate and
	 * corresponding
	 * private key:
	 * <ul>
	 * <li>Device owner</li>
	 ...
	 * permission</li>
	 * </ul>
	 * All apps within the profile will be able to access the certificate and use
	 * the private key,
	 * given direct user approval.
	 *
	 * <p>
	 * From Android {@link android.os.Build.VERSION_CODES#S}, the credential
	 * management app
	 * can call this API. However, this API sets the key pair as user selectable by
	 * default,
	 ...
	 *
	 * <p>
	 * Access to the installed credentials will not be granted to the caller of this
	 * API without
	 * direct user approval. This is for security - should a certificate installer
	 * become
	 ...
	 * {@link #installKeyPair(ComponentName, PrivateKey, Certificate[], String, boolean)}
	 * instead.
	 *
	 * <p>
	 * Note: If the provided {@code alias} is of an existing alias, all former
	 * grants that apps
	 * have been given to access the key and certificates associated with this alias
	 * will be
	 * revoked.
	 *
	 * @param admin   Which {@link DeviceAdminReceiver} this request is associated
	 ...
	 * @throws SecurityException if {@code admin} is not {@code null} and not a
	 *                           device or profile
	 *                           owner, or {@code admin} is null and the calling
	...
	/**
	 * This API can be called by the following to install a certificate chain and
	 * corresponding
	 ...
	 * permission</li>
	 * </ul>
	 * All apps within the profile will be able to access the certificate chain and
	 * use the private
	 ...
	 *
	 * <p>
	 * The caller of this API may grant itself access to the certificate and private
	 * key
	 * immediately, without user approval. It is a best practice not to request this
	 * unless strictly
	 ...
	 *                      {@code null} if the caller is not a device admin.
	 * @param privKey       The private key to install.
	 * @param certs         The certificate chain to install. The chain should start
	 *                      with the leaf
	 *                      certificate and include the chain of trust in order.
	 *                      This will be returned by
	 *                      {@link android.security.KeyChain#getCertificateChain}.
	 * @param alias         The private key alias under which to install the
	 *                      certificate. If a certificate
	 *                      with that alias already exists, it will be overwritten.
	 * @param requestAccess {@code true} to request that the calling app be granted
	 *                      access to the
	 *                      credentials immediately. Otherwise, access to the
	 *                      credentials will be gated by user
	 *                      approval.
	 * @return {@code true} if the keys were installed, {@code false} otherwise.
	 * @throws SecurityException if {@code admin} is not {@code null} and not a
	 *                           device or profile
	 ...
	 * All apps within the profile will be able to access the certificate chain and
	 * use the
	 * private key, given direct user approval (if the user is allowed to select the
	 * private key).
	 *
	 * <p>
	 * From Android {@link android.os.Build.VERSION_CODES#S}, the credential
	 ...
	 *
	 * <p>
	 * Include {@link #INSTALLKEY_SET_USER_SELECTABLE} in the {@code flags} argument
	 * to allow
	 ...
	 *                the leaf
	 *                certificate and include the chain of trust in order. This will
	 *                be returned by
	 *                {@link android.security.KeyChain#getCertificateChain}.
	 * @param alias   The private key alias under which to install the certificate.
	 *                If a certificate
	 *                with that alias already exists, it will be overwritten.
	 * @param flags   Flags to request that the calling app be granted access to the
	 *                credentials
	 *                and set the key to be user-selectable. See
	 *                {@link #INSTALLKEY_SET_USER_SELECTABLE} and
	 *                {@link #INSTALLKEY_REQUEST_CREDENTIALS_ACCESS}.
	 * @return {@code true} if the keys were installed, {@code false} otherwise.
	 * @throws SecurityException if {@code admin} is not {@code null} and not a
	 *                           device or
	 *                           profile owner, or {@code admin} is null but the
	 *                           calling application is not a
	 *                           delegated certificate installer, credential
	 *                           management app and does not have the
	 *                           {@link android.Manifest.permission#MANAGE_DEVICE_POLICY_CERTIFICATES}
	 *                           permission.
	 * @see android.security.KeyChain#getCertificateChain
	 * @see #setDelegatedScopes
	...
	/**
	 * This API can be called by the following to remove a certificate and private
	 * key pair
	 ...
	 * From Android {@link android.os.Build.VERSION_CODES#S}, the credential
	 * management app
	 * can call this API. If called by the credential management app, the
	 * componentName must be
	 * {@code null}. Note, there can only be a credential management app on an
	 * unmanaged device.
	 *
	 * @param admin Which {@link DeviceAdminReceiver} this request is associated
	 *              with, or
	 ...
	 * @return {@code true} if the private key alias no longer exists, {@code false}
	 *         otherwise.
	 * @throws SecurityException if {@code admin} is not {@code null} and not a
	 *                           device or profile
	 *                           owner, or {@code admin} is null but the calling
	 *                           application is not a delegated
	...
	// STOPSHIP(b/174298501): clarify the expected return value following
	// generateKeyPair call.
	/**
	 * This API can be called by the following to query whether a certificate and
	 * private key are
	 * installed under a given alias:
	 * <ul>
	 * <li>Device owner</li>
	 ...
	 * </ul>
	 *
	 * If called by the credential management app, the alias must exist in the
	 * credential
	 ...
	 * @return {@code true} if a key pair with this alias exists, {@code false}
	 *         otherwise.
	 * @throws SecurityException if the caller is not a device or profile owner, a
	 *                           delegated
	 *                           certificate installer, the credential management
	 *                           app and does not have the
	 *                           {@link android.Manifest.permission#MANAGE_DEVICE_POLICY_CERTIFICATES}
	 *                           permission.
	 * @see #setDelegatedScopes
	 * @see #DELEGATION_CERT_INSTALL
	...
	/**
	 * This API can be called by the following to generate a new private/public key
	 * pair:
	 * <ul>
	 * <li>Device owner</li>
	 * <li>Profile owner</li>
	 ...
	 * permission</li>
	 * </ul>
	 * If the device supports key generation via secure hardware, this method is
	 * useful for
	 * creating a key in KeyChain that never left the secure hardware. Access to the
	 * key is
	 ...
	 *
	 * <p>
	 * Because this method might take several seconds to complete, it should only be
	 * called from
	 * a worker thread. This method returns {@code null} when called from the main
	 * thread.
	 *
	 * <p>
	 * This method is not thread-safe, calling it from multiple threads at the same
	 * time will
	 * result in undefined behavior. If the calling thread is interrupted while the
	 * invocation is
	 ...
	 *
	 * <p>
	 * Attestation: to enable attestation, set an attestation challenge in
	 * {@code keySpec} via
	 * {@link KeyGenParameterSpec.Builder#setAttestationChallenge}. By specifying
	 * flags to the
	 * {@code idAttestationFlags} parameter, it is possible to request the device's
	 * unique
	 * identity to be included in the attestation record.
	 *
	 * <p>
	 * Specific identifiers can be included in the attestation record, and an
	 * individual
	 * attestation certificate can be used to sign the attestation record. To find
	 * out if the device
	 * supports these features, refer to {@link #isDeviceIdAttestationSupported()}
	 * and
	 * {@link #isUniqueDeviceAttestationSupported()}.
	 *
	 * <p>
	 * Device owner, profile owner, their delegated certificate installer and the
	 * credential
	 * management app can use {@link #ID_TYPE_BASE_INFO} to request inclusion of the
	 * general device
	 * information including manufacturer, model, brand, device and product in the
	 * attestation
	 * record.
	 * Only device owner, profile owner on an organization-owned device or
	 * affiliated user, and
	 * their delegated certificate installers can use {@link #ID_TYPE_SERIAL},
	 * {@link #ID_TYPE_IMEI}
	 * and {@link #ID_TYPE_MEID} to request unique device identifiers to be attested
	 * (the serial
	 * number, IMEI and MEID correspondingly), if supported by the device
	 * (see {@link #isDeviceIdAttestationSupported()}).
	 * Additionally, device owner, profile owner on an organization-owned device and
	 * their delegated
	 * certificate installers can also request the attestation record to be signed
	 * using an
	 * individual attestation certificate by specifying the
	 * {@link #ID_TYPE_INDIVIDUAL_ATTESTATION}
	 * flag (if supported by the device, see
	 * {@link #isUniqueDeviceAttestationSupported()}).
	 * <p>
	 * If any of {@link #ID_TYPE_SERIAL}, {@link #ID_TYPE_IMEI} and
	 * {@link #ID_TYPE_MEID}
	 * is set, it is implicitly assumed that {@link #ID_TYPE_BASE_INFO} is also set.
	 * <p>
	 * Attestation using {@link #ID_TYPE_INDIVIDUAL_ATTESTATION} can only be
	 * requested if
	 ...
	 *                           associated with, or
	 *                           {@code null} if the caller is not a device admin.
	 * @param algorithm          The key generation algorithm, see
	 *                           {@link java.security.KeyPairGenerator}.
	 * @param keySpec            Specification of the key to generate, see
	 *                           {@link java.security.KeyPairGenerator}.
	 * @param idAttestationFlags A bitmask of the identifiers that should be
	 *                           included in the
	 *                           attestation record ({@code ID_TYPE_BASE_INFO},
	 *                           {@code ID_TYPE_SERIAL},
	 *                           {@code ID_TYPE_IMEI} and {@code ID_TYPE_MEID}), and
	 *                           {@code ID_TYPE_INDIVIDUAL_ATTESTATION} if the
	 *                           attestation record should be signed
	 *                           using an individual attestation certificate.
	 *                           <p>
	 *                           {@code 0} should be passed in if no device
	 *                           identification is required in the
	 *                           attestation record and the batch attestation
	 *                           certificate should be used.
	 *                           <p>
	 *                           If any flag is specified, then an attestation
	 *                           challenge must be included in the
	 *                           {@code keySpec}.
	 * @return A non-null {@code AttestedKeyPair} if the key generation succeeded,
	 *         null otherwise.
	 * @throws SecurityException             if {@code admin} is not {@code null}
	 *                                       and not a device or profile
	 *                                       owner, or {@code admin} is null but the
	 *                                       calling application is not a delegated
	 *                                       certificate installer or credential
	 *                                       management app. If Device ID
	 *                                       attestation is
	 *                                       requested (using
	 *                                       {@link #ID_TYPE_SERIAL},
	 *                                       {@link #ID_TYPE_IMEI} or
	 *                                       {@link #ID_TYPE_MEID}), the caller must
	 *                                       be the Device Owner or the Certificate
	 *                                       Installer delegate.
	 * @throws IllegalArgumentException      in the following cases:
	 *                                       <p>
	 *                                       <ul>
	 *                                       <li>The alias in {@code keySpec} is
	 *                                       empty.</li>
	 *                                       <li>The algorithm specification in
	 *                                       {@code keySpec} is not
	 *                                       {@code RSAKeyGenParameterSpec} or
	 *                                       {@code ECGenParameterSpec}.</li>
	 *                                       <li>Device ID attestation was requested
	 *                                       but the {@code keySpec} does not
	 *                                       contain an
	 *                                       attestation challenge.</li>
	 *                                       </ul>
	 * @throws UnsupportedOperationException if Device ID attestation or individual
	 *                                       attestation
	 *                                       was requested but the underlying
	 *                                       hardware does not support it.
	 * @throws StrongBoxUnavailableException if the use of StrongBox for key
	 *                                       generation was
	 *                                       specified in {@code keySpec} but the
			...
			switch (e.errorCode) {
				...
				default:
					throw new RuntimeException(
							String.format("Unknown error while generating key: %d",
									e.errorCode));
			}
		}
		return null;
	...
	/**
	 * Called by a device or profile owner, or delegated certificate chooser (an app
	 * that has been
	 * delegated the {@link #DELEGATION_CERT_SELECTION} privilege), to grant an
	 * application access
	 * to an already-installed (or generated) KeyChain key.
	 * This is useful (in combination with {@link #installKeyPair} or
	 * {@link #generateKeyPair}) to
	 * let an application call {@link android.security.KeyChain#getPrivateKey}
	 * without having to
	 * call {@link android.security.KeyChain#choosePrivateKeyAlias} first.
	 *
	 * The grantee app will receive the
	 * {@link android.security.KeyChain#ACTION_KEY_ACCESS_CHANGED}
	 * broadcast when access to a key is granted.
	 *
	 * Starting from {@link android.os.Build.VERSION_CODES#UPSIDE_DOWN_CAKE} throws
	 * an
	 * {@link IllegalArgumentException} if {@code alias} doesn't correspond to an
	 * existing key.
	 *
	 * @param admin       Which {@link DeviceAdminReceiver} this request is
	 *                    associated with, or
	 ...
	 * @param packageName The name of the (already installed) package to grant
	 *                    access to.
	 * @return {@code true} if the grant was set successfully, {@code false}
	 *         otherwise.
	 *
	 * @throws SecurityException        if the caller is not a device owner, a
	 *                                  profile owner or
	 *                                  delegated certificate chooser.
	 * @throws IllegalArgumentException if {@code packageName} or {@code alias} are
	 *                                  empty, or if
	 *                                  {@code packageName} is not a name of an
	 *                                  installed package.
	 ...
	 * Called by a device or profile owner, or delegated certificate chooser (an app
	 * that has been
	 * delegated the {@link #DELEGATION_CERT_SELECTION} privilege), to query which
	 * apps have access
	 * to a given KeyChain key.
	 *
	 * Key are granted on a per-UID basis, so if several apps share the same UID,
	 * granting access to
	 * one of them automatically grants it to others. This method returns a map
	 * containing one entry
	 * per grantee UID. Entries have UIDs as keys and sets of corresponding package
	 * names as values.
	 * In particular, grantee packages that don't share UID with other packages are
	 * represented by
	 * entries having singleton sets as values.
	 *
	 * @param alias The alias of the key to grant access to.
	 * @return apps that have access to a given key, arranged in a map from UID to
	 *         sets of
	 *         package names.
	 *
	 * @throws SecurityException        if the caller is not a device owner, a
	 *                                  profile owner or
	 *                                  delegated certificate chooser.
	 * @throws IllegalArgumentException if {@code alias} doesn't correspond to an
	 *                                  existing key.
	 *
	 ...
	 * Called by a device or profile owner, or delegated certificate chooser (an app
	 * that has been
	 * delegated the {@link #DELEGATION_CERT_SELECTION} privilege), to revoke an
	 * application's
	 ...
	 * @param packageName The name of the (already installed) package to revoke
	 *                    access from.
	 * @return {@code true} if the grant was revoked successfully, {@code false}
	 *         otherwise.
	 *
	 * @throws SecurityException        if the caller is not a device owner, a
	 *                                  profile owner or
	 ...
	 * Called by a device or profile owner, or delegated certificate chooser (an app
	 * that has been
	 * delegated the {@link #DELEGATION_CERT_SELECTION} privilege), to allow using a
	 * KeyChain key
	 * pair for authentication to Wifi networks. The key can then be used in
	 ...
	 * @return {@code true} if the operation was set successfully, {@code false}
	 *         otherwise.
	 *
	 ...
	 * Called by a device or profile owner, or delegated certificate chooser (an app
	 * that has been
	 * delegated the {@link #DELEGATION_CERT_SELECTION} privilege), to deny using a
	 * KeyChain key
	 * pair for authentication to Wifi networks. Configured networks using this key
	 * won't be able to
	 * authenticate.
	 *
	 * Starting from {@link android.os.Build.VERSION_CODES#UPSIDE_DOWN_CAKE} throws
	 ...
	 * Called by a device or profile owner, or delegated certificate chooser (an app
	 * that has been
	 * delegated the {@link #DELEGATION_CERT_SELECTION} privilege), to query whether
	 * a KeyChain key
	...
	/**
	 * Returns {@code true} if the device supports attestation of device identifiers
	 * in addition
	...
	/**
	 * Returns {@code true} if the StrongBox Keymaster implementation on the device
	 * was provisioned
	 * with an individual attestation certificate and can sign attestation records
	 * using it (as
	 * attestation using an individual attestation certificate is a feature only
	 * Keymaster
	...
	/**
	 * This API can be called by the following to associate certificates with a key
	 * pair that was
	 * generated using {@link #generateKeyPair}, and set whether the key is
	 ...
	 *                         {@code null} if the caller is not a device admin.
	 * @param alias            The private key alias under which to install the
	 *                         certificate. The {@code alias}
	 *                         should denote an existing private key. If a
	 *                         certificate with that alias already
	 *                         exists, it will be overwritten.
	 * @param certs            The certificate chain to install. The chain should
	 *                         start with the leaf
	 *                         certificate and include the chain of trust in order.
	 *                         This will be returned by
	 *                         {@link android.security.KeyChain#getCertificateChain}.
	 * @param isUserSelectable {@code true} to indicate that a user can select this
	 *                         key via the
	 *                         certificate selection prompt, {@code false} to
	 *                         indicate that this key can only be
	 *                         granted access by implementing
	 *                         {@link android.app.admin.DeviceAdminReceiver#onChoosePrivateKeyAlias}.
	 * @return {@code true} if the provided {@code alias} exists and the
	...
	/**
	 * @return the alias of a given CA certificate in the certificate store, or
	 *         {@code null} if it
	 *         doesn't exist.
	 */
	...
	/**
	 * Called by a profile owner or device owner to grant access to privileged
	 * certificate
	 * manipulation APIs to a third-party certificate installer app. Granted APIs
	 * include
	 * {@link #getInstalledCaCerts}, {@link #hasCaCertInstalled},
	 * {@link #installCaCert},
	 * {@link #uninstallCaCert}, {@link #uninstallAllUserCaCerts} and
	 * {@link #installKeyPair}.
	 * <p>
	 * Delegated certificate installer is a per-user state. The delegated access is
	 * persistent until
	 * it is later cleared by calling this method with a null value or uninstallling
	 * the certificate
	 * installer.
	 * <p>
	 * <b>Note:</b>Starting from {@link android.os.Build.VERSION_CODES#N}, if the
	 * caller
	 * application's target SDK version is {@link android.os.Build.VERSION_CODES#N}
	 * or newer, the
	 * supplied certificate installer package must be installed when calling this
	 * API, otherwise an
	 * {@link IllegalArgumentException} will be thrown.
	 *
	 * @param admin            Which {@link DeviceAdminReceiver} this request is
	 *                         associated with.
	 * @param installerPackage The package name of the certificate installer which
	 *                         will be given
	 *                         access. If {@code null} is given the current package
	 *                         will be cleared.
	 ...
	 */
	@Deprecated
	public void setCertInstallerPackage(@NonNull ComponentName admin, @Nullable String installerPackage)
	...
	/**
	 * Called by a profile owner or device owner to retrieve the certificate
	 * installer for the user,
	 * or {@code null} if none is set. If there are multiple delegates this function
	 * will return one
	 * of them.
	 *
	 * @param admin Which {@link DeviceAdminReceiver} this request is associated
	 *              with.
	 * @return The package name of the current delegated certificate installer, or
	 *         {@code null} if
	 *         none is set.
	 * @throws SecurityException if {@code admin} is not a device or a profile
	 *                           owner.
	...
	/**
	 * Called by a profile owner or device owner to grant access to privileged APIs
	 * to another app.
	 * Granted APIs are determined by {@code scopes}, which is a list of the
	 * {@code DELEGATION_*}
	 * constants.
	 * <p>
	 * A broadcast with the {@link #ACTION_APPLICATION_DELEGATION_SCOPES_CHANGED}
	 * action will be
	 * sent to the {@code delegatePackage} with its new scopes in an
	 * {@code ArrayList<String>} extra
	 * under the {@link #EXTRA_DELEGATION_SCOPES} key. The broadcast is sent with
	 * the
	 * {@link Intent#FLAG_RECEIVER_REGISTERED_ONLY} flag.
	 * <p>
	 * Delegated scopes are a per-user state. The delegated access is persistent
	 * until it is later
	 * cleared by calling this method with an empty {@code scopes} list or
	 * uninstalling the
	 ...
	 *                        access.
	 * @param scopes          The groups of privileged APIs whose access should be
	 *                        granted to
	...
	/**
	 * Called by a profile owner or device owner to retrieve a list of the scopes
	 * given to a
	 * delegate package. Other apps can use this method to retrieve their own
	 * delegated scopes by
	 ...
	 * @param admin            Which {@link DeviceAdminReceiver} this request is
	 *                         associated with, or
	 *                         {@code null} if the caller is
	 *                         {@code delegatedPackage}.
	 * @param delegatedPackage The package name of the app whose scopes should be
	 *                         retrieved.
	...
	/**
	 * Called by a profile owner or device owner to retrieve a list of delegate
	 * packages that were
	...
	/**
	 * Service-specific error code used in implementation of
	 * {@code setAlwaysOnVpnPackage} methods.
	 * 
	 * @hide
	 */
	 ...
	 * through a
	 * specific application for the current user. This connection is automatically
	 * granted and
	 ...
	 * <p>
	 * Enabling lockdown via {@code lockdownEnabled} argument carries the risk that
	 * any failure
	 * of the VPN provider could break networking for all apps. This method clears
	 * any lockdown
	 * allowlist set by
	 * {@link #setAlwaysOnVpnPackage(ComponentName, String, boolean, Set)}.
	 * <p>
	 * Starting from {@link android.os.Build.VERSION_CODES#S API 31} calling this
	 * method with
	 * {@code vpnPackage} set to {@code null} only removes the existing
	 * configuration if it was
	 * previously created by this admin. To remove VPN configuration created by the
	 * user use
	 * {@link UserManager#DISALLOW_CONFIG_VPN}.
	 *
	 * @param vpnPackage      The package name for an installed VPN app on the
	 *                        device, or {@code null} to
	 *                        remove an existing always-on VPN configuration.
	 * @param lockdownEnabled {@code true} to disallow networking when the VPN is
	 *                        not connected or
	 *                        {@code false} otherwise. This has no effect when
	 *                        clearing.
	 * @throws SecurityException             if {@code admin} is not a device or a
	 *                                       profile owner.
	 * @throws NameNotFoundException         if {@code vpnPackage} is not installed.
	 * @throws UnsupportedOperationException if {@code vpnPackage} exists but does
	 *                                       not support being
	 *                                       set as always-on, or if always-on VPN
	 *                                       is not available.
	 ...
	 * that allows the
	 * admin to specify a set of apps that should be able to access the network
	 * directly when VPN
	 * is not connected. When VPN connects these apps switch over to VPN if allowed
	 * to use that VPN.
	 * System apps can always bypass VPN.
	 * <p>
	 * Note that the system doesn't update the allowlist when packages are installed
	 * or
	 * uninstalled, the admin app must call this method to keep the list up to date.
	 * <p>
	 * When {@code lockdownEnabled} is false {@code lockdownAllowlist} is ignored .
	 * When
	 * {@code lockdownEnabled} is {@code true} and {@code lockdownAllowlist} is
	 * {@code null} or
	 * empty, only system apps can bypass VPN.
	 * <p>
	 * Setting always-on VPN package to {@code null} or using
	 * {@link #setAlwaysOnVpnPackage(ComponentName, String, boolean)} clears
	 * lockdown allowlist.
	 *
	 * @param vpnPackage        package name for an installed VPN app on the device,
	 *                          or {@code null}
	 ...
	 *                          {@code false} otherwise. This has no effect when
	 *                          clearing.
	 * @param lockdownAllowlist Packages that will be able to access the network
	 *                          directly when VPN
	 *                          is in lockdown mode but not connected. Has no effect
	 *                          when clearing.
	 * @throws SecurityException             if {@code admin} is not a device or a
	 *                                       profile
	 *                                       owner.
	 * @throws NameNotFoundException         if {@code vpnPackage} or one of
	 *                                       {@code lockdownAllowlist} is not
	 *                                       installed.
	 * @throws UnsupportedOperationException if {@code vpnPackage} exists but does
	 *                                       not support being set as always-on, or
	 *                                       if always-on VPN is not
	 *                                       available.
	 */
	public void setAlwaysOnVpnPackage(@NonNull ComponentName admin, @Nullable String vpnPackage,
	...
	/**
	 * Called by device or profile owner to query whether current always-on VPN is
	 * configured in
	...
	public boolean isAlwaysOnVpnLockdownEnabled(@NonNull ComponentName admin) {
		...
		if (mService != null) {
			try {
				...
				// DevicePolicyManagerService if it holds
				// android.permission.MAINLINE_NETWORK_STACK.
				// Note that the android.permission.MAINLINE_NETWORK_STACK is a signature
				// permission
	...
	/**
	 * Returns whether the admin has enabled always-on VPN lockdown for the current
	 * user.
	 *
	...
	/**
	 * Called by device or profile owner to query the set of packages that are
	 * allowed to access
	 * the network directly when always-on VPN is in lockdown mode but not
	 * connected. Returns
	...
	/**
	 * Called by a device or profile owner to read the name of the package
	 * administering an
	 * always-on VPN connection for the current user. If there is no such package,
	 * or the always-on
	 * VPN is provided by the system instead of by an application, {@code null} will
	 * be returned.
	 *
	 * @return Package name of VPN controller responsible for always-on VPN, or
	 *         {@code null} if none
	 *         is set.
	 * @throws SecurityException if {@code admin} is not a device or a profile
	 *                           owner.
	...
	/**
	 * Returns the VPN package name if the admin has enabled always-on VPN on the
	 * current user,
	 ...
	 * cameras on the
	 * device, for this user. After setting this, no applications running as this
	 * user will be able
	 * to access any cameras on the device.
	 * <p>
	 * This method can be called on the {@link DevicePolicyManager} instance,
	 * returned by {@link #getParentProfileInstance(ComponentName)}, where the
	 * caller must be
	 * the profile owner of an organization-owned managed profile.
	 * <p>
	 * If the caller is device owner, then the restriction will be applied to all
	 * users. If
	 * called on the parent instance, then the restriction will be applied on the
	 * personal profile.
	 * <p>
	 * The calling device admin must have requested
	 * {@link DeviceAdminInfo#USES_POLICY_DISABLE_CAMERA} to be able to call this
	 * method; if it has
	 * not, a security exception will be thrown.
	 * <p>
	 * <b>Note</b>, this policy type is deprecated for legacy device admins since
	 * {@link android.os.Build.VERSION_CODES#Q}. On Android
	 * {@link android.os.Build.VERSION_CODES#Q} devices, legacy device admins
	 * targeting SDK
	 * version {@link android.os.Build.VERSION_CODES#P} or below can still call this
	 * API to
	 * disable camera, while legacy device admins targeting SDK version
	 * {@link android.os.Build.VERSION_CODES#Q} will receive a SecurityException.
	 * Starting
	 * from Android {@link android.os.Build.VERSION_CODES#R}, requests to disable
	 * camera from
	 * legacy device admins targeting SDK version
	 * {@link android.os.Build.VERSION_CODES#P} or
	 * below will be silently ignored.
	 * <p>
	 * Starting from {@link Build.VERSION_CODES#UPSIDE_DOWN_CAKE}, after the camera
	 * disabled
	 * policy has been set,
	 * {@link PolicyUpdateReceiver#onPolicySetResult(Context, String,
	 * Bundle, TargetUser, PolicyUpdateResult)} will notify the admin on whether the
	 * policy was
	 * successfully set or not. This callback will contain:
	 * <ul>
	 * <li>The policy identifier returned from
	 * {@link DevicePolicyIdentifiers#getIdentifierForUserRestriction(String)} with
	 * user restriction
	 ...
	 * {@link PolicyUpdateResult#RESULT_POLICY_SET} if the policy was successfully
	 * set or the
	 ...
	 * PolicyUpdateResult)} will notify the admin of this change. This callback will
	 * contain the
	 * same parameters as PolicyUpdateReceiver#onPolicySetResult and the
	...
	/**
	 * Determine whether or not the device's cameras have been disabled for this
	 * user,
	 ...
	 * <p>
	 * If the device contains secondary users or profiles, they must be affiliated
	 * with the device.
	 * Otherwise a {@link SecurityException} will be thrown. See
	 * {@link #isAffiliatedUser}.
	 *
	 * @param admin Which {@link DeviceAdminReceiver} this request is associated
	 *              with.
	 * @return {@code true} if the bugreport collection started successfully, or
	 *         {@code false} if it
	 *         wasn't triggered because a previous bugreport operation is still
	 *         active (either the
	 *         bugreport is still running or waiting for the user to share or
	 *         decline)
	 * @throws SecurityException if {@code admin} is not a device owner, or there is
	 *                           at least one
	 ...
	 * disabled. Disabling
	 * screen capture also prevents the content from being shown on display devices
	 * that do not have
	 * a secure video output. See {@link android.view.Display#FLAG_SECURE} for more
	 * details about
	 * secure surfaces and secure displays.
	 * <p>
	 * This method can be called on the {@link DevicePolicyManager} instance,
	 * returned by
	 * {@link #getParentProfileInstance(ComponentName)}, where the calling device
	 * admin must be
	 * the profile owner of an organization-owned managed profile. If it is not, a
	 * security
	 * exception will be thrown.
	 * <p>
	 * If the caller is device owner or called on the parent instance by a profile
	 * owner of an
	 * organization-owned managed profile, then the restriction will be applied to
	 * all users.
	 * <p>
	 * From version {@link android.os.Build.VERSION_CODES#M} disabling screen
	 * capture also blocks
	 ...
	 * @throws SecurityException if the caller is not permitted to control screen
	 *                           capture policy.
	 */
	 ...
	 * returned by {@link #getParentProfileInstance(ComponentName)}, where the
	 * caller must be
	 * the profile owner of an organization-owned managed profile (the calling admin
	 * must be
	 * specified).
	 *
	 * @param admin The name of the admin component to check, or {@code null} to
	 ...
	 * Notification
	 * streaming is sending notification data from pre-installed apps to nearby
	 * devices.
	 *
	...
	/**
	 * Returns the current runtime nearby notification streaming policy set by the
	 * device or profile
	 * owner.
	 * <p>
	 * The caller must be the target user's device owner/profile owner or hold the
	 * {@link android.Manifest.permission#READ_NEARBY_STREAMING_POLICY
	 * READ_NEARBY_STREAMING_POLICY}
	 * permission.
	 */
	@RequiresPermission(value = android.Manifest.permission.READ_NEARBY_STREAMING_POLICY, conditional = true)
	...
	/**
	 * Called by a device/profile owner to set nearby app streaming policy. App
	 * streaming is when
	 * the device starts an app on a virtual display and sends a video stream of the
	 * app to nearby
	 * devices.
	 *
	 * @param policy One of the {@code NearbyStreamingPolicy} constants.
	...
	/**
	 * Returns the current runtime nearby app streaming policy set by the device or
	 * profile owner.
	 * <p>
	 * The caller must be the target user's device owner/profile owner or hold the
	 * {@link android.Manifest.permission#READ_NEARBY_STREAMING_POLICY
	...
	/**
	 * Called by a device owner, or alternatively a profile owner from Android 8.0
	 * (API level 26) or
	 * higher, to set whether auto time is required. If auto time is required, no
	 * user will be able
	 ...
	 * Staring from
	 * Android 11, if auto time is required, the user cannot manually set the time
	 * zone.
	 * <p>
	 * The calling device admin must be a device owner, or alternatively a profile
	 * owner from
	 * Android 8.0 (API level 26) or higher. If it is not, a security exception will
	 * be thrown.
	 * <p>
	 * Staring from Android 11, this API switches to use
	 * {@link UserManager#DISALLOW_CONFIG_DATE_TIME} to enforce the auto time
	 * settings. Calling
	 * this API to enforce auto time will result in
	 * {@link UserManager#DISALLOW_CONFIG_DATE_TIME} being set, while calling this
	 * API to lift
	 * the requirement will result in {@link UserManager#DISALLOW_CONFIG_DATE_TIME}
	 * being cleared.
	 ...
	 * @throws SecurityException if {@code admin} is not a device owner, not a
	 *                           profile owner or
	 *                           if this API is called on a managed profile.
	 * @deprecated From {@link android.os.Build.VERSION_CODES#R}. Use
	 *             {@link #setAutoTimeEnabled}
	 *             to turn auto time on or off and use
	 *             {@link UserManager#DISALLOW_CONFIG_DATE_TIME}
	 ...
	 *                with. Null if the
	 *                caller is not a device admin.
	 * @param enabled Whether time should be obtained automatically from the network
	 *                or not.
	 * @throws SecurityException if caller is not a device owner, a profile owner
	 *                           for the
	 *                           primary user, or a profile owner of an
	 *                           organization-owned managed profile.
	 */
	...
	/**
	 * Called by a device owner, a profile owner for the primary user or a profile
	 * owner of an organization-owned managed profile to turn auto time zone on and
	 * off.
	 * Callers are recommended to use {@link UserManager#DISALLOW_CONFIG_DATE_TIME}
	 * to prevent the user from changing this setting.
	 * <p>
	 ...
	 *                with or Null if the
	 *                caller is not a device admin.
	 * @param enabled Whether time zone should be obtained automatically from the
	 *                network or not.
	 * @throws SecurityException if caller is not a device owner, a profile owner
	 *                           for the
	 *                           primary user, or a profile owner of an
	 ...
	 * Called by a device owner to set whether all users created on the device
	 * should be ephemeral.
	 * 
	 * @hide
	 */
	 ...
	 * customizations,
	 * such as widgets. After setting this, keyguard features will be disabled
	 * according to the
	 * provided feature list.
	 * <p>
	 * A calling device admin must have requested
	 * {@link DeviceAdminInfo#USES_POLICY_DISABLE_KEYGUARD_FEATURES} to be able to
	 * call this method;
	 * if it has not, a security exception will be thrown.
	 * <p>
	 * Calling this from a managed profile before version
	 * {@link android.os.Build.VERSION_CODES#M}
	 * will throw a security exception. From version
	 * {@link android.os.Build.VERSION_CODES#M} the
	 * profile owner of a managed profile can set:
	 * <ul>
	 * <li>{@link #KEYGUARD_DISABLE_TRUST_AGENTS}, which affects the parent user,
	 * but only if there
	 ...
	 * <li>{@link #KEYGUARD_DISABLE_UNREDACTED_NOTIFICATIONS} which affects
	 ...
	 * <ul>
	 * <li>{@link #KEYGUARD_DISABLE_SECURE_CAMERA} which affects the parent user
	 * when called on the
	 * parent profile.
	 * <li>{@link #KEYGUARD_DISABLE_SECURE_NOTIFICATIONS} which affects the parent
	 * user when called
	 ...
	 * {@link #KEYGUARD_DISABLE_SECURE_CAMERA} and
	 * {@link #KEYGUARD_DISABLE_SECURE_NOTIFICATIONS}
	 * can also be set on the {@link DevicePolicyManager} instance returned by
	 * {@link #getParentProfileInstance(ComponentName)} in order to set restrictions
	 * on the parent
	 * profile. {@link #KEYGUARD_DISABLE_SECURE_CAMERA} can only be set on the
	 * parent profile
	 * instance if the calling device admin is the profile owner of an
	 * organization-owned
	 * managed profile.
	 * <p>
	 ...
	 *              {@link #KEYGUARD_DISABLE_WIDGETS_ALL},
	 *              {@link #KEYGUARD_DISABLE_SECURE_CAMERA},
	 *              {@link #KEYGUARD_DISABLE_SECURE_NOTIFICATIONS},
	 *              {@link #KEYGUARD_DISABLE_TRUST_AGENTS},
	...
	/**
	 * Determine whether or not features have been disabled in keyguard either by
	 * the calling
	 * admin, if specified, or all admins that set restrictions on this user and its
	 * participating
	 * profiles. Restrictions on profiles that have a separate challenge are not
	 * taken into account.
	 *
	 * <p>
	 * This method can be called on the {@link DevicePolicyManager} instance
	 ...
	 * @return bitfield of flags. See
	 *         {@link #setKeyguardDisabledFeatures(ComponentName, int)}
	 *         for a list.
	 */
	...
	private IpcDataCache<Pair<ComponentName, Integer>, Integer> mGetKeyGuardDisabledFeaturesCache = new IpcDataCache<>(
	...
	/**
	 * Called by a privileged caller holding {@code BIND_DEVICE_ADMIN} permission to
	 * retrieve
	 ...
	 * <li>Only apps with the {@code MANAGE_PROFILE_AND_DEVICE_OWNERS} permission or
	 * the
	 * {@link Process#SHELL_UID Shell UID} can call this method.
	 * </ul>
	 *
	 * <p>
	 * Calling this after the setup phase of the device owner user has completed is
	 * allowed only
	 * if the caller is the {@link Process#SHELL_UID Shell UID}, and there are no
	 * additional users
	 * (except when the device runs on headless system user mode, in which case it
	 * could have exact
	 * one extra user, which is the current user - the device owner will be set in
	 * the
	 * {@link UserHandle#SYSTEM system} user and a profile owner will be set in the
	 * current user)
	 * and no accounts.
	 *
	 ...
	 *
	 * @throws IllegalArgumentException if the package name is {@code null} or
	 *                                  invalid.
	...
	/**
	 * Same as {@link #setDeviceOwner(ComponentName, int)}, but without setting the
	 * profile
	 * owner on current user when running on headless system user mode - should be
	 * used only by
	 * testing infra.
	 *
	 * @hide
	...
	/**
	 * Used to determine if a particular package has been registered as a Device
	 * Owner app.
	 * A device owner app is a special device admin that cannot be deactivated by
	 * the user, once
	 * activated as a device admin. It also cannot be uninstalled. To check whether
	 * a particular
	 * package is currently registered as the device owner app, pass in the package
	 * name from
	 * {@link Context#getPackageName()} to this method.
	 * <p/>
	 * This is useful for device
	 * admin apps that want to check whether they are also registered as the device
	 * owner app. The
	 * exact mechanism by which a device admin app is registered as a device owner
	 * app is defined by
	 * the setup process.
	 * 
	 * @param packageName the package name of the app, to compare with the
	 *                    registered device owner
	 *                    app, if any.
	...
	/**
	 * @return true if a package is registered as device owner, only when it's
	 *         running on the
	 *         calling user.
	 *
	 *         <p>
	 *         Same as {@link #isDeviceOwnerApp}, but bundled code should use it for
	 *         clarity.
	 * @hide
	 */
	...
	/**
	 * @return true if a package is registered as device owner, even if it's running
	 *         on a different
	 *         user.
	 *
	 *         <p>
	...
	/**
	 * @return device owner component name, only when it's running on the calling
	 *         user.
	 *
	 * @hide
	 */
	...
	/**
	 * @return device owner component name, even if it's running on a different
	 *         user.
	 *
	 * @hide
	 */
	...
	/**
	 * @return Handle of the user who runs device owner, or {@code null} if there's
	 *         no device owner.
	 *
	 * @hide
	 */
	...
	/**
	 * Clears the current device owner. The caller must be the device owner. This
	 * function should be
	 * used cautiously as once it is called it cannot be undone. The device owner
	 * can only be set as
	 * a part of device setup, before it completes.
	 * <p>
	 * While some policies previously set by the device owner will be cleared by
	 * this method, it is
	 * a best-effort process and some other policies will still remain in place
	 * after the device
	 * owner is cleared.
	 *
	 * @param packageName The package name of the device owner.
	 * @throws SecurityException if the caller is not in {@code packageName} or
	 *                           {@code packageName}
	 *                           does not own the current device owner component.
	 *
	 * @deprecated This method is expected to be used for testing purposes only. The
	 *             device owner
	 *             will lose control of the device and its data after calling it. In
	 *             order to protect any
	 *             sensitive data that remains on the device, it is advised that the
	 *             device owner factory resets
	...
	/**
	 * Returns the device owner package name, only if it's running on the calling
	 * user.
	 *
	 * <p>
	...
	private IpcDataCache<Void, Boolean> mHasDeviceOwnerCache = new IpcDataCache<>(
			sDpmCaches.child("hasDeviceOwner"),
			(query) -> getService().hasDeviceOwner());

	/**
	 * Called by the system to find out whether the device is managed by a Device
	 * Owner.
	 *
	 * @return whether the device is managed by a Device Owner.
	 * @throws SecurityException if the caller is not the device owner, does not
	 *                           hold
	 *                           MANAGE_USERS or MANAGE_PROFILE_AND_DEVICE_OWNERS
	 *                           permissions and is not the system.
	 *
	 * @hide
	 */
	@SystemApi
	...
	/**
	 * Returns the device owner name. Note this method *will* return the device
	 * owner
	 ...
	 *             Sets the given component as an active admin and registers the
	 *             package as the profile
	 *             owner for this user. The package must already be installed and
	 *             there shouldn't be
	 *             an existing profile owner registered for this user. Also, this
	 *             method must be called
	 *             before the user setup has been completed.
	 *             <p>
	 *             This method can only be called by system apps that hold
	 *             MANAGE_USERS permission and
	 *             MANAGE_DEVICE_ADMINS permission.
	 * @param admin     The component to register as an active admin and profile
	 *                  owner.
	 * @param ownerName The user-visible name of the entity that is managing this
	 *                  user.
	 * @return whether the admin was successfully registered as the profile owner.
	 * @throws IllegalArgumentException if packageName is null, the package isn't
	 *                                  installed, or
	...
	/**
	 * Clears the active profile owner. The caller must be the profile owner of this
	 * user, otherwise
	 * a SecurityException will be thrown. This method is not available to managed
	 * profile owners.
	 * <p>
	 * While some policies previously set by the profile owner will be cleared by
	 * this method, it is
	 * a best-effort process and some other policies will still remain in place
	 * after the profile
	 * owner is cleared.
	 ...
	 * @throws SecurityException if {@code admin} is not an active profile owner, or
	 *                           the method is
	 *                           being called from a managed profile.
	 *
	 * @deprecated This method is expected to be used for testing purposes only. The
	 *             profile owner
	 *             will lose control of the user and its data after calling it. In
	 *             order to protect any
	 *             sensitive data that remains on this user, it is advised that the
	 *             profile owner deletes it
	...
	/**
	 * @hide
	 *       Sets the given component as the profile owner of the given user
	 *       profile. The package must
	 *       already be installed. There must not already be a profile owner for
	 *       this user.
	 *       Only apps with the MANAGE_PROFILE_AND_DEVICE_OWNERS permission and the
	 *       shell uid can call
	 *       this method.
	 *       Calling this after the setup phase of the specified user has completed
	 *       is allowed only if:
	 *       - the caller is SYSTEM_UID.
	 *       - or the caller is the shell uid, and there are no accounts on the
	 *       specified user.
	 * @param admin      the component name to be registered as profile owner.
	 * @param userHandle the userId to set the profile owner for.
	 * @return whether the component was successfully registered as the profile
	 *         owner.
	 * @throws IllegalArgumentException if admin is null, the package isn't
	 *                                  installed, or the
	 ...
	 * <p>
	 * Device owner information set using this method overrides any owner
	 * information manually set
	 * by the user and prevents the user from further changing it.
	 * <p>
	 * If the device owner information is {@code null} or empty then the device
	 * owner info is
	 * cleared and the user owner info is shown on the lock screen if it is set.
	 * <p>
	 * If the device owner information contains only whitespaces then the message on
	 * the lock screen
	 * will be blank and the user will not be allowed to change it.
	 * <p>
	 * If the device owner information needs to be localized, it is the
	 * responsibility of the
	 * {@link DeviceAdminReceiver} to listen to the
	 * {@link Intent#ACTION_LOCALE_CHANGED} broadcast
	 * and set a new version of this string accordingly.
	 * <p>
	 * May be called by the device owner or the profile owner of an
	 * organization-owned device.
	 *
	 * @param admin The name of the admin component to check.
	 * @param info  Device owner information which will be displayed instead of the
	 *              user owner info.
	 * @throws SecurityException if {@code admin} is not a device owner.
	 */
	...
	/**
	 * Called by device or profile owners to suspend packages for this user. This
	 * function can be
	 ...
	 * <p>
	 * A suspended package will not be able to start activities. Its notifications
	 * will be hidden,
	 * it will not show up in recents, will not be able to show toasts or dialogs or
	 * ring the
	 * device.
	 * <p>
	 ...
	 *
	 * <p>
	 * Some apps cannot be suspended, such as device admins, the active launcher,
	 * the required
	 * package installer, the required package uninstaller, the required package
	 * verifier, the
	 * default dialer, and the permission controller.
	 *
	 * @param admin        Which {@link DeviceAdminReceiver} this request is
	 *                     associated with. Null if the
	 *                     caller is not a device admin.
	 * @param packageNames The package names to suspend or unsuspend.
	 * @param suspended    If set to {@code true} than the packages will be
	 *                     suspended, if set to
	 *                     {@code false} the packages will be unsuspended.
	 * @return an array of package names for which the suspended status is not set
	 *         as requested in
	 *         this method.
	 * @throws SecurityException if {@code admin} is not a device or profile owner.
	 * @see #setDelegatedScopes
	...
	/**
	 * Determine if a package is suspended. This function can be called by a device
	 * owner, profile
	 * owner, or by a delegate given the {@link #DELEGATION_PACKAGE_ACCESS} scope
	 * via
	 ...
	 *                    associated with. Null if the
	 *                    caller is not a device admin.
	 * @param packageName The name of the package to retrieve the suspended status
	 *                    of.
	 * @return {@code true} if the package is suspended or {@code false} if the
	 *         package is not
	 *         suspended, could not be found or an error occurred.
	 * @throws SecurityException     if {@code admin} is not a device or profile
	 *                               owner or has not been
	...
	/**
	 * Sets the enabled state of the profile. A profile should be enabled only once
	 * it is ready to
	...
	/**
	 * Sets the name of the profile. In the device owner case it sets the name of
	 * the user which it
	 * is called from. Only a profile owner or device owner can call this. If this
	 * is never called
	 ...
	 * @param admin       Which {@link DeviceAdminReceiver} this request is
	 *                    associate with.
	 * @param profileName The name of the profile. If the name is longer than 200
	 *                    characters
	...
	/**
	 * Used to determine if a particular package is registered as the profile owner
	 * for the
	 * user. A profile owner is a special device admin that has additional
	 * privileges
	 * within the profile.
	 *
	 * @param packageName The package name of the app to compare with the registered
	 *                    profile owner.
	...
	/**
	 * @hide
	 * @return the packageName of the owner of the given user profile or
	 *         {@code null} if no profile
	 ...
	 * @hide
	 */
	@RequiresPermission(value = android.Manifest.permission.INTERACT_ACROSS_USERS, conditional = true)
	...
	private final IpcDataCache<UserHandle, ComponentName> mGetProfileOwnerOrDeviceOwnerSupervisionComponentCache = new IpcDataCache<>(
	...
	/**
	 * Returns the configured supervision app if it exists and is the device owner
	 * or policy owner.
	 * 
	 * @hide
	 */
	...
	/**
	 * @hide
	 * @return the human readable name of the organisation associated with this DPM
	 *         or {@code null}
	 *         if one is not set.
	 * @throws IllegalArgumentException if the userId is invalid.
	 */
	...
	private final IpcDataCache<Void, Boolean> mIsOrganizationOwnedDeviceWithManagedProfileCache = new IpcDataCache(
	 ...
	 *
	 * This, together with checking whether the device has a device owner (by
	 * calling
	 * {@link #isDeviceOwnerApp}), could be used to learn whether the device is
	 * owned by an
	 * organization or an individual:
	 * If this method returns true OR {@link #isDeviceOwnerApp} returns true (for
	 * any package),
	 * then the device is owned by an organization. Otherwise, it's owned by an
	 * individual.
	 *
	 * @return {@code true} if the device was provisioned as organization-owned
	 *         device,
	 *         {@code false} otherwise.
	 */
	 ...
	 *
	 * @param packageName The package name of the app to check for device identifier
	 *                    access.
	 ...
	 * {@link android.Manifest.permission#MANAGE_DEVICE_POLICY_LOCK_TASK}. to set a
	 * default activity
	 * that the system selects to handle intents that match the given
	 * {@link IntentFilter}.
	 * This activity will remain the default intent handler even if the set of
	 * potential event
	 * handlers for the intent filter changes and if the intent preferences are
	 * reset.
	 * <p>
	 * Note that the caller should still declare the activity in the manifest, the
	 * API just sets
	 * the activity to be the default one to handle the given intent filter.
	 * <p>
	 * The default disambiguation mechanism takes over if the activity is not
	 * installed (anymore).
	 * When the activity is (re)installed, it is automatically reset as default
	 * intent handler for
	 * the filter.
	 * <p>
	 * The calling device admin must be a profile owner or device owner. If it is
	 * not, a security
	 * exception will be thrown.
	 * <p>
	 * Starting from {@link Build.VERSION_CODES#UPSIDE_DOWN_CAKE}, after the
	 * persistent preferred
	 * activity policy has been set,
	 * {@link PolicyUpdateReceiver#onPolicySetResult(Context, String,
	 * Bundle, TargetUser, PolicyUpdateResult)} will notify the admin on whether the
	 * policy was
	 * successfully set or not. This callback will contain:
	 ...
	 * {@link PolicyUpdateReceiver#EXTRA_INTENT_FILTER} the intent filter the policy
	 * applies to
	 * <li>The {@link TargetUser} that this policy relates to
	 * <li>The {@link PolicyUpdateResult}, which will be
	 * {@link PolicyUpdateResult#RESULT_POLICY_SET} if the policy was successfully
	 ...
	 *                           or holder of the
	 *                           permission
	 *                           {@link android.Manifest.permission#MANAGE_DEVICE_POLICY_LOCK_TASK}.
	 */
	 ...
	 * to remove all
	 * persistent intent handler preferences associated with the given package that
	 * were set by
	 ...
	 * Starting from {@link Build.VERSION_CODES#UPSIDE_DOWN_CAKE}, after the
	 * persistent preferred
	 * activity policy has been cleared,
	 * {@link PolicyUpdateReceiver#onPolicySetResult(Context,
	 * String, Bundle, TargetUser, PolicyUpdateResult)} will notify the admin on
	 * whether the policy
	...
	/**
	 * Must be called by a device owner or a profile owner of an organization-owned
	 * managed profile
	 * to set the default SMS application.
	 * <p>
	 * This method can be called on the {@link DevicePolicyManager} instance,
	 * returned by
	 * {@link #getParentProfileInstance(ComponentName)}, where the caller must be
	 * the profile owner
	 * of an organization-owned managed profile and the package must be a
	 * pre-installed system
	 * package. If called on the parent instance, then the default SMS application
	 * is set on the
	 * personal profile.
	 * <p>
	 * Starting from Android
	 * {@link android.os.Build.VERSION_CODES#UPSIDE_DOWN_CAKE}, the profile
	 * owner of an organization-owned managed profile can also call this method
	 * directly (not on the
	 * parent profile instance) to set the default SMS application in the work
	 * profile. This is only
	 * meaningful when work profile telephony is enabled by
	 * {@link #setManagedSubscriptionsPolicy}.
	 *
	 * @param admin       Which {@link DeviceAdminReceiver} this request is
	 *                    associated with. Null if the
	 *                    caller is not a device admin.
	 * @param packageName The name of the package to set as the default SMS
	 *                    application.
	 * @throws SecurityException        if {@code admin} is not a device or profile
	 *                                  owner or if
	 *                                  called on the parent profile and the
	 *                                  {@code admin} is not a
	 *                                  profile owner of an organization-owned
	 *                                  managed profile.
	 * @throws IllegalArgumentException if called on the parent profile and the
	 *                                  package
	 *                                  provided is not a pre-installed system
	 *                                  package.
	 * @throws IllegalStateException    while trying to set default sms app on the
	 *                                  profile and
	 ...
	 * <p>
	 * When the profile owner of an organization-owned managed profile calls this
	 * method, it sets
	 * the default dialer application in the work profile. This is only meaningful
	 * when work profile
	 * telephony is enabled by {@link #setManagedSubscriptionsPolicy}.
	 * <p>
	 * If the device does not support telephony
	 * ({@link PackageManager#FEATURE_TELEPHONY}), calling
	 * this method will do nothing.
	 *
	 * @param packageName The name of the package to set as the default dialer
	 *                    application.
	 * @throws SecurityException        if {@code admin} is not a device or profile
	 *                                  owner or a
	 *                                  profile owner of an organization-owned
	 *                                  managed profile.
	 * @throws IllegalArgumentException if the package cannot be set as the default
	 *                                  dialer, for
	 *                                  example if the package is not installed or
	 *                                  does not expose
	 *                                  the expected activities or services that a
	 *                                  dialer app is
	 *                                  required to have.
	 */
	...
	/**
	 * Called by a profile owner or device owner to grant permission to a package to
	 * manage
	 * application restrictions for the calling user via
	 * {@link #setApplicationRestrictions} and
	 * {@link #getApplicationRestrictions}.
	 * <p>
	 * This permission is persistent until it is later cleared by calling this
	 * method with a
	 * {@code null} value or uninstalling the managing package.
	 * <p>
	 * The supplied application restriction managing package must be installed when
	 * calling this
	 * API, otherwise an {@link NameNotFoundException} will be thrown.
	 *
	 * @param admin       Which {@link DeviceAdminReceiver} this request is
	 *                    associated with.
	 * @param packageName The package name which will be given access to application
	 *                    restrictions
	 *                    APIs. If {@code null} is given the current package will be
	 *                    cleared.
	 * @throws SecurityException     if {@code admin} is not a device or profile
	 *                               owner.
	...
	/**
	 * Called by a profile owner or device owner to retrieve the application
	 * restrictions managing
	 * package for the current user, or {@code null} if none is set. If there are
	 * multiple
	 * delegates this function will return one of them.
	 *
	 * @param admin Which {@link DeviceAdminReceiver} this request is associated
	 *              with.
	 * @return The package name allowed to manage application restrictions on the
	 *         current user, or
	...
	/**
	 * Called by any application to find out whether it has been granted permission
	 * via
	 ...
	 *
	 * <p>
	 * This is done by comparing the calling Linux uid with the uid of the package
	 * specified by
	 * that method.
	 *
	 * @deprecated From {@link android.os.Build.VERSION_CODES#O}. Use
	 *             {@link #getDelegatedScopes}
	 *             instead.
	 */
	@Deprecated
	...
	/**
	 * Sets the application restrictions for a given target application running in
	 * the calling user.
	 * <p>
	 * The caller must be a profile or device owner on that user, or the package
	 * allowed to manage
	 * application restrictions via {@link #setDelegatedScopes} with the
	 * {@link #DELEGATION_APP_RESTRICTIONS} scope; otherwise a security exception
	 * will be thrown.
	 * <p>
	 * The provided {@link Bundle} consists of key-value pairs, where the types of
	 * values may be:
	 * <ul>
	 * <li>{@code boolean}
	 * <li>{@code int}
	 * <li>{@code String} or {@code String[]}
	 * <li>From {@link android.os.Build.VERSION_CODES#M}, {@code Bundle} or
	 * {@code Bundle[]}
	 * </ul>
	 * <p>
	 * If the restrictions are not available yet, but may be applied in the near
	 * future, the caller
	 * can notify the target application of that by adding
	 * {@link UserManager#KEY_RESTRICTIONS_PENDING} to the settings parameter.
	 * <p>
	 * The application restrictions are only made visible to the target application
	 * via
	 * {@link UserManager#getApplicationRestrictions(String)}, in addition to the
	 * profile or device
	 * owner, and the application restrictions managing package via
	 * {@link #getApplicationRestrictions}.
	 *
	 * <p>
	 * Starting from Android Version
	 * {@link android.os.Build.VERSION_CODES#UPSIDE_DOWN_CAKE},
	 * multiple admins can set app restrictions for the same application, the target
	 * application can
	 ...
	 * @param admin       Which {@link DeviceAdminReceiver} this request is
	 *                    associated with, or
	 *                    {@code null} if called by the application restrictions
	 *                    managing package.
	 * @param packageName The name of the package to update restricted settings for.
	 * @param settings    A {@link Bundle} to be parsed by the receiving
	 *                    application, conveying a new
	 ...
	 * This is meant to
	 * be used in conjunction with {@link #KEYGUARD_DISABLE_TRUST_AGENTS}, which
	 * disables all trust
	 * agents but those enabled by this function call. If flag
	 * {@link #KEYGUARD_DISABLE_TRUST_AGENTS} is not set, then this call has no
	 * effect.
	 * <p>
	 * For any specific trust agent, whether it is disabled or not depends on the
	 * aggregated state
	 * of each admin's {@link #KEYGUARD_DISABLE_TRUST_AGENTS} setting and its trust
	 * agent
	 ...
	 * trust agent configuration, the trust agent is disabled completely. Otherwise,
	 * the trust agent
	 * will receive the list of configurations from all admins who set
	 * {@link #KEYGUARD_DISABLE_TRUST_AGENTS} and aggregate the configurations to
	 * determine its
	 ...
	 * This method can be called on the {@link DevicePolicyManager} instance
	 * returned by
	 * {@link #getParentProfileInstance(ComponentName)} in order to set the
	 * configuration for
	 * the parent profile.
	 * <p>
	 * On devices not supporting {@link PackageManager#FEATURE_SECURE_LOCK_SCREEN}
	 ...
	 * @param configuration Trust-agent-specific feature configuration bundle.
	 *                      Please consult
	 *                      documentation of the specific trust agent to determine
		...
		if (mService != null) {
			try {
				mService.setTrustAgentConfiguration(
						admin, mContext.getPackageName(), target, configuration,
						mParentInstance);
			} catch (RemoteException e) {
				throw e.rethrowFromSystemServer();
			}
	...
	/**
	 * Gets configuration for the given trust agent based on aggregating all calls
	 * to
	 ...
	 * @param admin Which {@link DeviceAdminReceiver} this request is associated
	 *              with. If null,
	 *              this function returns a list of configurations for all admins
	 *              that declare
	 *              {@link #KEYGUARD_DISABLE_TRUST_AGENTS}. If any admin declares
	 *              {@link #KEYGUARD_DISABLE_TRUST_AGENTS} but doesn't call
	 *              {@link #setTrustAgentConfiguration(ComponentName, ComponentName, PersistableBundle)}
	 *              for this {@param agent} or calls it with a null configuration,
	 *              null is returned.
	...
	/**
	 * Called by a profile owner of a managed profile to set whether caller-Id
	 ...
	 * with a {@link PackagePolicy#PACKAGE_POLICY_BLOCKLIST} policy type when
	 * {@code disabled} is
	 ...
	 * @param admin    Which {@link DeviceAdminReceiver} this request is associated
	 *                 with.
	 * @param disabled If true caller-Id information in the managed profile is not
	 *                 displayed.
	 * @throws SecurityException if {@code admin} is not a profile owner.
	 * @deprecated starting with
	 *             {@link android.os.Build.VERSION_CODES#UPSIDE_DOWN_CAKE}, use
	 *             {@link #setManagedProfileCallerIdAccessPolicy(PackagePolicy)}
	 *             instead
	 */
	@Deprecated
	...
	/**
	 * Called by a profile owner of a managed profile to determine whether or not
	 * caller-Id
	...
	/**
	 * Called by the system to determine whether or not caller-Id information has
	 * been disabled.
	 * <p>
	 * Starting with {@link android.os.Build.VERSION_CODES#UPSIDE_DOWN_CAKE},
	 * this will return true when
	 ...
	 * @deprecated use {@link #hasManagedProfileCallerIdAccess(UserHandle, String)}
	 *             and provide the
	...
	/**
	 * Called by a device owner or profile owner of a managed profile to set the
	 * credential manager
	 * policy.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * A {@link PackagePolicy#PACKAGE_POLICY_ALLOWLIST} policy type will limit the
	 * credential
	 ...
	 * allows access from the OEM default credential providers and the allowlist of
	 * credential
	 * providers.
	 *
	 * <p>
	 * A {@link PackagePolicy#PACKAGE_POLICY_BLOCKLIST} policy type will block the
	 * credential
	 * providers listed in the policy from being used by the user.
	 *
	 * @param policy the policy to set, setting this value to {@code null} will
	 *               allow all packages
	 * @throws SecurityException if caller is not a device owner or profile owner of
	 *                           a
	 *                           managed profile
	 */
	...
	/**
	 * Called by a device owner or profile owner of a managed profile to retrieve
	 * the credential
	 * manager policy.
	 *
	 * @throws SecurityException if caller is not a device owner or profile owner of
	 *                           a
	 *                           managed profile.
	 * @return the current credential manager policy if null then this policy has
	 *         not been
	 *         configured.
	 */
	@UserHandleAware(enabledSinceTargetSdkVersion = UPSIDE_DOWN_CAKE, requiresPermissionIfNotCaller = INTERACT_ACROSS_USERS)
	...
	/**
	 * Called by a profile owner of a managed profile to set the packages that are
	 * allowed to
	 * lookup contacts in the managed profile based on caller id information.
	 * <p>
	 * For example, the policy determines if a dialer app in the parent profile
	 * resolving
	 ...
	 * <p>
	 * A {@link PackagePolicy#PACKAGE_POLICY_ALLOWLIST_AND_SYSTEM} policy type
	 * allows access from the OEM default packages for the Sms, Dialer and Contact
	 * roles,
	 * in addition to the packages specified in
	 * {@link PackagePolicy#getPackageNames()}
	 *
	 * @param policy the policy to set, setting this value to {@code null} will
	 *               allow
	 *               all packages
	 * @throws SecurityException if caller is not a profile owner of a managed
	 *                           profile
	 */
	...
	/**
	 * Called by a profile owner of a managed profile to retrieve the caller id
	 * policy.
	 * <p>
	 * The calling device admin must be a profile owner of a managed profile.
	 * If it is not, a {@link SecurityException} will be thrown.
	...
	/**
	 * Called by a profile owner of a managed profile to determine the current
	 * policy applied
	...
	/**
	 * Determine whether requesting package has ability to access contacts of the
	 * requested user
	 *
	 * @param userHandle  The user for whom to check the contacts search permission
	...
	/**
	 * Called by a profile owner of a managed profile to set whether contacts search
	 * from the
	 * managed profile will be shown in the parent profile, for incoming calls.
	 * <p>
	 * The calling device admin must be a profile owner. If it is not, a security
	 ...
	 * calling this function
	 * is similar to calling
	 * {@link #setManagedProfileContactsAccessPolicy(PackagePolicy)} with a
	 * {@link PackagePolicy#PACKAGE_POLICY_BLOCKLIST} policy type when
	 * {@code disabled} is false
	 * or a {@link PackagePolicy#PACKAGE_POLICY_ALLOWLIST} policy type when
	 * {@code disabled}
	 * is true.
	 *
	 * @param admin    Which {@link DeviceAdminReceiver} this request is associated
	 *                 with.
	 * @param disabled If true contacts search in the managed profile is not
	 *                 displayed.
	 * @throws SecurityException if {@code admin} is not a profile owner.
	 *
	 ...
	 * 
	 * @param userHandle The user for whom to check the contacts search permission
	 * @deprecated use {@link #hasManagedProfileContactsAccess(UserHandle, String)}
	 *             and provide the
	 *             package name requesting access
	 * @hide
	 */
	...
	/**
	 * Start Quick Contact on the managed profile for the user, if the policy
	 * allows.
	 *
	 * @hide
	 */
	...
	/**
	 * Called by the profile owner of a managed profile so that some intents sent in
	 * the managed
	 * profile can also be resolved in the parent, or vice versa. Only activity
	 * intents are
	 * supported.
	 *
	 * @param admin  Which {@link DeviceAdminReceiver} this request is associated
	 *               with.
	 * @param filter The {@link IntentFilter} the intent has to match to be also
	 *               resolved in the
	 *               other profile
	 * @param flags  {@link DevicePolicyManager#FLAG_MANAGED_CAN_ACCESS_PARENT} and
	 *               {@link DevicePolicyManager#FLAG_PARENT_CAN_ACCESS_MANAGED} are
	 *               supported.
	 * @throws SecurityException if {@code admin} is not a device or profile owner.
	 */
	...
	/**
	 * Called by a profile owner of a managed profile to remove the cross-profile
	 * intent filters
	 * that go from the managed profile to the parent, or from the parent to the
	 * managed profile.
	 * Only removes those that have been set by the profile owner.
	 * <p>
	 * <em>Note</em>: A list of default cross profile intent filters are set up by
	 * the system when
	 * the profile is created, some of them ensure the proper functioning of the
	 * profile, while
	 * others enable sharing of data from the parent to the managed profile for user
	 * convenience.
	 * These default intent filters are not cleared when this API is called. If the
	 * default cross
	 ...
	 * a device owner or profile owner the restriction applies to all profiles of
	 * the user the
	 * device owner or profile owner is an admin for. By default, the user can use
	 * any accessibility
	 * service. When zero or more packages have been added, accessibility services
	 * that are not in
	 * the list and not part of the system can not be enabled by the user.
	 * <p>
	 * Calling with a {@code null} value for the list disables the restriction so
	 * that all services
	 * can be used, calling with an empty list only allows the built-in system
	 * services. Any
	 * non-system accessibility service that's currently enabled must be included in
	 * the list.
	 * <p>
	 * System accessibility services are always available to the user and this
	 * method can't
	 * disable them.
	 * 
	 * @param admin        Which {@link DeviceAdminReceiver} this request is
	 ...
	 * @return {@code true} if the operation succeeded, or {@code false} if the list
	 *         didn't
	...
	/**
	 * Returns the list of permitted accessibility services set by this device or
	 * profile owner.
	 * <p>
	 * An empty list means no accessibility services except system services are
	 * allowed.
	...
	/**
	 * Called by the system to check if a specific accessibility service is disabled
	 * by admin.
	 *
	 * @param admin       Which {@link DeviceAdminReceiver} this request is
	 *                    associated with.
	 * @param packageName Accessibility service package name that needs to be
	 *                    checked.
	...
	/**
	 * Returns the list of accessibility services permitted by the device or
	 * profiles
	 * owners of this user.
	 *
	 * <p>
	 ...
	 * is returned
	 * it will contain the intersection of the permitted lists for any device or
	 * profile
	 * owners that apply to this user. It will also include any system accessibility
	 * services.
	 *
	 ...
	 * {@link android.Manifest.permission#MANAGE_DEVICE_POLICY_INPUT_METHODS}
	 * permission to set
	 * the permitted input methods services for this user. By default, the user can
	 * use any input
	 * method.
	 * <p>
	 * This method can be called on the {@link DevicePolicyManager} instance,
	 ...
	 * <li>Can only permit all input methods (calling this method with a
	 * {@code null} package
	 * list) or only permit system input methods (calling this method with an empty
	 * package
	 * list). This is to prevent the caller from learning which packages are
	 * installed on
	 * the personal side</li>
	 * </ul>
	 * <p>
	 * When zero or more packages have been added, input method that are not in the
	 * list and not
	 * part of the system can not be enabled by the user. This method will fail if
	 * it is called for
	 * a admin that is not for the foreground user or a profile of the foreground
	 * user. Any
	 * non-system input method service that's currently enabled must be included in
	 * the list.
	 * <p>
	 * Calling with a null value for the list disables the restriction so that all
	 * input methods can
	 * be used, calling with an empty list disables all but the system's own input
	 * methods.
	 * <p>
	 * System input methods are always available to the user - this method can't
	 * modify this.
	 *
	 * @param admin        Which {@link DeviceAdminReceiver} this request is
	 *                     associated with. Null if the
	 ...
	 *                                  hold the
	 *                                  {@link android.Manifest.permission#MANAGE_DEVICE_POLICY_INPUT_METHODS}
	 *                                  permission, or if called on the parent
	 *                                  profile and the
	 *                                  {@code admin} is not a profile owner of an
	 *                                  organization-owned
	 *                                  managed profile.
	 * @throws IllegalArgumentException if called on the parent profile, the
	 *                                  {@code admin} is a
	 *                                  profile owner of an organization-owned
	 *                                  managed profile and the
	 *                                  list of permitted input method package names
	...
	/**
	 * Returns the list of permitted input methods set by this device or profile
	 * owner.
	 * <p>
	 * This method can be called on the {@link DevicePolicyManager} instance,
	 * returned by {@link #getParentProfileInstance(ComponentName)}, where the
	 * caller must be
	 * a profile owner of an organization-owned managed profile. If called on the
	 * parent instance,
	 * then the returned list of permitted input methods are those which are applied
	 * on the
	 * personal profile.
	 * <p>
	 ...
	 * @throws SecurityException if {@code admin} is not a device, profile owner or
	 *                           if called on
	 *                           the parent profile and the {@code admin} is not a
	 *                           profile owner
	...
	public @Nullable List<String> getPermittedInputMethods(@Nullable ComponentName admin) {
		if (mService != null) {
			try {
				return mService.getPermittedInputMethods(admin, mContext.getPackageName(),
						mParentInstance);
			} catch (RemoteException e) {
				throw e.rethrowFromSystemServer();
			}
	...
	/**
	 * Called by the system to check if a specific input method is disabled by
	 * admin.
	 * <p>
	 * This method can be called on the {@link DevicePolicyManager} instance,
	 * returned by {@link #getParentProfileInstance(ComponentName)}. If called on
	 * the parent
	 * instance, this method will check whether the given input method is permitted
	 * on
	 ...
	 *
	 * <p>
	 * On {@link android.os.Build.VERSION_CODES#Q} and later devices, this method
	 * returns the
	 * result for the calling user.
	 * </p>
	 *
	 * <p>
	 * On Android P and prior devices, this method returns the result for the
	 * current user.
	 * </p>
	 *
	 * <p>
	 ...
	 * it will contain the intersection of the permitted lists for any device or
	 * profile
	 * owners that apply to this user. It will also include any system input
	 * methods.
	 *
	 * @return List of input method package names.
	 * @hide
	 ...
	 *
	 * <p>
	 * {@code null} means all input methods are allowed, if a non-null list is
	 * returned
	 * it will contain the intersection of the permitted lists for any device or
	 * profile
	 * owners that apply to this user. It will also include any system input
	 ...
	 * Called by a profile owner of a managed profile to set the packages that are
	 * allowed to use
	 * a {@link android.service.notification.NotificationListenerService} in the
	 * primary user to
	 * see notifications from the managed profile. By default all packages are
	 * permitted by this
	 * policy. When zero or more packages have been added, notification listeners
	 * installed on the
	 * primary user that are not in the list and are not part of the system won't
	 * receive events
	 * for managed profile notifications.
	 * <p>
	 * Calling with a {@code null} value for the list disables the restriction so
	 * that all
	 * notification listener services be used. Calling with an empty list disables
	 * all but the
	 * system's own notification listeners. System notification listener services
	 * are always
	 * available to the user.
	 * <p>
	 * If a device or profile owner want to stop notification listeners in their
	 * user from seeing
	 * that user's notifications they should prevent that service from running
	 * instead (e.g. via
	 ...
	 * @return true if setting the restriction succeeded. It will fail if called
	 *         outside a managed
	 *         profile
	 * @throws SecurityException if {@code admin} is not a profile owner.
	 *
	...
	/**
	 * Returns the list of packages installed on the primary user that allowed to
	 * use a
	 ...
	 * <p>
	 * An empty list means no notification listener services except system ones are
	 * allowed.
	 * A {@code null} return value indicates that all notification listeners are
	 * allowed.
	 */
	...
	/**
	 * Returns true if {@code NotificationListenerServices} from the given package
	 * are allowed to
	 * receive events for notifications from the given user id. Can only be called
	 * by the system uid
	 *
	...
	/**
	 * Get the list of apps to keep around as APKs even if no user has currently
	 * installed it. This
	 * function can be called by a device owner or by a delegate given the
	 * {@link #DELEGATION_KEEP_UNINSTALLED_PACKAGES} scope via
	 * {@link #setDelegatedScopes}.
	 * <p>
	 * Please note that packages returned in this method are not automatically
	 * pre-cached.
	 *
	 * @param admin Which {@link DeviceAdminReceiver} this request is associated
	 *              with, or
	 *              {@code null} if the caller is a keep uninstalled packages
	 *              delegate.
	...
	/**
	 * Set a list of apps to keep around as APKs even if no user has currently
	 * installed it. This
	 * function can be called by a device owner or by a delegate given the
	 * {@link #DELEGATION_KEEP_UNINSTALLED_PACKAGES} scope via
	 * {@link #setDelegatedScopes}.
	 *
	 * <p>
	 * Please note that setting this policy does not imply that specified apps will
	 * be
	...
	/**
	 * Flag used by {@link #createAndManageUser} to skip setup wizard after creating
	 * a new user.
	 */
	public static final int SKIP_SETUP_WIZARD = 0x0001;

	/**
	 * Flag used by {@link #createAndManageUser} to specify that the user should be
	 * created
	 * ephemeral. Ephemeral users will be removed after switching to another user or
	 * rebooting the
	 * device.
	 */
	...
	/**
	 * Flag used by {@link #createAndManageUser} to specify that the newly created
	 * user should skip
	 ...
	 * component of
	 * the calling package as profile owner. The UserHandle returned by this method
	 * should not be
	 * persisted as user handles are recycled as users are removed and created. If
	 * you need to
	 * persist an identifier for this user, use
	 * {@link UserManager#getSerialNumberForUser}. The new
	 * user will not be started in the background.
	 * <p>
	 * admin is the {@link DeviceAdminReceiver} which is the device owner.
	 * profileOwner is also a
	 * DeviceAdminReceiver in the same package as admin, and will become the profile
	 * owner and will
	 * be registered as an active admin on the new user. The profile owner package
	 * will be installed
	 * on the new user.
	 * <p>
	 * If the adminExtras are not null, they will be stored on the device until the
	 * user is started
	 * for the first time. Then the extras will be passed to the admin when onEnable
	 * is called.
	 * <p>
	 * From {@link android.os.Build.VERSION_CODES#P} onwards, if targeting
	 * {@link android.os.Build.VERSION_CODES#P}, throws
	 * {@link UserOperationException} instead of
	 * returning {@code null} on failure.
	 *
	 * @param admin        Which {@link DeviceAdminReceiver} this request is
	 *                     associated with.
	 * @param name         The user's name.
	 * @param profileOwner Which {@link DeviceAdminReceiver} will be profile owner.
	 *                     Has to be in the
	 *                     same package as admin, otherwise no user is created and
	 *                     an
	 *                     IllegalArgumentException is thrown.
	 * @param adminExtras  Extras that will be passed to onEnable of the admin
	 *                     receiver on the new
	 *                     user.
	 * @param flags        {@link #SKIP_SETUP_WIZARD}, {@link #MAKE_USER_EPHEMERAL}
	 *                     and
	 *                     {@link #LEAVE_ALL_SYSTEM_APPS_ENABLED} are supported.
	 * @see UserHandle
	 * @return the {@link android.os.UserHandle} object for the created user, or
	 *         {@code null} if the
	 *         user could not be created.
	 * @throws SecurityException      if {@code admin} is not a device owner.
	 * @throws UserOperationException if the user could not be created and the
	 *                                calling app is
	 *                                targeting
	 *                                {@link android.os.Build.VERSION_CODES#P} and
	 *                                running on
	...
	/**
	 * Called by a device owner to switch the specified secondary user to the
	 * foreground.
	 *
	 * @param admin      Which {@link DeviceAdminReceiver} this request is
	 *                   associated with.
	...
	/**
	 * Called by a profile owner of secondary user that is affiliated with the
	 * device to stop the
	 * calling user and switch back to primary user (when the user was
	 * {@link #switchUser(ComponentName, UserHandle)} switched to) or stop the user
	 * (when it was
	 * {@link #startUserInBackground(ComponentName, UserHandle) started in
	 * background}.
	 *
	 * <p>
	 ...
	 * there is no primary
	 * user, so it switches back to the user that was in the foreground before the
	 * first call to
	 * {@link #switchUser(ComponentName, UserHandle)} (or fails with
	 * {@link UserManager#USER_OPERATION_ERROR_UNKNOWN} if that method was not
	 ...
	 *         {@link UserManager#USER_OPERATION_ERROR_MANAGED_PROFILE},
	 *         {@link UserManager#USER_OPERATION_ERROR_CURRENT_USER}
	 * @throws SecurityException if {@code admin} is not a profile owner affiliated
	 *                           with the device.
	 * @see #getSecondaryUsers(ComponentName)
	 */
	 ...
	 *
	 * <p>
	 * Managed subscriptions policy controls how SIMs would be associated with the
	 ...
	 *
	 * @param policy {@link ManagedSubscriptionsPolicy} policy, passing null for
	 *               this resets the
	 *               policy to be the default.
	 * @throws SecurityException             if the caller is not a profile owner on
	 *                                       an organization-owned
	 *                                       managed profile.
	 * @throws IllegalStateException         if called after the device setup has
	 *                                       been completed.
	 * @throws UnsupportedOperationException if managed subscriptions policy is not
	 *                                       explicitly
	 *                                       enabled by the device policy management
	 *                                       role holder during device setup.
	 ...
	 * If the policy has not been set, it will return a default policy of Type
	 * {@link
	...
	/**
	 * Called by a device owner to list all secondary users on the device. Managed
	 * profiles are not
	...
	/**
	 * Retrieves the application restrictions for a given target application running
	 * in the calling
	 * user.
	 * <p>
	 * The caller must be a profile or device owner on that user, or the package
	 ...
	 *         {@link DevicePolicyManager#setApplicationRestrictions} was called, or
	 *         an empty
	...
	/**
	 * Called by a profile owner, device owner or a holder of any permission that is
	 * associated with
	 * a user restriction to set a user restriction specified by the key.
	 * <p>
	 * The calling device admin must be a profile owner, device owner or holder of
	 * any permission
	 * that is associated with a user restriction; if it is not, a security
	 * exception will be thrown.
	 * <p>
	 * The profile owner of an organization-owned managed profile may invoke this
	 * method on
	 * the {@link DevicePolicyManager} instance it obtained from
	 * {@link #getParentProfileInstance(ComponentName)}, for enforcing device-wide
	 * restrictions.
	 * <p>
	 * See the constants in {@link android.os.UserManager} for the list of
	 * restrictions that can
	 * be enforced device-wide. These constants will also state in their
	 * documentation which
	 * permission is required to manage the restriction using this API.
	 *
	 * <p>
	 * For callers targeting Android
	 * {@link android.os.Build.VERSION_CODES#UPSIDE_DOWN_CAKE} or
	 * above, calling this API will result in applying the restriction locally on
	 * the calling user,
	 * or locally on the parent profile if called from the
	 * {@link DevicePolicyManager} instance obtained from
	 * {@link #getParentProfileInstance(ComponentName)}. To set a restriction
	 * globally, call
	 * {@link #addUserRestrictionGlobally} instead.
	 *
	 * <p>
	 * Starting from {@link Build.VERSION_CODES#UPSIDE_DOWN_CAKE}, after the user
	 * restriction
	 * policy has been set,
	 * {@link PolicyUpdateReceiver#onPolicySetResult(Context, String,
	 * Bundle, TargetUser, PolicyUpdateResult)} will notify the admin on whether the
	 ...
	 * @throws SecurityException if {@code admin} is not a device or profile owner
	 *                           and if the caller
	 *                           has not been granted the permission to set the
	 ...
	 *
	 * <p>
	 * For a given user, a restriction will be set if it was applied globally or
	 * locally by any
	 * admin.
	 *
	 * <p>
	 * The calling device admin must be a profile owner, device owner or or a holder
	 * of any
	 * permission that is associated with a user restriction; if it is not, a
	 * security
	 * exception will be thrown.
	 *
	 * <p>
	 ...
	 * PolicyUpdateResult)} will notify the admin on whether the policy was
	 ...
	 * @throws SecurityException     if {@code admin} is not a device or profile
	 *                               owner and if the
	 *                               caller has not been granted the permission to
	 *                               set the given user restriction.
	 ...
	 * <p>
	 * The calling device admin must be a profile or device owner; if it is not, a
	 * security
	 * exception will be thrown.
	 * <p>
	 * The profile owner of an organization-owned managed profile may invoke this
	 * method on
	 * the {@link DevicePolicyManager} instance it obtained from
	 * {@link #getParentProfileInstance(ComponentName)}, for clearing device-wide
	 * restrictions.
	 * <p>
	 * See the constants in {@link android.os.UserManager} for the list of
	 * restrictions. These
	 * constants state in their documentation which permission is required to manage
	 * the restriction
	 * using this API.
	 *
	 * <p>
	 * For callers targeting Android
	 * {@link android.os.Build.VERSION_CODES#UPSIDE_DOWN_CAKE} or
	 * above, calling this API will result in clearing any local and global
	 ...
	 * @throws SecurityException if {@code admin} is not a device or profile owner
	 *                           and if the
	 *                           caller has not been granted the permission to set
	 *                           the given user restriction.
	 */
	 ...
	 * method on
	 * the {@link DevicePolicyManager} instance it obtained from
	 * {@link #getParentProfileInstance(ComponentName)}, for retrieving device-wide
	 * restrictions
	 ...
	 * For callers targeting Android
	 * {@link android.os.Build.VERSION_CODES#UPSIDE_DOWN_CAKE} or
	 * above, this API will return the local restrictions set on the calling user,
	 * or on the parent
	 * profile if called from the {@link DevicePolicyManager} instance obtained from
	 * {@link #getParentProfileInstance(ComponentName)}. To get global restrictions
	 * set by admin,
	 * call {@link #getUserRestrictionsGlobally()} instead.
	 *
	 * <p>
	 * Note that this is different that the returned restrictions for callers
	 * targeting pre
	 * Android {@link android.os.Build.VERSION_CODES#UPSIDE_DOWN_CAKE}, were this
	 * API returns
	 * all local/global restrictions set by the admin on the calling user using
	 * {@link #addUserRestriction(ComponentName, String)} or the parent user if
	 * called on the
	 * {@link DevicePolicyManager} instance it obtained from
	 * {@link #getParentProfileInstance}.
	 *
	 * @param admin Which {@link DeviceAdminReceiver} this request is associated
	 *              with.
	 * @return a {@link Bundle} whose keys are the user restrictions, and the values
	 *         a
	...
	/**
	 * Called by any app to display a support dialog when a feature was disabled by
	 * an admin.
	 * This returns an intent that can be used with
	 * {@link Context#startActivity(Intent)} to
	 * display the dialog. It will tell the user that the feature indicated by
	 * {@code restriction}
	 * was disabled by an admin, and include a link for more information. The
	 * default content of
	 * the dialog can be changed by the restricting admin via
	 * {@link #setShortSupportMessage(ComponentName, CharSequence)}. If the
	 * restriction is not
	 * set (i.e. the feature is available), then the return value will be
	 * {@code null}.
	 * 
	 * @param restriction Indicates for which feature the dialog should be
	 *                    displayed. Can be a
	 *                    user restriction from {@link UserManager}, e.g.
	 *                    {@link UserManager#DISALLOW_ADJUST_VOLUME}, or one of the
	 *                    constants
	 *                    {@link #POLICY_DISABLE_CAMERA} or
	 *                    {@link #POLICY_DISABLE_SCREEN_CAPTURE}.
	 * @return Intent An intent to be used to start the dialog-activity if the
	 *         restriction is
	 *         set by an admin, or null if the restriction does not exist or no
	 *         admin set it.
	 */
	 ...
	 * @param restriction The restriction enforced by admin. It could be any user
	 *                    restriction or
	 *                    policy like
	 *                    {@link DevicePolicyManager#POLICY_DISABLE_CAMERA} and
	 *                    {@link DevicePolicyManager#POLICY_DISABLE_SCREEN_CAPTURE}.
	 * @return Details of admin and user which enforced the restriction for the
	 *         userId. If
	 *         restriction is null, profile owner for the user or device owner info
	 *         is returned.
	 * @hide
	 */
	 ...
	 *
	 * <p>
	 * Note that for {@link #POLICY_SUSPEND_PACKAGES} it returns the PO or DO to
	 * keep the
	 * behavior the same as before the bug fix for b/192245204.
	 *
	 * <p>
	 * This API is only callable by the system UID
	 *
	 * @param userId      The user for whom to retrieve the information.
	 * @param restriction The restriction enforced by admins. It could be any user
	 *                    restriction or
	 *                    policy like
	 *                    {@link DevicePolicyManager#POLICY_DISABLE_CAMERA} and
	 *                    {@link DevicePolicyManager#POLICY_DISABLE_SCREEN_CAPTURE}.
	...
	/**
	 * Hide or unhide packages. When a package is hidden it is unavailable for use,
	 * but the data and
	 * actual package file remain. This function can be called by a device owner,
	 * profile owner, or
	 ...
	 * {@link #getParentProfileInstance(ComponentName)}, where the caller must be
	 * the profile owner
	 * of an organization-owned managed profile and the package must be a system
	 * package. If called
	 * on the parent instance, then the package is hidden or unhidden in the
	 * personal profile.
	 * <p>
	 * Starting from {@link Build.VERSION_CODES#UPSIDE_DOWN_CAKE}, after the
	 * application hidden
	 ...
	 * <li>The additional policy params bundle, which contains
	 * {@link PolicyUpdateReceiver#EXTRA_PACKAGE_NAME} the package name the policy
	 * applies to
	 * <li>The {@link TargetUser} that this policy relates to
	 * <li>The {@link PolicyUpdateResult}, which will be
	 * {@link PolicyUpdateResult#RESULT_POLICY_SET} if the policy was successfully
	 ...
	 * @param hidden      {@code true} if the package should be hidden,
	 *                    {@code false} if it should be
	 *                    unhidden.
	 * @return boolean Whether the hidden setting of the package was successfully
	 *         updated.
	 * @throws SecurityException        if {@code admin} is not a device or profile
	 *                                  owner or if called on
	 *                                  the parent profile and the {@code admin} is
	 *                                  not a profile owner of an
	 *                                  organization-owned managed profile.
	 * @throws IllegalArgumentException if called on the parent profile and the
	 *                                  package provided
	...
	/**
	 * Determine if a package is hidden. This function can be called by a device
	 * owner, profile
	 * owner, or by a delegate given the {@link #DELEGATION_PACKAGE_ACCESS} scope
	 * via
	 * {@link #setDelegatedScopes}.
	 ...
	 * of an organization-owned managed profile and the package must be a system
	 * package. If called
	 * on the parent instance, this will determine whether the package is hidden or
	 * unhidden in the
	 * personal profile.
	 * <p>
	 * Starting from {@link Build.VERSION_CODES#UPSIDE_DOWN_CAKE}, the returned
	 * policy will be the
	 ...
	 * @return boolean {@code true} if the package is hidden, {@code false}
	 *         otherwise.
	 * @throws SecurityException        if {@code admin} is not a device or profile
	 *                                  owner or if called on
	 *                                  the parent profile and the {@code admin} is
	...
	/**
	 * Re-enable a system app that was disabled by default when the user was
	 * initialized. This
	 ...
	 * @param admin       Which {@link DeviceAdminReceiver} this request is
	 *                    associated with, or
	 *                    {@code null} if the caller is an enable system app
	 *                    delegate.
	...
	/**
	 * Re-enable system apps by intent that were disabled by default when the user
	 * was initialized.
	 * This function can be called by a device owner, profile owner, or by a
	 * delegate given the
	 * {@link #DELEGATION_ENABLE_SYSTEM_APP} scope via {@link #setDelegatedScopes}.
	 *
	 * @param admin  Which {@link DeviceAdminReceiver} this request is associated
	 ...
	 * @param intent An intent matching the app(s) to be installed. All apps that
	 *               resolve for this
	 *               intent will be re-enabled in the calling profile.
	 * @return int The number of activities that matched the intent and were
	 *         installed.
	 * @throws SecurityException if {@code admin} is not a device or profile owner.
	 * @see #setDelegatedScopes
	 * @see #DELEGATION_PACKAGE_ACCESS
	...
	/**
	 * Install an existing package that has been installed in another user, or has
	 * been kept after
	 * removal via {@link #setKeepUninstalledPackages}.
	 * This function can be called by a device owner, profile owner or a delegate
	 * given
	 * the {@link #DELEGATION_INSTALL_EXISTING_PACKAGE} scope via
	 * {@link #setDelegatedScopes}.
	 * When called in a secondary user or managed profile, the user/profile must be
	 * affiliated with
	 ...
	 * @throws SecurityException if {@code admin} is not the device owner, or the
	 *                           profile owner of
	...
	/**
	 * Called by a device owner or profile owner to disable account management for a
	 * specific type
	 * of account.
	 * <p>
	 * The calling device admin must be a device owner or profile owner. If it is
	 * not, a security
	 * exception will be thrown.
	 * <p>
	 * When account management is disabled for an account type, adding or removing
	 * an account of
	 * that type will not be possible.
	 * <p>
	 * From {@link android.os.Build.VERSION_CODES#N} the profile or device owner can
	 * still use
	 * {@link android.accounts.AccountManager} APIs to add or remove accounts when
	 * account
	 * management for a specific type is disabled.
	 * <p>
	 * This method may be called on the {@code DevicePolicyManager} instance
	 * returned from
	 * {@link #getParentProfileInstance(ComponentName)} by the profile owner on an
	 * organization-owned device, to restrict accounts that may not be managed on
	 * the primary
	 * profile.
	 * <p>
	 * Starting from {@link Build.VERSION_CODES#UPSIDE_DOWN_CAKE}, after the account
	 * management
	 * disabled policy has been set,
	 * {@link PolicyUpdateReceiver#onPolicySetResult(Context, String,
	 * Bundle, TargetUser, PolicyUpdateResult)} will notify the admin on whether the
	 * policy was
	 * successfully set or not. This callback will contain:
	 ...
	 * <li>The additional policy params bundle, which contains
	 * {@link PolicyUpdateReceiver#EXTRA_ACCOUNT_TYPE} the account type the policy
	 * applies to
	 * <li>The {@link TargetUser} that this policy relates to
	 * <li>The {@link PolicyUpdateResult}, which will be
	 * {@link PolicyUpdateResult#RESULT_POLICY_SET} if the policy was successfully
	 ...
	 * @param disabled    The boolean indicating that account management will be
	 *                    disabled (true) or
	 *                    enabled (false).
	 * @throws SecurityException if {@code admin} is not a device or profile owner.
	 */
	...
	/**
	 * Gets the array of accounts for which account management is disabled by the
	 * profile owner
	 * or device owner.
	 *
	 * <p>
	 ...
	 * This method may be called on the {@code DevicePolicyManager} instance
	 * returned from
	 * {@link #getParentProfileInstance(ComponentName)}. Note that only a profile
	 * owner on
	 * an organization-owned device can affect account types on the parent profile
	 * instance.
	 *
	 * @return a list of account types for which account management has been
	 *         disabled.
	 *
	 ...
	 *      Note that calling this method on the parent profile instance will return
	 *      the same
	...
	/**
	 * Called by device owner or profile owner to set whether a secondary lockscreen
	 * needs to be
	 * shown.
	 *
	 * <p>
	 * The secondary lockscreen will by displayed after the primary keyguard
	 * security screen
	 * requirements are met. To provide the lockscreen content the DO/PO will need
	 * to provide a
	 * service handling the {@link #ACTION_BIND_SECONDARY_LOCKSCREEN_SERVICE} intent
	 * action,
	 * extending the {@link DevicePolicyKeyguardService} class.
	 *
	 * <p>
	 * Relevant interactions on the secondary lockscreen should be communicated back
	 * to the
	 * keyguard via {@link IKeyguardCallback}, such as when the screen is ready to
	 * be dismissed.
	 *
	 * <p>
	 * This API, and associated APIs, can only be called by the default supervision
	 * app when it
	 ...
	 * <p>
	 * Any packages that share uid with an allowed package will also be allowed to
	 * activate lock
	 * task. From {@link android.os.Build.VERSION_CODES#M} removing packages from
	 * the lock task
	 * package list results in locked tasks belonging to those packages to be
	 * finished.
	 * <p>
	 * This function can only be called by the device owner, a profile owner of an
	 * affiliated user
	 * or profile, or the profile owner when no device owner is set or holders of
	 * the permission
	 * {@link android.Manifest.permission#MANAGE_DEVICE_POLICY_LOCK_TASK}. See
	 * {@link #isAffiliatedUser}.
	 * Any package set via this method will be cleared if the user becomes
	 * unaffiliated.
	 * <p>
	 * Starting from {@link Build.VERSION_CODES#UPSIDE_DOWN_CAKE}, after the lock
	 * task policy has
	 * been set,
	 * {@link PolicyUpdateReceiver#onPolicySetResult(Context, String, Bundle, TargetUser,
	 * PolicyUpdateResult)} will notify the admin on whether the policy was
	 ...
	 * will contain the reason why the policy changed.
	 * <p>
	 * Starting from {@link Build.VERSION_CODES#UPSIDE_DOWN_CAKE}, lock task
	 * features and lock task
	 * packages are bundled as one policy. A failure to apply one will result in a
	 * failure to apply
	 * the other.
	 *
	 ...
	 *                 with. Null if the
	 *                 caller is not a device admin.
	 * @throws SecurityException if {@code admin} is not the device owner, the
	 *                           profile owner of an
	 *                           affiliated user or profile, or the profile owner
	 *                           when no device owner is set or holder of the
	 *                           permission
	 *                           {@link android.Manifest.permission#MANAGE_DEVICE_POLICY_LOCK_TASK}.
	 * @see #isAffiliatedUser
	...
	/**
	 * This function lets the caller know whether the given component is allowed to
	 * start the
	 * lock task mode.
	 * 
	...
	/**
	 * Sets which system features are enabled when the device runs in lock task
	 * mode. This method
	 * doesn't affect the features when lock task mode is inactive. Any system
	 * features not included
	 * in {@code flags} are implicitly disabled when calling this method. By
	 * default, only
	 * {@link #LOCK_TASK_FEATURE_GLOBAL_ACTIONS} is enabled; all the other features
	 * are disabled. To
	 * disable the global actions dialog, call this method omitting
	 * {@link #LOCK_TASK_FEATURE_GLOBAL_ACTIONS}.
	 *
	 * <p>
	 * This method can only be called by the device owner, a profile owner of an
	 * affiliated
	 * user or profile, or the profile owner when no device owner is set or holders
	 * of the
	 * permission
	 * {@link android.Manifest.permission#MANAGE_DEVICE_POLICY_LOCK_TASK}. See
	 * {@link #isAffiliatedUser}.
	 * Any features set using this method are cleared if the user becomes
	 * unaffiliated.
	 * <p>
	 * Starting from {@link Build.VERSION_CODES#UPSIDE_DOWN_CAKE}, after the lock
	 * task features
	 * policy has been set,
	 * {@link PolicyUpdateReceiver#onPolicySetResult(Context, String, Bundle,
	 * TargetUser, PolicyUpdateResult)} will notify the admin on whether the policy
	 * was
	 * successfully set or not. This callback will contain:
	 * <ul>
	 * <li>The policy identifier {@link DevicePolicyIdentifiers#LOCK_TASK_POLICY}
	 ...
	 *              caller is not a device admin.
	 * @return bitfield of flags. See
	 *         {@link #setLockTaskFeatures(ComponentName, int)} for a list.
	 * @throws SecurityException if {@code admin} is not the device owner, the
	 *                           profile owner of an
	 *                           affiliated user or profile, or the profile owner
	 *                           when no device owner is set or holder of the
	 ...
	 * For example, an organization can have a deal/agreement with a carrier that
	 * all of
	 ...
	 * slice on 5G networks. For devices on 4G networks, the profile owner needs to
	 * additionally
	 ...
	 *
	 * <p>
	 * This method enables preferential network service with a default
	 * configuration.
	 * To fine-tune the configuration, use
	 * {@link #setPreferentialNetworkServiceConfigs) instead.
	 * <p>
	 * Before Android version {@link android.os.Build.VERSION_CODES#TIRAMISU}:
	 ...
	 * @throws SecurityException if the caller is not the profile owner or device
	 *                           owner.
	 **/
	 ...
	 * By default, preferential network service is disabled on the work profile and
	 * fully managed
	 * devices, on supported carriers and devices. Admins can explicitly enable it
	 * with this API.
	 * If admin wants to have multiple enterprise slices,
	 * it can be configured by passing list of
	 * {@link PreferentialNetworkServiceConfig} objects.
	 *
	 ...
	 * <p>
	 * Called by device owner to update {@link android.provider.Settings.Global}
	 * settings.
	 * Validation that the value of the setting is in the correct form for the
	 * setting type should
	 ...
	 * {@link UserManager#DISALLOW_DEBUGGING_FEATURES} instead to restrict users
	 * from enabling
	 ...
	 * <li>{@link android.provider.Settings.Global#STAY_ON_WHILE_PLUGGED_IN} This
	 * setting is only
	 * available from {@link android.os.Build.VERSION_CODES#M} onwards and can only
	 * be set if
	 ...
	 * This
	 * setting is only available from {@link android.os.Build.VERSION_CODES#M}
	 * onwards.</li>
	 * </ul>
	 * <p>
	 * The following settings used to be supported, but can be controlled in other
	 * ways:
	 * <ul>
	 * <li>{@link android.provider.Settings.Global#AUTO_TIME} : Use
	 * {@link #setAutoTimeEnabled} and
	 * {@link UserManager#DISALLOW_CONFIG_DATE_TIME} instead.</li>
	 * <li>{@link android.provider.Settings.Global#AUTO_TIME_ZONE} : Use
	 * {@link #setAutoTimeZoneEnabled} and
	 * {@link UserManager#DISALLOW_CONFIG_DATE_TIME}
	 * instead.</li>
	 ...
	 * </ul>
	 * <p>
	 * Changing the following settings has no effect as of
	 * {@link android.os.Build.VERSION_CODES#M}:
	 * <ul>
	 ...
	 * <li>{@link android.provider.Settings.Global#WIFI_SLEEP_POLICY}. No longer has
	 * effect.</li>
	 * </ul>
	 *
	 * @param admin   Which {@link DeviceAdminReceiver} this request is associated
	 ...
	 * settings. Validation that the value of the setting is in the correct form for
	 * the setting
	 * type should be performed by the caller.
	 * <p>
	 * The settings that can be updated by a device owner or profile owner of
	 * secondary user with
	 * this method are:
	 * <ul>
	 ...
	 * profile to
	 * control whether the user can change networks configured by the admin. When
	 * this lockdown is
	 * enabled, the user can still configure and connect to other Wi-Fi networks, or
	 * use other Wi-Fi
	 ...
	 * and calling
	 * this API effectively modifies the global settings. Previously device owners
	 * can also
	 * control this directly via {@link #setGlobalSetting} but they are recommended
	 * to switch
	 * to this API.
	 *
	 * @param admin    admin Which {@link DeviceAdminReceiver} this request is
	 *                 associated
	 *                 with. Null if the caller is not a device admin.
	 * @param lockdown Whether the admin configured networks should be unmodifiable
	 *                 by the
	 *                 user.
	 * @throws SecurityException if caller is not a device owner or a profile owner
	 *                           of an
	 *                           organization-owned managed profile.
	 */
	 ...
	 * Called by a device owner or a profile owner of an organization-owned managed
	 * profile to
	 * determine whether the user is prevented from modifying networks configured by
	 * the admin.
	 *
	 * @param admin admin Which {@link DeviceAdminReceiver} this request is
	 *              associated
	...
	/**
	 * Called by a device owner or a profile owner of an organization-owned managed
	 * profile to set the system wall clock time. This only takes effect if called
	 * when
	 * {@link android.provider.Settings.Global#AUTO_TIME} is 0, otherwise
	 * {@code false}
	 * will be returned.
	 *
	 * @param admin  Which {@link DeviceAdminReceiver} this request is associated
	 ...
	 * @throws SecurityException if {@code admin} is not a device owner or a profile
	 *                           owner
	 *                           of an organization-owned managed profile.
	 */
	@RequiresPermission(value = SET_TIME, conditional = true)
	 ...
	 * type should be performed by the caller.
	 * <p>
	 * The settings that can be updated by a profile or device owner with this
	 * method are:
	 * <ul>
	 ...
	 * <ul>
	 * <li>{@link android.provider.Settings.Secure#LOCATION_MODE}, but see note
	 * below.</li>
	 * </ul>
	 *
	 * <strong>Note: Starting from Android O, apps should no longer call this method
	 * with the
	 * setting {@link android.provider.Settings.Secure#INSTALL_NON_MARKET_APPS},
	 * which is
	 * deprecated. Instead, device owners or profile owners should use the
	 * restriction
	 * {@link UserManager#DISALLOW_INSTALL_UNKNOWN_SOURCES}.
	 * If any app targeting {@link android.os.Build.VERSION_CODES#O} or higher calls
	 * this method
	 ...
	 * {@link UserManager#DISALLOW_INSTALL_UNKNOWN_SOURCES_GLOBALLY} to restrict
	 * unknown sources for
	 * all users.
	 * </strong>
	 *
	 * <strong>Note: Starting from Android R, apps should no longer call this method
	 * with the
	 * setting {@link android.provider.Settings.Secure#LOCATION_MODE}, which is
	 * deprecated. Instead,
	 * device owners should call
	 * {@link #setLocationEnabled(ComponentName, boolean)}. This will be
	...
	/**
	 * Designates a specific service component as the provider for making permission
	 * requests of a
	 ...
	 *                 {@link RestrictionsReceiver}. If this param is null, it
	 ...
	 * @param admin Which {@link DeviceAdminReceiver} this request is associated
	 *              with.
	 * @param on    {@code true} to mute global volume, {@code false} to turn mute
	 *              off.
	 * @throws SecurityException if {@code admin} is not a device or profile owner.
	 */
	...
	/**
	 * Called by profile or device owners to check whether the global volume mute is
	 * on or off.
	 *
	 * @param admin Which {@link DeviceAdminReceiver} this request is associated
	 *              with.
	...
	/**
	 * Change whether a user can uninstall a package. This function can be called by
	 * a device owner,
	 * profile owner, or by a delegate given the {@link #DELEGATION_BLOCK_UNINSTALL}
	 * scope via
	 * {@link #setDelegatedScopes} or holders of the permission
	 * {@link android.Manifest.permission#MANAGE_DEVICE_POLICY_APPS_CONTROL}.
	 * <p>
	 * Starting from {@link Build.VERSION_CODES#UPSIDE_DOWN_CAKE}, after the set
	 * uninstall blocked
	 * policy has been set,
	 * {@link PolicyUpdateReceiver#onPolicySetResult(Context, String,
	 * Bundle, TargetUser, PolicyUpdateResult)} will notify the admin on whether the
	 ...
	 * @param uninstallBlocked true if the user shouldn't be able to uninstall the
	 *                         package.
	 * @throws SecurityException if {@code admin} is not a device or profile owner
	 *                           or holder of the
	 *                           permission
	...
	/**
	 * Check whether the user has been blocked by device policy from uninstalling a
	 * package.
	 * Requires the caller to be the profile owner if checking a specific admin's
	 * policy.
	 * <p>
	 * <strong>Note:</strong> Starting from
	 * {@link android.os.Build.VERSION_CODES#LOLLIPOP_MR1}, the
	 * behavior of this API is changed such that passing {@code null} as the
	 * {@code admin} parameter
	 * will return if any admin has blocked the uninstallation. Before L MR1,
	 * passing {@code null}
	 ...
	 * <a href="/training/basics/intents/package-visibility">manage package
	 * visibility</a>.
	 * <p>
	 * Starting from {@link Build.VERSION_CODES#UPSIDE_DOWN_CAKE}, the returned
	 * policy will be the
	 * current resolved policy rather than the policy set by the calling admin.
	 *
	 * @param admin       The name of the admin component whose blocking policy will
	 *                    be checked, or
	 *                    {@code null} to check whether any admin has blocked the
	 *                    uninstallation. Starting
	 *                    from
	 *                    {@link android.os.Build.VERSION_CODES#UPSIDE_DOWN_CAKE}
	 *                    admin will be
	 *                    ignored and assumed {@code null}.
	 * @param packageName package to check.
	 * @return true if uninstallation is blocked and the given package is visible to
	 *         you, false
	 *         otherwise if uninstallation isn't blocked or the given package isn't
	 *         visible to you.
	 * @throws SecurityException if {@code admin} is not a device or profile owner.
	 */
	...
	/**
	 * Called by the profile owner of a managed profile or a holder of the
	 * permission
	 ...
	 * As a result the
	 * user will be able to add widgets from the allowlisted package running under
	 * the profile to a
	 * widget host which runs under the parent profile, for example the home screen.
	 * Note that a
	 * package may have zero or more provider components, where each component
	 ...
	 * @throws SecurityException if {@code admin} is not a profile owner and not a
	 *                           holder of the
	 *                           permission
	 *                           {@link android.Manifest.permission#MANAGE_DEVICE_POLICY_PROFILE_INTERACTION}.
	...
	/**
	 * Called by device owners or profile owners of an organization-owned managed
	 * profile to to set
	 * a local system update policy. When a new policy is set,
	 * {@link #ACTION_SYSTEM_UPDATE_POLICY_CHANGED} is broadcast.
	 * <p>
	 * If the supplied system update policy has freeze periods set but the freeze
	 * periods do not
	 * meet 90-day maximum length or 60-day minimum separation requirement set out
	 * in
	 * {@link SystemUpdatePolicy#setFreezePeriods},
	 * {@link SystemUpdatePolicy.ValidationFailedException} will the thrown. Note
	 * that the system
	 * keeps a record of freeze periods the device experienced previously, and
	 * combines them with
	 * the new freeze periods to be set when checking the maximum freeze length and
	 * minimum freeze
	 * separation constraints. As a result, freeze periods that passed validation
	 * during
	 * {@link SystemUpdatePolicy#setFreezePeriods} might fail the additional checks
	 * here due to
	 * the freeze period history. If this is causing issues during development,
	 * {@code adb shell dpm clear-freeze-period-record} can be used to clear the
	 * record.
	 *
	 * @param admin  Which {@link DeviceAdminReceiver} this request is associated
	 *               with. All
	 *               components in the package can set system update policies and
	 *               the most
	 *               recent policy takes effect. This should be null if the caller
	 *               is not a device
	 *               admin.
	 * @param policy the new policy, or {@code null} to clear the current policy.
	 * @throws SecurityException                            if {@code admin} is not
	 *                                                      a device owner or a
	 *                                                      profile owner of an
	 *                                                      organization-owned
	 ...
	 *                                                      to set this policy
	 * @throws IllegalArgumentException                     if the policy type or
	 *                                                      maintenance window is
	 *                                                      not valid.
	 * @throws SystemUpdatePolicy.ValidationFailedException if the policy's freeze
	 *                                                      period does not
	 ...
	 * <p>
	 * Setting the keyguard to disabled has the same effect as choosing "None" as
	 * the screen lock
	 * type. However, this call has no effect if a password, pin or pattern is
	 * currently set. If a
	 * password, pin or pattern is set after the keyguard was disabled, the keyguard
	 * stops being
	 * disabled.
	 *
	 * <p>
	 ...
	 * @param admin    Which {@link DeviceAdminReceiver} this request is associated
	 *                 with.
	 * @param disabled {@code true} disables the keyguard, {@code false} reenables
	 *                 it.
	 * @return {@code false} if attempting to disable the keyguard while a lock
	 *         password was in
	 *         place. {@code true} otherwise.
	 * @throws SecurityException if {@code admin} is not the device owner, or a
	 *                           profile owner of
	...
	/**
	 * Called by device owner or profile owner of secondary users that is affiliated
	 * with the
	 * device to disable the status bar. Disabling the status bar blocks
	 * notifications and quick
	 * settings.
	 * <p>
	 * <strong>Note:</strong> This method has no effect for LockTask mode. The
	 * behavior of the
	 * status bar in LockTask mode can be configured with
	 * {@link #setLockTaskFeatures(ComponentName, int)}. Calls to this method when
	 * the device is in
	 * LockTask mode will be registered, but will only take effect when the device
	 * leaves LockTask
	 * mode.
	 *
	 * <p>
	 * This policy does not have any effect while on the lock screen, where the
	 * status bar will
	 ...
	 *                 with. Null if the
	 *                 caller is not a device admin.
	 * @param disabled {@code true} disables the status bar, {@code false} reenables
	 *                 it.
	 ...
	 * {@link #setStatusBarDisabled}.
	 *
	 * <p>
	 * Callable by device owner or profile owner of secondary users that is
	 * affiliated with the
	 * device owner.
	 *
	 * <p>
	 * This policy has no effect in LockTask mode. The behavior of the
	 * status bar in LockTask mode can be configured with
	 * {@link #setLockTaskFeatures(ComponentName, int)}.
	 *
	 * <p>
	 * This policy also does not have any effect while on the lock screen, where the
	 * status bar
	 * will not be disabled.
	 *
	 * @throws SecurityException if the caller is not the device owner, or a profile
	 *                           owner of
	 *                           secondary user that is affiliated with the device.
	 * @see #isAffiliatedUser
	 * @see #getSecondaryUsers
	...
	/**
	 * Called by the system update service to notify device and profile owners of
	 * pending system
	 * updates.
	 *
	 ...
	 *
	 * @param updateReceivedTime The time as given by
	 *                           {@link System#currentTimeMillis()}
	 *                           indicating when the current pending update was
	 *                           first available. {@code -1} if no
	 *                           update is available.
	 ...
	 * {@link #notifyPendingSystemUpdate(long)}
	 ...
	 *                           first available. {@code -1} if no
	 *                           update is available.
	 * @param isSecurityPatch    {@code true} if this system update is purely a
	 *                           security patch;
	 *                           {@code false} if not.
	...
	/**
	 * Called by device or profile owners to get information about a pending system
	 * update.
	 *
	 * @param admin Which profile or device owner this request is associated with.
	 * @return Information about a pending system update or {@code null} if no
	 *         update pending.
	 * @throws SecurityException if {@code admin} is not a device or profile owner.
	...
	/**
	 * Set the default response for future runtime permission requests by
	 * applications. This
	 * function can be called by a device owner, profile owner, or by a delegate
	 * given the
	 * {@link #DELEGATION_PERMISSION_GRANT} scope via {@link #setDelegatedScopes}.
	 * The policy can allow for normal operation which prompts the user to grant a
	 * permission, or
	 * can allow automatic granting or denying of runtime permission requests by an
	 * application.
	 * This also applies to new permissions declared by app updates. When a
	 * permission is denied or
	 * granted this way, the effect is equivalent to setting the permission * grant
	 * state via
	 * {@link #setPermissionGrantState}.
	 * <p/>
	 * As this policy only acts on runtime permission requests, it only applies to
	 * applications
	 * built with a {@code targetSdkVersion} of
	 * {@link android.os.Build.VERSION_CODES#M} or later.
	 *
	 * <p>
	 * NOTE: On devices running {@link android.os.Build.VERSION_CODES#S} and above,
	 * an auto-grant
	 * policy will not apply to certain sensors-related permissions on some
	 * configurations.
	 * See {@link #setPermissionGrantState(ComponentName, String, String, int)} for
	 * the list of
	 * permissions affected, and the behavior change for managed profiles and
	 * fully-managed
	 * devices.
	 *
	 * @param admin  Which profile or device owner this request is associated with.
	 * @param policy One of the policy constants {@link #PERMISSION_POLICY_PROMPT},
	 *               {@link #PERMISSION_POLICY_AUTO_GRANT} and
	 *               {@link #PERMISSION_POLICY_AUTO_DENY}.
	 * @throws SecurityException if {@code admin} is not a device or profile owner.
	...
	/**
	 * Returns the current runtime permission policy set by the device or profile
	 * owner. The
	...
	/**
	 * Sets the grant state of a runtime permission for a specific application. The
	 * state can be
	 * {@link #PERMISSION_GRANT_STATE_DEFAULT default} in which a user can manage it
	 * through the UI,
	 * {@link #PERMISSION_GRANT_STATE_DENIED denied}, in which the permission is
	 * denied and the user
	 * cannot manage it through the UI, and {@link #PERMISSION_GRANT_STATE_GRANTED
	 * granted} in which
	 * the permission is granted and the user cannot manage it through the UI. This
	 * method can only
	 * be called by a profile owner, device owner, or a delegate given the
	 * {@link #DELEGATION_PERMISSION_GRANT} scope via {@link #setDelegatedScopes}.
	 * <p/>
	 * Note that user cannot manage other permissions in the affected group through
	 * the UI
	 * either and their granted state will be kept as the current value. Thus, it's
	 * recommended that
	 * you set the grant state of all the permissions in the affected group.
	 * <p/>
	 * Setting the grant state to {@link #PERMISSION_GRANT_STATE_DEFAULT default}
	 * does not revoke
	 * the permission. It retains the previous grant, if any.
	 * <p/>
	 * Device admins with a {@code targetSdkVersion} &lt;
	 * {@link android.os.Build.VERSION_CODES#Q}
	 * cannot grant and revoke permissions for applications built with a
	 * {@code targetSdkVersion}
	 * &lt; {@link android.os.Build.VERSION_CODES#M}.
	 * <p/>
	 * Admins with a {@code targetSdkVersion} &ge;
	 * {@link android.os.Build.VERSION_CODES#Q} can
	 * grant and revoke permissions of all apps. Similar to the user revoking a
	 * permission from a
	 * application built with a {@code targetSdkVersion} &lt;
	 * {@link android.os.Build.VERSION_CODES#M} the app-op matching the permission
	 * is set to
	 * {@link android.app.AppOpsManager#MODE_IGNORED}, but the permission stays
	 * granted.
	 * <p>
	 * NOTE: On devices running {@link android.os.Build.VERSION_CODES#S} and above,
	 * control over
	 ...
	 * </ul>
	 * <p>
	 * A profile owner may not grant these permissions (i.e. call this method with
	 * any of the
	 * permissions listed above and {@code grantState} of
	 * {@code #PERMISSION_GRANT_STATE_GRANTED}),
	 * but may deny them.
	 * <p>
	 * A device owner, by default, may continue granting these permissions. However,
	 * for increased
	 * user control, the admin may opt out of controlling grants for these
	 * permissions by including
	 * {@link #EXTRA_PROVISIONING_SENSORS_PERMISSION_GRANT_OPT_OUT} in the
	 * provisioning parameters.
	 * In that case the device owner's control will be limited do denying these
	 * permissions.
	 * <p>
	 * NOTE: On devices running {@link android.os.Build.VERSION_CODES#S} and above,
	 * control over
	 ...
	 * </ul>
	 * <p>
	 * A managed profile owner may not grant these permissions (i.e. call this
	 * method with any of
	 * the permissions listed above and {@code grantState} of
	 * {@code #PERMISSION_GRANT_STATE_GRANTED}), but may deny them.
	 * <p>
	 * Attempts by the admin to grant these permissions, when the admin is
	 * restricted from doing
	 * so, will be silently ignored (no exception will be thrown).
	 *
	 * Control over the following permissions are restricted for managed profile
	 * owners:
	 * <ul>
	 * <li>Manifest.permission.READ_SMS</li>
	 * </ul>
	 ...
	 *                    {@link #PERMISSION_GRANT_STATE_DENIED},
	 *                    {@link #PERMISSION_GRANT_STATE_DEFAULT},
	 *                    {@link #PERMISSION_GRANT_STATE_GRANTED},
	...
	/**
	 * Returns the current grant state of a runtime permission for a specific
	 * application. This
	 * function can be called by a device owner, profile owner, or by a delegate
	 * given the
	 * {@link #DELEGATION_PERMISSION_GRANT} scope via {@link #setDelegatedScopes}.
	 ...
	 * @return the current grant state specified by device policy. If admins have
	 *         not set a grant
	 *         has not set a grant state, the return value is
	 *         {@link #PERMISSION_GRANT_STATE_DEFAULT}. This does not indicate
	 *         whether or not the
	 *         permission is currently granted for the package.
	 *         <p/>
	 *         If a grant state was set by the profile or device owner, then the
	 *         return value will
	 *         be one of {@link #PERMISSION_GRANT_STATE_DENIED} or
	 *         {@link #PERMISSION_GRANT_STATE_GRANTED}, which indicates if the
	 *         permission is
	...
	/**
	 * Returns whether it is possible for the caller to initiate provisioning of a
	 * managed profile
	 ...
	 * @param action      One of {@link #ACTION_PROVISION_MANAGED_DEVICE},
	 *                    {@link #ACTION_PROVISION_MANAGED_PROFILE}
	 * @param packageName The package of the component that would be set as device,
	 *                    user, or profile
	 *                    owner.
	...
	/**
	 * Return if this user is a managed profile of another user. An admin can become
	 * the profile
	 * owner of a managed profile with {@link #ACTION_PROVISION_MANAGED_PROFILE} and
	 * of a managed
	...
	/**
	 * Called by a device owner or profile owner on organization-owned device to get
	 * the MAC
	 * address of the Wi-Fi device.
	 *
	 * NOTE: The MAC address returned here should only be used for inventory
	 * management and is
	 * not likely to be the MAC address used by the device to connect to Wi-Fi
	 * networks: MAC
	 * addresses used for scanning and connecting to Wi-Fi networks are randomized
	 * by default.
	 * To get the randomized MAC address used, call
	 * {@link android.net.wifi.WifiConfiguration#getRandomizedMacAddress}.
	 *
	 * @param admin Which admin this request is associated with. Null if the caller
	 *              is not a device
	 *              admin
	 * @return the MAC address of the Wi-Fi device, or null when the information is
	 *         not available.
	 *         (For example, Wi-Fi hasn't been enabled, or the device doesn't
	 *         support Wi-Fi.)
	 *         <p>
	 *         The address will be in the {@code XX:XX:XX:XX:XX:XX} format.
	 * @throws SecurityException if {@code admin} is not permitted to get wifi mac
	 *                           addresses
	 */
	// @RequiresPermission(value = MANAGE_DEVICE_POLICY_WIFI, conditional = true)
	...
	/**
	 * Called by device owner to reboot the device. If there is an ongoing call on
	 * the device,
	...
	/**
	 * Called by a device admin to set the short support message. This will be
	 * displayed to the user
	 * in settings screens where functionality has been disabled by the admin. The
	 * message should be
	 * limited to a short statement such as "This setting is disabled by your
	 * administrator. Contact
	 * someone@example.com for support." If the message is longer than 200
	 * characters it may be
	 * truncated.
	 * <p>
	 * If the short support message needs to be localized, it is the responsibility
	 * of the
	 * {@link DeviceAdminReceiver} to listen to the
	 * {@link Intent#ACTION_LOCALE_CHANGED} broadcast
	 * and set a new version of this string accordingly.
	 ...
	 *                with. Null if the
	 *                caller is not a device admin.
	 * @param message Short message to be displayed to the user in settings or null
	 *                to clear the
	 *                existing message.
	 * @throws SecurityException if {@code admin} is not an active administrator.
	 */
	 ...
	 * {@link android.Manifest.permission#MANAGE_DEVICE_POLICY_SUPPORT_MESSAGE} to
	 * get the short
	 * support message.
	 *
	 * @param admin Which {@link DeviceAdminReceiver} this request is associated
	 ...
	 *         {@link #setShortSupportMessage(ComponentName, CharSequence)} or
	 *         null if no message has been set.
	 * @throws SecurityException if {@code admin} is not an active administrator and
	 *                           not a holder of
	 *                           the permission
	...
	/**
	 * Called by a device admin to set the long support message. This will be
	 * displayed to the user
	 * in the device administrators settings screen. If the message is longer than
	 * 20000 characters
	 * it may be truncated.
	 * <p>
	 * If the long support message needs to be localized, it is the responsibility
	 * of the
	 * {@link DeviceAdminReceiver} to listen to the
	 * {@link Intent#ACTION_LOCALE_CHANGED} broadcast
	 * and set a new version of this string accordingly.
	 ...
	 * @param admin   Which {@link DeviceAdminReceiver} this request is associated
	 *                with.
	 * @param message Long message to be displayed to the user in settings or null
	 *                to clear the
	 *                existing message.
	 * @throws SecurityException if {@code admin} is not an active administrator.
	 */
	 ...
	 *              with.
	 * @return The message set by
	 *         {@link #setLongSupportMessage(ComponentName, CharSequence)} or
	 *         null if no message has been set.
	 * @throws SecurityException if {@code admin} is not an active administrator.
	 */
	 ...
	 * @param userHandle user id the admin is running as.
	 * @return The message set by
	 *         {@link #setShortSupportMessage(ComponentName, CharSequence)}
	 *
	 * @hide
	 */
	 ...
	 *
	 * <p>
	 * The following methods are supported for the parent instance, all other
	 * methods will
	 ...
	 * </ul>
	 * <p>
	 * The following methods are supported for the parent instance but can only be
	 * called by the
	 * profile owner of a managed profile that was created during the device
	 * provisioning flow:
	 * <ul>
	 ...
	 * </ul>
	 *
	 * @return a new instance of {@link DevicePolicyManager} that acts on the parent
	 *         profile.
	 * @throws SecurityException if {@code admin} is not a profile owner.
	 */
	...
	/**
	 * Called by device owner or a profile owner of an organization-owned managed
	 * profile to
	 * control the security logging feature.
	 *
	 * <p>
	 * Security logs contain various information intended for security auditing
	 * purposes.
	 * When security logging is enabled by any app other than the device owner,
	 * certain security
	 * logs are not visible (for example personal app launch events) or they will be
	 * redacted
	 * (for example, details of the physical volume mount events).
	 * Please see {@link SecurityEvent} for details.
	 *
	 * <p>
	 * <strong>Note:</strong> The device owner won't be able to retrieve security
	 * logs if there
	 * are unaffiliated secondary users or profiles on the device, regardless of
	 * whether the
	 * feature is enabled. Logs will be discarded if the internal buffer fills up
	 * while waiting for
	 * all users to become affiliated. Therefore it's recommended that affiliation
	 * ids are set for
	 * new users as soon as possible after provisioning via
	 * {@link #setAffiliationIds}. Non device
	 * owners are not subject to this restriction since all
	 * privacy-sensitive events happening outside the managed profile would have
	 * been redacted
	 * already.
	 *
	 * @param admin   Which device admin this request is associated with, or
	 *                {@code null}
	 *                if called by a delegated app.
	 * @param enabled whether security logging should be enabled or not.
	 * @throws SecurityException if the caller is not permitted to control security
	 *                           logging.
	 ...
	 *
	 * <p>
	 * Can only be called by the device owner or a profile owner of an
	 * organization-owned
	 * managed profile, otherwise a {@link SecurityException} will be thrown.
	 *
	 * @param admin Which device admin this request is associated with. Null if the
	 *              caller is not
	 *              a device admin
	 * @return {@code true} if security logging is enabled, {@code false} otherwise.
	 * @throws SecurityException if the caller is not allowed to control security
	 *                           logging.
	 */
	@RequiresPermission(value = MANAGE_DEVICE_POLICY_SECURITY_LOGGING, conditional = true)
	...
	/**
	 * Called by device owner or profile owner of an organization-owned managed
	 * profile to retrieve
	 * all new security logging entries since the last call to this API after device
	 * boots.
	 *
	 * <p>
	 * Access to the logs is rate limited and it will only return new logs after the
	 * admin has
	 * been notified via {@link DeviceAdminReceiver#onSecurityLogsAvailable}.
	 *
	 * <p>
	 * When called by a device owner, if there is any other user or profile on the
	 * device,
	 * it must be affiliated with the device. Otherwise a {@link SecurityException}
	 * will be thrown.
	 ...
	 *              {@code null}
	 *              if called by a delegated app.
	 * @return the new batch of security logs which is a list of
	 *         {@link SecurityEvent},
	 *         or {@code null} if rate limitation is exceeded or if logging is
	 *         currently disabled.
	 * @throws SecurityException if the caller is not allowed to access security
	 *                           logging,
	 *                           or there is at least one profile or secondary user
	...
	/**
	 * Forces a batch of security logs to be fetched from logd and makes it
	 * available for DPC.
	 * If throttled, returns time to wait in milliseconds, otherwise 0.
	 * 
	 * @hide
	...
	/**
	 * Called by the system to obtain a {@link DevicePolicyManager} whose calls act
	 * on the parent
	 * profile.
	 *
	 * @hide
	...
	/**
	 * Called by a device or profile owner to restrict packages from using metered
	 * data.
	 *
	 * @param admin        which {@link DeviceAdminReceiver} this request is
	 *                     associated with.
	...
	/**
	 * Called by a device or profile owner to retrieve the list of packages which
	 * are restricted
	...
	/**
	 * Called by the system to check if a package is restricted from using metered
	 * data
	 * by {@param admin}.
	 *
	 * @param admin       which {@link DeviceAdminReceiver} this request is
	 ...
	 * @return {@code true} if the package is restricted by admin, otherwise
	 *         {@code false}
	 * @throws SecurityException if the caller doesn't run with
	 ...
	 * <p>
	 * <strong> This API is not supported on all devices. Calling this API on
	 * unsupported devices
	 * will result in {@code null} being returned. The device logs are retrieved
	 * from a RAM region
	 * which is not guaranteed to be corruption-free during power cycles, as a
	 * result be cautious
	 ...
	 *              {@code null}
	 *              if called by a delegated app.
	 * @return Device logs from before the latest reboot of the system, or
	 *         {@code null} if this API
	 *         is not supported on the device.
	 * @throws SecurityException if the caller is not allowed to access security
	 *                           logging, or
	 *                           there is at least one profile or secondary user
	 *                           that is not affiliated with the device.
	 * @see #isAffiliatedUser
	 * @see #retrieveSecurityLogs
	 */
	...
	/**
	 * Called by a profile owner of a managed profile to set the color used for
	 * customization. This
	 * color is used as background color of the confirm credentials screen for that
	 * user. The
	 ...
	 * <p>
	 * Starting from Android R, the organization color will no longer be used as the
	 * background
	 ...
	 * @throws SecurityException if {@code admin} is not a profile owner.
	 * @deprecated From {@link android.os.Build.VERSION_CODES#R}, the organization
	 *             color is never
	...
	/**
	 * Called by a profile owner of a managed profile to retrieve the color used for
	 * customization.
	 * This color is used as background color of the confirm credentials screen for
	 * that user.
	 *
	 * @param admin Which {@link DeviceAdminReceiver} this request is associated
	 *              with.
	...
	/**
	 * Called by the device owner (since API 26) or profile owner (since API 24) to
	 * set the name of
	 * the organization under management.
	 *
	 * <p>
	 * If the organization name needs to be localized, it is the responsibility of
	 * the caller
	 * to listen to the {@link Intent#ACTION_LOCALE_CHANGED} broadcast and set a new
	 * version of this
	 * string accordingly.
	 *
	 * @param admin Which {@link DeviceAdminReceiver} this request is associated
	...
	/**
	 * Called by the device owner (since API 26) or profile owner (since API 24) or
	 * holders of the
	 * permission
	 ...
	 * @throws SecurityException if {@code admin} if {@code admin} is not a device
	 *                           or profile
	...
	private final IpcDataCache<Void, CharSequence> mGetDeviceOwnerOrganizationNameCache = new IpcDataCache(
			sDpmCaches.child("getDeviceOwnerOrganizationName"),
			(query) -> getService().getDeviceOwnerOrganizationName());

	/**
	 * Called by the system to retrieve the name of the organization managing the
	 * device.
	 *
	 * @return The organization name or {@code null} if none is set.
	 * @throws SecurityException if the caller is not the device owner, does not
	...
	private final IpcDataCache<Integer, CharSequence> mGetOrganizationNameForUserCache = new IpcDataCache<>(
			sDpmCaches.child("getOrganizationNameForUser"),
			(query) -> getService().getOrganizationNameForUser(query));

	/**
	 * Retrieve the default title message used in the confirm credentials screen for
	 * a given user.
	 *
	 * @param userHandle The user id of the user we're interested in.
	 * @return The organization name or {@code null} if none is set.
	...
	/**
	 * @return the {@link UserProvisioningState} for the current user - for
	 *         unmanaged users will
	...
	/**
	 * Set the {@link UserProvisioningState} for the supplied user, if they are
	 * managed.
	 *
	...
	/**
	 * Set the {@link UserProvisioningState} for the supplied user. The supplied
	 * user has to be
	 * manged, otherwise it will throw an {@link IllegalStateException}.
	 *
	 * <p>
	 * For managed users/profiles/devices, only the following state changes are
	 * allowed:
	 * <ul>
	 * <li>{@link #STATE_USER_UNMANAGED} can change to any other state except itself
	 * <li>{@link #STATE_USER_SETUP_INCOMPLETE} and
	 * {@link #STATE_USER_SETUP_COMPLETE} can only
	 ...
	 * <li>{@link #STATE_USER_SETUP_FINALIZED} can't be changed to any other
	 * state</li>
	 ...
	 * affiliated if the set of ids set by the device owner and the admin of the
	 * secondary user.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * <strong>Note:</strong> Features that depend on user affiliation (such as
	 * security logging
	 * or {@link #bindDeviceAdminServiceAsUser}) won't be available when a secondary
	 * user
	 * is created, until it becomes affiliated. Therefore it is recommended that the
	 * appropriate
	 * affiliation ids are set by its owner as soon as possible after the user is
	 * created.
	 * <p>
	 * Note: This method used to be available for affiliating device owner and
	 * profile
	 * owner. However, since Android 11, this combination is not possible. This
	 * method is now
	 * only useful for affiliating the primary user with managed secondary users.
	 *
	 * @param admin Which device owner, or owner of secondary user, this request is
	 *              associated with.
	 * @param ids   A set of opaque non-empty affiliation ids.
	 *
	 * @throws IllegalArgumentException if {@code ids} is null or contains an empty
	 *                                  string.
	 * @see #isAffiliatedUser
	 */
	 ...
	 * with the device.
	 * Any other user is considered affiliated with the device if the set specified
	 * by its
	 * profile owner via {@link #setAffiliationIds} intersects with the device
	 * owner's.
	 * 
	 * @see #setAffiliationIds
	 */
	...
	/**
	 * @hide
	 *       Returns whether the uninstall for {@code packageName} for the current
	 *       user is in queue
	 *       to be started
	...
	/**
	 * @hide
	 *       Remove a test admin synchronously without sending it a broadcast about
	 *       being removed.
	 *       If the admin is a profile owner or device owner it will still be
	 *       removed.
	 *
	 * @param userHandle user id to remove the admin for.
	 * @param admin      The administration compononent to remove.
	 * @throws SecurityException if the caller is not shell / root or the admin
	 *                           package
	 *                           isn't a test application see
	 *                           {@link ApplicationInfo#FLAG_TEST_APP}.
	 */
	@TestApi
	@RequiresPermission(android.Manifest.permission.MANAGE_PROFILE_AND_DEVICE_OWNERS)
	...
	/**
	 * Allows the device owner or profile owner to enable or disable the backup
	 * service.
	 *
	 * <p>
	 * Each user has its own backup service which manages the backup and restore
	 * mechanisms in
	 * that user. Disabling the backup service will prevent data from being backed
	 * up or restored.
	 *
	 * <p>
	 * Device owner calls this API to control backup services across all users on
	 * the device.
	 * Profile owner can use this API to enable or disable the profile's backup
	 * service. However,
	 * for a managed profile its backup functionality is only enabled if both the
	 * device owner
	 * and the profile owner have enabled the backup service.
	 *
	 * <p>
	 * By default, backup service is disabled on a device with device owner, and
	 * within a
	 * managed profile.
	 *
	 * @param admin   Which {@link DeviceAdminReceiver} this request is associated
	 *                with.
	 * @param enabled {@code true} to enable the backup service, {@code false} to
	 *                disable it.
	 * @throws SecurityException if {@code admin} is not a device owner or a profile
	 *                           owner.
	 */
	...
	/**
	 * Return whether the backup service is enabled by the device owner or profile
	 * owner for the
	 * current user, as previously set by
	 * {@link #setBackupServiceEnabled(ComponentName, boolean)}.
	 *
	 * <p>
	 * Whether the backup functionality is actually enabled or not depends on
	 * settings from both
	...
	/**
	 * Called by a device owner, profile owner of a managed profile or delegated app
	 * with
	 * {@link #DELEGATION_NETWORK_LOGGING} to control the network logging feature.
	 *
	 * <p>
	 * Supported for a device owner from Android 8 and a delegated app granted by a
	 * device
	 * owner from Android 10. Supported for a profile owner of a managed profile and
	 * a delegated
	 * app granted by a profile owner from Android 12. When network logging is
	 * enabled by a
	 * profile owner, the network logs will only include work profile network
	 * activity, not
	 * activity on the personal profile.
	 *
	 * <p>
	 * Network logs contain DNS lookup and connect() library call events. The
	 * following library
	 ...
	 *
	 * <p>
	 * Network logging is a low-overhead tool for forensics but it is not guaranteed
	 * to use
	 * full system call logging; event reporting is enabled by default for all
	 * processes but not
	 * strongly enforced.
	 * Events from applications using alternative implementations of libc, making
	 * direct kernel
	 ...
	 * <ul>
	 * <li>Applications may hardcode IP addresses to reduce the number of DNS
	 * lookups, or use
	 * an alternative system for name resolution, and so avoid calling
	 * {@code getaddrinfo()} or {@code gethostbyname}.</li>
	 * <li>Applications may use datagram sockets for performance reasons, for
	 * example
	 ...
	 *
	 * <p>
	 * It is possible to directly intercept layer 3 traffic leaving the device using
	 * an
	 ...
	 *
	 * <p>
	 * <strong>Note:</strong> The device owner won't be able to retrieve network
	 * logs if there
	 * are unaffiliated secondary users or profiles on the device, regardless of
	 * whether the
	 * feature is enabled. Logs will be discarded if the internal buffer fills up
	 ...
	 * @throws SecurityException if {@code admin} is not a device owner or profile
	 *                           owner.
	 * @see #setAffiliationIds
	...
	private IpcDataCache<ComponentName, Boolean> mIsNetworkLoggingEnabledCache = new IpcDataCache<>(
			sDpmCaches.child("isNetworkLoggingEnabled"),
			(admin) -> getService().isNetworkLoggingEnabled(admin,
					getContext().getPackageName()));

	/**
	 * Return whether network logging is enabled by a device owner or profile owner
	 * of
	 * a managed profile.
	 *
	 * @param admin Which {@link DeviceAdminReceiver} this request is associated
	 *              with. Can only
	 *              be {@code null} if the caller is a delegated app with
	 *              {@link #DELEGATION_NETWORK_LOGGING}
	 *              or has MANAGE_USERS permission.
	 * @return {@code true} if network logging is enabled by device owner or profile
	 *         owner,
	 *         {@code false} otherwise.
	 * @throws SecurityException if {@code admin} is not a device owner or profile
	 *                           owner and
	...
	/**
	 * Called by device owner, profile owner of a managed profile or delegated app
	 * with
	 * {@link #DELEGATION_NETWORK_LOGGING} to retrieve the most recent batch of
	 * network logging events.
	 *
	 * <p>
	 * When network logging is enabled by a profile owner, the network logs will
	 * only include
	 * work profile network activity, not activity on the personal profile.
	 *
	 * A device owner or profile owner has to provide a batchToken provided as part
	 * of
	 * {@link DeviceAdminReceiver#onNetworkLogsAvailable} callback. If the token
	 * doesn't match the
	 * token of the most recent available batch of logs, {@code null} will be
	 * returned.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * The list of network events is sorted chronologically, and contains at most
	 * 1200 events.
	 *
	 * <p>
	 * Access to the logs is rate limited and this method will only return a new
	 * batch of logs
	 * after the device device owner has been notified via
	 * {@link DeviceAdminReceiver#onNetworkLogsAvailable}.
	 *
	 * <p>
	 * If the caller is not a profile owner and a secondary user or profile is
	 * created, calling
	 * this method will throw a {@link SecurityException} until all users become
	 * affiliated again.
	 * It will also no longer be possible to retrieve the network logs batch with
	 * the most recent
	 ...
	 * @return A new batch of network logs which is a list of {@link NetworkEvent}.
	 *         Returns
	 *         {@code null} if the batch represented by batchToken is no longer
	 *         available or if
	 *         logging is disabled.
	 * @throws SecurityException if {@code admin} is not a device owner, profile
	 *                           owner or if the
	 *                           {@code admin} is not a profile owner and there is
	 *                           at least one profile or secondary user
	 *                           that is not affiliated with the device.
	 * @see #setAffiliationIds
	...
	/**
	 * Called by a device owner to bind to a service from a secondary managed user
	 * or vice versa.
	 ...
	 * <p>
	 * The service must be protected by
	 * {@link android.Manifest.permission#BIND_DEVICE_ADMIN}.
	 * Note that the {@link Context} used to obtain this
	 * {@link DevicePolicyManager} instance via
	 * {@link Context#getSystemService(Class)} will be used
	 * to bind to the {@link android.app.Service}.
	 * <p>
	 * Note: This method used to be available for communication between device owner
	 * and profile
	 * owner. However, since Android 11, this combination is not possible. This
	 * method is now
	 * only useful for communication between device owner and managed secondary
	 * users.
	 *
	 * @param admin         Which {@link DeviceAdminReceiver} this request is
	 *                      associated with.
	 ...
	 * @param conn          Receives information as the service is started and
	 *                      stopped in main thread. This
	 *                      must be a valid {@link ServiceConnection} object; it
	 *                      must not be {@code null}.
	 * @param flags         Operation options for the binding operation. See
	 *                      {@link Context#bindService(Intent, ServiceConnection, int)}.
	 * @param targetUser    Which user to bind to. Must be one of the users returned
	 *                      by
	 *                      {@link #getBindDeviceAdminTargetUsers}, otherwise a
	 *                      {@link SecurityException} will
	 *                      be thrown.
	 * @return If you have successfully bound to the service, {@code true} is
	 *         returned;
	 *         {@code false} is returned if the connection is not made and you will
	 *         not
	...
	/**
	 * See
	 * {@link #bindDeviceAdminServiceAsUser(ComponentName, Intent, ServiceConnection, int,
	 * UserHandle)}.
	 * Call {@link Context.BindServiceFlags#of(long)} to obtain a BindServiceFlags
	 * object.
	 */
	public boolean bindDeviceAdminServiceAsUser(@NonNull ComponentName admin,
			@NonNull Intent serviceIntent, @NonNull ServiceConnection conn,
	...
	/**
	 * Returns the list of target users that the calling device owner or owner of
	 * secondary user
	 * can use when calling {@link #bindDeviceAdminServiceAsUser}.
	 * <p>
	 * A device owner can bind to a service from a secondary managed user and vice
	 * versa, provided
	...
	/**
	 * Called by the system to get the time at which the device owner last retrieved
	 * security
	 * logging entries.
	 *
	 * @return the time at which the device owner most recently retrieved security
	 *         logging entries,
	 *         in milliseconds since epoch; -1 if security logging entries were
	 *         never retrieved.
	 * @throws SecurityException if the caller is not the device owner, does not
	 *                           hold the
	 *                           MANAGE_USERS permission and is not the system.
	...
	/**
	 * Called by the system to get the time at which the device owner last requested
	 * a bug report.
	 *
	 * @return the time at which the device owner most recently requested a bug
	 *         report, in
	...
	/**
	 * Called by the system to get the time at which the device owner or profile
	 * owner of a
	 * managed profile last retrieved network logging events.
	 *
	 * @return the time at which the device owner or profile owner most recently
	 *         retrieved network
	 *         logging events, in milliseconds since epoch; -1 if network logging
	 *         events were
	 *         never retrieved.
	 * @throws SecurityException if the caller is not the device owner, does not
	 *                           hold the
	...
	/**
	 * Called by the system to find out whether the current user's IME was set by
	 * the device/profile
	 * owner or the user.
	 *
	 * @return {@code true} if the user's IME was set by the device or profile
	 *         owner, {@code false}
	 *         otherwise.
	 * @throws SecurityException if the caller is not the device owner/profile
	 *                           owner.
	 *
	 * @hide
	 */
	...
	/**
	 * Called by the system to get a list of CA certificates that were installed by
	 * the device or
	 * profile owner.
	 *
	 * <p>
	 ...
	 * @return list of aliases identifying CA certificates installed by the device
	 *         or profile owner
	...
	/**
	 * Called by the device owner or profile owner to clear application user data of
	 * a given
	 * package. The behaviour of this is equivalent to the target application
	 * calling
	 * {@link android.app.ActivityManager#clearApplicationUserData()}.
	 *
	 * <p>
	 * <strong>Note:</strong> an application can store data outside of its
	 * application data, e.g.
	 * external storage or user dictionary. This data will not be wiped by calling
	 * this API.
	 *
	 * @param admin       Which {@link DeviceAdminReceiver} this request is
	 *                    associated with.
	 * @param packageName The name of the package which will have its user data
	 *                    wiped.
	 * @param executor    The executor through which the listener should be invoked.
	 * @param listener    A callback object that will inform the caller when the
	 *                    clearing is done.
	 * @throws SecurityException if the caller is not the device owner/profile
	 *                           owner.
	 */
	...
	/**
	 * Called by a device owner to specify whether logout is enabled for all
	 * secondary users. The
	 * system may show a logout button that stops the user and switches back to the
	 * primary user.
	 *
	 * @param admin   Which {@link DeviceAdminReceiver} this request is associated
	 *                with.
	 ...
	 *
	 * @return {@code true} if logout is enabled by device owner, {@code false}
	 *         otherwise.
	 */
	...
	public interface OnClearApplicationUserDataListener {
		 ...
		 * @param succeeded   Whether the clearing succeeded. Clearing fails for device
		 *                    administrator
	 ...
	 * @param provisioningAction action indicating type of provisioning, should be
	 *                           one of
	 *                           {@link #ACTION_PROVISION_MANAGED_DEVICE},
	 *                           {@link #ACTION_PROVISION_MANAGED_PROFILE} or
	 *                           {@link #ACTION_PROVISION_MANAGED_USER}.
	...
	/**
	 * Changes the current administrator to another one. All policies from the
	 * current
	 * administrator are migrated to the new administrator. The whole operation is
	 * atomic -
	 * the transfer is either complete or not done at all.
	 *
	 * <p>
	 * Depending on the current administrator (device owner, profile owner), you
	 * have the
	 ...
	 *
	 * <p>
	 * Use the {@code bundle} parameter to pass data to the new administrator. The
	 * data
	 ...
	 *
	 * <p>
	 * The transfer has failed if the original administrator is still the
	 * corresponding owner
	 ...
	 * <code>&lt;device-admin&gt;&lt;/device-admin&gt;</code> tags in the xml file
	 * referenced by
	 ...
	 * @param admin  which {@link DeviceAdminReceiver} this request is associated
	 *               with.
	 * @param target which {@link DeviceAdminReceiver} we want the new administrator
	 *               to be.
	 * @param bundle data to be sent to the new administrator.
	 * @throws SecurityException        if {@code admin} is not a device owner nor a
	 *                                  profile owner.
	 * @throws IllegalArgumentException if {@code admin} or {@code target} is
	 *                                  {@code null}, they
	 *                                  are components in the same package or
	 *                                  {@code target} is not an active admin.
	 */
	...
	/**
	 * Called by a device owner to specify the user session start message. This may
	 * be displayed
	 ...
	 *
	 * @param admin                   which {@link DeviceAdminReceiver} this request
	 *                                is associated with.
	 * @param startUserSessionMessage message for starting user session, or
	 *                                {@code null} to use
	...
	/**
	 * Called by a device owner to specify the user session end message. This may be
	 * displayed
	 * during a user switch.
	 * <p>
	 * The message should be limited to a short statement or it may be truncated.
	 ...
	 * @param admin                 which {@link DeviceAdminReceiver} this request
	 *                              is associated with.
	 * @param endUserSessionMessage message for ending user session, or {@code null}
	 *                              to use system
	 *                              default message.
	 * @throws SecurityException if {@code admin} is not a device owner.
	 */
	 ...
	 *
	 * <p>
	 * This method may returns {@code -1} if {@code apnSetting} conflicts with an
	 * existing
	 * override APN. Update the existing conflicted APN with
	 * {@link #updateOverrideApn(ComponentName, int, ApnSetting)} instead of adding
	 * a new entry.
	 * <p>
	 * Two override APNs are considered to conflict when all the following APIs
	 * return
	 ...
	 * ({@link ApnSetting#TYPE_ENTERPRISE}), while only device owners can add other
	 * type of APNs.
	 * Enterprise APNs are specific to the managed profile and do not override any
	 * user-configured
	 * VPNs. They are prerequisites for enabling preferential network service on the
	 * managed
	 ...
	 * @return The {@code id} of inserted override APN. Or {@code -1} when failed to
	 *         insert into
	 *         the database.
	 * @throws SecurityException If request is for enterprise APN {@code admin} is
	 *                           either device
	 *                           owner or profile owner and in all other types of
	 *                           APN if {@code admin} is not a device owner.
	 *
	 ...
	 *
	 * <p>
	 * This method may returns {@code false} if there is no override APN with the
	 * given
	 * {@code apnId}.
	 * <p>
	 * This method may also returns {@code false} if {@code apnSetting} conflicts
	 * with an
	 ...
	 * ({@link ApnSetting#TYPE_ENTERPRISE}), while only device owners can update
	 * other type of APNs.
	 *
	 * @param admin      which {@link DeviceAdminReceiver} this request is
	 *                   associated with
	 ...
	 * ({@link ApnSetting#TYPE_ENTERPRISE}), while only device owners can remove
	 * other type of APNs.
	 *
	 * @param admin which {@link DeviceAdminReceiver} this request is associated
	 *              with
	 * @param apnId the {@code id} of the override APN to remove
	 * @return {@code true} if the required override APN is successfully removed,
	 *         {@code false}
	 *         otherwise.
	 * @throws SecurityException If request is for enterprise APN {@code admin} is
	 *                           either device
	...
	/**
	 * Called by device owner or managed profile owner to get all override APNs
	 * inserted by
	 ...
	 * <p>
	 * Override APNs are separated from other APNs on the device, and can only be
	 * inserted or
	 * modified by the device owner. When enabled, only override APNs are in use,
	 * any other APNs
	 * are ignored.
	 * <p>
	 * Note: Enterprise APNs added by managed profile owners do not need to be
	 * enabled by
	 * this API. They are part of the preferential network service config and is
	 * controlled by
	 * {@link #setPreferentialNetworkServiceConfigs}.
	 *
	 * @param admin   which {@link DeviceAdminReceiver} this request is associated
	 *                with
	 * @param enabled {@code true} if override APNs should be enabled, {@code false}
	 *                otherwise
	 * @throws SecurityException if {@code admin} is not a device owner.
	 */
	 ...
	 * @param admin which {@link DeviceAdminReceiver} this request is associated
	 *              with
	 * @return {@code true} if override APNs are currently enabled, {@code false}
	 *         otherwise.
	 * @throws SecurityException if {@code admin} is not a device owner.
	 *
	 * @see #setOverrideApnsEnabled(ComponentName, boolean)
	...
	/**
	 * Returns the data passed from the current administrator to the new
	 * administrator during an
	 ...
	 * {@link DeviceAdminReceiver#onTransferOwnershipComplete(Context, PersistableBundle)}.
	 * Use this method to retrieve it after the transfer as long as the new
	 * administrator is the
	 * active device or profile owner.
	 *
	 * <p>
	 * Returns <code>null</code> if no ownership transfer was started for the
	 * calling user.
	 *
	 * @see #transferOwnership
	 * @see DeviceAdminReceiver#onTransferOwnershipComplete(Context,
	 *      PersistableBundle)
	 ...
	 *
	 * <p>
	 * In this mode, the DNS subsystem will attempt a TLS handshake to the
	 * network-supplied
	 * resolver prior to attempting name resolution in cleartext.
	 *
	 * <p>
	 * Note: The device owner won't be able to set the global private DNS mode if
	 * there are
	 * unaffiliated secondary users or profiles on the device. It's recommended that
	 * affiliation
	 ...
	 *
	 * <p>
	 * Note that the method is blocking as it will perform a connectivity check to
	 * the resolver,
	 * to ensure it is valid. Because of that, the method should not be called on
	 * any thread that
	 * relates to user interaction, such as the UI thread.
	 *
	 * <p>
	 * In case a VPN is used in conjunction with Private DNS resolver, the Private
	 * DNS resolver
	 * must be reachable both from within and outside the VPN. Otherwise, the device
	 * may lose
	 * the ability to resolve hostnames as system traffic to the resolver may not go
	 * through the
	 * VPN.
	 *
	 * <p>
	 ...
	 * @param admin          which {@link DeviceAdminReceiver} this request is
	 *                       associated with.
	 * @param privateDnsHost The hostname of a server that implements DNS over TLS
	 *                       (RFC7858).
	 *
	 * @return {@code PRIVATE_DNS_SET_NO_ERROR} if the mode was set successfully,
	 *         {@code PRIVATE_DNS_SET_ERROR_FAILURE_SETTING} if it could not be set
	 *         or
	 *         {@code PRIVATE_DNS_SET_ERROR_HOST_NOT_SERVING} if the specified host
	 *         does not
	 *         implement RFC7858.
	 *
	 * @throws IllegalArgumentException if the {@code privateDnsHost} is not a valid
	 *                                  hostname.
	 *
	 * @throws SecurityException        if the caller is not the device owner.
	 */
	@WorkerThread
	public @PrivateDnsModeErrorCodes int setGlobalPrivateDnsModeSpecifiedHost(
	 ...
	 * rebooted in order to finish installing the update. Note that if the device is
	 * rebooted, this
	 * doesn't necessarily mean that the update has been applied successfully. The
	 * caller should
	 * additionally check the system version with
	 * {@link android.os.Build#FINGERPRINT} or {@link
	 * android.os.Build.VERSION}. If an error occurs during processing the OTA
	 * before the reboot,
	 * the caller will be notified by {@link InstallSystemUpdateCallback}. If device
	 * does not have
	 * sufficient battery level, the installation will fail with error {@link
	 * InstallSystemUpdateCallback#UPDATE_ERROR_BATTERY_LOW}.
	 *
	 * @param admin          The {@link DeviceAdminReceiver} that this request is
	 *                       associated with. Null if
	 *                       the caller is not a device admin
	 * @param updateFilePath A Uri of the file that contains the update. The file
	 *                       should be
	 *                       readable by the calling app.
	 * @param executor       The executor through which the callback should be
	 *                       invoked.
	 * @param callback       A callback object that will inform the caller when
	 ...
	 * @param admin which {@link DeviceAdminReceiver} this request is associated
	 *              with.
	 * @return one of {@code PRIVATE_DNS_MODE_OFF},
	 *         {@code PRIVATE_DNS_MODE_OPPORTUNISTIC},
	 *         {@code PRIVATE_DNS_MODE_PROVIDER_HOSTNAME} or
	 ...
	 * When called by an app targeting SDK level
	 * {@link android.os.Build.VERSION_CODES#Q} or
	 * below, will behave the same as
	 * {@link #setProfileOwnerOnOrganizationOwnedDevice}.
	 *
	 * When called by an app targeting SDK level
	...
	@Deprecated
	@SystemApi
	@RequiresPermission(value = android.Manifest.permission.GRANT_PROFILE_OWNER_DEVICE_IDS_ACCESS, conditional = true)
	...
	/**
	 * Sets whether the profile owner of the given user as managing an
	 * organization-owned device.
	 * Managing an organization-owned device will give it access to device
	 * identifiers (such as
	 ...
	 *
	 * <p>
	 * Calling with a {@code null} value for the set disables the restriction so
	 * that all
	 * packages are allowed to access cross-profile calendar APIs. Calling with an
	 * empty set
	 * disallows all packages from accessing cross-profile calendar APIs. If this
	 * method isn't
	 * called, no package is allowed to access cross-profile calendar APIs by
	 * default.
	 *
	 * @param admin        which {@link DeviceAdminReceiver} this request is
	 *                     associated with
	...
	/**
	 * Gets a set of package names that are allowed to access cross-profile calendar
	 * APIs.
	 *
	 * <p>
	 * Called by a profile owner of a managed profile.
	 ...
	 *
	 * <p>
	 * A package is allowed to access cross-profile calendar APIs if it's allowed by
	 ...
	 * {@link Context#createPackageContextAsUser(String, int, UserHandle)} to create
	 * a context for
	 * that user, and get a {@link DevicePolicyManager} from this context.
	 *
	 * @param packageName the name of the package
	 * @return {@code true} if the package is allowed to access cross-profile
	 *         calendar APIs,
	 *         {@code false} otherwise
	 *
	 * @see #setCrossProfileCalendarPackages(ComponentName, Set)
	...
	/**
	 * Sets the set of admin-allowlisted package names that are allowed to request
	 * user consent for
	 ...
	 *
	 * <p>
	 * Note that other apps may be able to request user consent for cross-profile
	 * communication
	 * if they have been explicitly allowlisted by the OEM.
	 *
	 * <p>
	 * When previously-set cross-profile packages are missing from
	 * {@code packageNames}, the
	 * app-op for {@code INTERACT_ACROSS_PROFILES} will be reset for those packages.
	 * This will not
	 * occur for packages that are allowlisted by the OEM.
	 *
	 * @param admin        the {@link DeviceAdminReceiver} this request is
	 *                     associated with
	...
	/**
	 * Returns the set of package names that the admin has previously set as allowed
	 * to request user
	 * consent for cross-profile communication, via
	 * {@link #setCrossProfilePackages(ComponentName,
	 * Set)}.
	 *
	 * <p>
	 * Assumes that the caller is a profile owner and is the given {@code admin}.
	 *
	 * <p>
	 * Note that other apps not included in the returned set may be able to request
	 * user consent
	 * for cross-profile communication if they have been explicitly allowlisted by
	 * the OEM.
	 *
	 * @param admin the {@link DeviceAdminReceiver} this request is associated with
	 * @return the set of package names the admin has previously set as allowed to
	 *         request user
	 *         consent for cross-profile communication, via
	 *         {@link #setCrossProfilePackages(ComponentName,
	 *         Set)}
	 ...
	 * <ul>
	 * <li>The package names that the admin has previously set as allowed to request
	 * user consent
	 * for cross-profile communication, via
	 * {@link #setCrossProfilePackages(ComponentName,
	 * Set)}.</li>
	 * <li>The default package names set by the OEM that are allowed to request user
	 * consent for
	 * cross-profile communication without being explicitly enabled by the admin,
	 * via {@link
	 * com.android.internal.R.array#cross_profile_apps} and
	 * {@link com.android.internal.R.array
	...
	/**
	 * Returns the default package names set by the OEM that are allowed to
	 * communicate
	...
	/**
	 * Returns whether the device is being used as a managed kiosk. These
	 * requirements are as
	 * follows:
	 * <ul>
	 * <li>The device is in Lock Task (therefore there is also a Device Owner app on
	 * the
	 * device)</li>
	 * <li>The Lock Task feature
	 * {@link DevicePolicyManager#LOCK_TASK_FEATURE_SYSTEM_INFO} is
	 * not enabled, so the system info in the status bar is not visible</li>
	 * <li>The device does not have a secure lock screen (e.g. it has no lock screen
	 * or has
	 * swipe-to-unlock)</li>
	 ...
	 *
	 * <p>
	 * Publicly-accessible dedicated devices don't have the same privacy model as
	 * personally-used devices. In particular, user consent popups don't make sense
	 * as a barrier to
	 * accessing persistent data on these devices since the user giving consent and
	 * the user whose
	 ...
	 *
	 * <p>
	 * This condition is not sufficient to cover APIs that would access data that
	 * only lives for
	 * the duration of the user's session, since the user has an expectation of
	 * privacy in these
	 * conditions that more closely resembles use of a personal device. In those
	 * cases, see {@link
	...
	/**
	 * Returns whether the device is being used as an unattended managed kiosk.
	 * These requirements
	 * are as follows:
	 * <ul>
	 ...
	 *
	 * <p>
	 * See {@link #isManagedKiosk()} for context. This is a stronger requirement
	 * that also
	 * ensures that the device hasn't been interacted with recently, making it an
	 * appropriate check
	 * for privacy-sensitive APIs that wouldn't be appropriate during an active user
	 * session.
	 *
	 * @hide
	 */
	 ...
	 * @return {@code true} if the activity is started successfully, {@code false}
	 *         otherwise
	 *
	 * @see CalendarContract#startViewCalendarEventInManagedProfile(Context, String,
	 *      long, long,
	 *      long, boolean, int)
	 *
	 * @hide
	...
	/**
	 * Service-specific error code used in
	 * {@link #setApplicationExemptions(String, Set)} and
	 ...
	 *                               {@link android.Manifest.permission#MANAGE_DEVICE_POLICY_APP_EXEMPTIONS}
	 * @throws NameNotFoundException If either the package is not installed or the
	 *                               package is not
		...
		if (mService != null) {
			try {
				...
				switch (e.errorCode) {
					...
					default:
						throw new RuntimeException(
								"Unknown error setting application exemptions: "
										+ e.errorCode,
								e);
				}
			} catch (RemoteException e) {
	...
	/**
	 * Returns all the platform restriction exemptions currently applied to an
	 * application. Called
	...
	public Set<Integer> getApplicationExemptions(@NonNull String packageName)
		...
		try {
			...
			switch (e.errorCode) {
				...
				default:
					throw new RuntimeException(
							"Unknown error getting application exemptions: " + e.errorCode,
							e);
			}
		} catch (RemoteException e) {
			throw e.rethrowFromSystemServer();
	 ...
	 * {@link android.Manifest.permission#MANAGE_DEVICE_POLICY_APPS_CONTROL} to
	 * disable user
	 * control over apps. User will not be able to clear app data or force-stop
	 * packages. When
	 * called by a device owner, applies to all users on the device. Packages with
	 * user control
	 * disabled are exempted from App Standby Buckets.
	 * <p>
	 * Starting from {@link Build.VERSION_CODES#UPSIDE_DOWN_CAKE}, after the user
	 * control disabled
	 * packages policy has been set,
	 * {@link PolicyUpdateReceiver#onPolicySetResult(Context, String,
	 * Bundle, TargetUser, PolicyUpdateResult)} will notify the admin on whether the
	 * policy was
	 * successfully set or not. This callback will contain:
	...
	/**
	 * Returns the list of packages over which user control is disabled by a device
	 * or profile
	 ...
	 * Called by device owner or profile owner of an organization-owned managed
	 * profile to toggle
	 * Common Criteria mode for the device. When the device is in Common Criteria
	 * mode,
	 ...
	 * <ul>
	 * <li>Bluetooth long term key material is additionally integrity-protected with
	 * AES-GCM.</li>
	 * <li>WiFi configuration store is additionally integrity-protected with
	 * AES-GCM.</li>
	 * </ul>
	 * Common Criteria mode is disabled by default.
	 *
	 * <p>
	 * <em>Note:</em> if Common Critera mode is turned off after being enabled
	 * previously,
	...
	/**
	 * Returns whether Common Criteria mode is currently enabled. Device owner and
	 * profile owner of
	 * an organization-owned managed profile can query its own Common Criteria mode
	 * setting by
	 * calling this method with its admin {@link ComponentName}. Any caller can
	 * obtain the
	 * aggregated device-wide Common Criteria mode state by passing {@code null} as
	 * the
	 * {@code admin} argument.
	 *
	 * @param admin Which {@link DeviceAdminReceiver} this request is associated
	 *              with. Null if the
	 *              caller is not a device admin.
	 * @return {@code true} if Common Criteria mode is enabled, {@code false}
	 *         otherwise.
	 */
	...
	/**
	 * Called by profile owner of an organization-owned managed profile to check
	 * whether
	...
	/**
	 * Called by a profile owner of an organization-owned managed profile to suspend
	 * personal
	 * apps on the device. When personal apps are suspended the device can only be
	 * used for calls.
	 *
	 * <p>
	 * When personal apps are suspended, an ongoing notification about that is shown
	 * to the user.
	 * When the user taps the notification, system invokes
	 * {@link #ACTION_CHECK_POLICY_COMPLIANCE}
	 * in the profile owner package. Profile owner implementation that uses personal
	 * apps suspension
	 ...
	 * @throws IllegalStateException if the profile owner doesn't have an activity
	 *                               that handles
	 *                               {@link #ACTION_CHECK_POLICY_COMPLIANCE}
	 */
	...
	/**
	 * Called by a profile owner of an organization-owned managed profile to set
	 * maximum time
	 * the profile is allowed to be turned off. If the profile is turned off for
	 * longer, personal
	 ...
	 * @param admin         Which {@link DeviceAdminReceiver} this request is
	 *                      associated with
	 * @param timeoutMillis Maximum time the profile is allowed to be off in
	 *                      milliseconds or 0 if
	 *                      not limited. The minimum non-zero value corresponds to
	 *                      72 hours. If an admin sets a
	...
	/**
	 * Called by a profile owner of an organization-owned managed profile to get
	 * maximum time
	 * the profile is allowed to be turned off.
	 *
	 * @param admin Which {@link DeviceAdminReceiver} this request is associated
	 *              with
	 * @return Maximum time the profile is allowed to be off in milliseconds or 0 if
	 *         not limited.
	 * @see #setPersonalAppsSuspended
	 */
	...
	/**
	 * Called by a profile owner of an organization-owned managed profile to
	 * acknowledge that the
	 * device is compliant and the user can turn the profile off if needed according
	 * to the maximum
	 * time off policy.
	 *
	 * This method should be called when the device is deemed compliant after
	 * getting
	 * {@link DeviceAdminReceiver#onComplianceAcknowledgementRequired(Context, Intent)}
	 * callback in
	 * case it is overridden. Before this method is called the user is still free to
	 * turn the
	 * profile off, but the timer won't be reset, so personal apps will be suspended
	 * sooner.
	 *
	...
	/**
	 * Called by a profile owner of an organization-owned managed profile to query
	 * whether it needs
	 * to acknowledge device compliance to allow the user to turn the profile off if
	 * needed
	 ...
	 * {@link DeviceAdminReceiver#onComplianceAcknowledgementRequired(Context, Intent)}
	 * callback.
	 * But if the callback was not delivered or handled for some reason, this method
	 * can be used to
	...
	/**
	 * Returns {@code true} when {@code userId} has a profile owner that is capable
	 * of resetting
	 * password in RUNNING_LOCKED state. For that it should have at least one direct
	 * boot aware
	 * component and have an active password reset token. Can only be called by the
	 * system.
	 * 
	 * @hide
	 */
	...
	/**
	 * Returns an enrollment-specific identifier of this device, which is guaranteed
	 * to be the same
	 * value for the same device, enrolled into the same organization by the same
	 * managing app.
	 * This identifier is high-entropy, useful for uniquely identifying individual
	 * devices within
	 * the same organisation.
	 * It is available both in a work profile and on a fully-managed device.
	 * The identifier would be consistent even if the work profile is removed and
	 * enrolled again
	 ...
	 * If {@link #setOrganizationId(String)} was not called, then the returned value
	 * will be an
	 * empty string.
	 *
	 * <p>
	 * Note about access to device identifiers: a device owner, a profile owner of
	 * an
	 * organization-owned device or the delegated certificate installer (holding the
	 * {@link #DELEGATION_CERT_INSTALL} delegation) on such a device can still
	 * obtain hardware
	 * identifiers by calling e.g. {@link android.os.Build#getSerial()}, in addition
	 * to using
	 * this method. However, a profile owner on a personal (non organization-owned)
	 * device, or the
	 * delegated certificate installer on such a device, cannot obtain hardware
	 * identifiers anymore
	 * and must switch to using this method.
	 *
	 * @return A stable, enrollment-specific identifier.
	 * @throws SecurityException if the caller is not a profile owner or device
	 *                           owner.
	 */
	@NonNull
	...
	/**
	 * Sets the Enterprise ID for the work profile or managed device. This is a
	 * requirement for
	 * generating an enrollment-specific ID for the device, see
	 * {@link #getEnrollmentSpecificId()}.
	 *
	 * It is recommended that the Enterprise ID is at least 6 characters long, and
	 * no more than
	 * 64 characters.
	 *
	 * @param enterpriseId An identifier of the organization this work profile or
	 *                     device is
	 *                     enrolled into.
	 */
	...
	/**
	 * Clears organization ID set by the DPC and resets the precomputed enrollment
	 * specific ID.
	 * 
	 * @hide
	 */
	 ...
	 * {@link ManagedProfileProvisioningParams#getProfileAdminComponentName()} as
	 * the profile
	 * owner.
	 *
	 * <p>
	 ...
	 * @return The {@link UserHandle} of the created profile or {@code null} if the
	 *         service is
	 *         not available.
	...
	/**
	 * The localized error message to show to the end-user. If {@code null}, a
	 * generic error
	...
	/**
	 * Provisions a managed device and sets the {@code deviceAdminComponentName} as
	 * the device
	 * owner.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * Holders of {@link android.Manifest.permission#PROVISION_DEMO_DEVICE} can call
	 * this API
	 * only if {@link FullyManagedDeviceProvisioningParams#isDemoDevice()} is
	 * {@code true}.
	 * </p>
	 *
	 * @param provisioningParams Params required to provision a fully managed
	 *                           device,
	 ...
	 * {@link #createAndProvisionManagedProfile} between {@code userId} and all it's
	 * managed
	 * profiles if any.
	 *
	 * @hide
	 ...
	 * This is a signal that the device is a fully-managed device where personal
	 * usage is
	 * discouraged.
	 ...
	 * 
	 * @return true if an admin can grant device sensors-related permissions, false
	 *         otherwise.
	 */
	 ...
	 *
	 * @param admin           The {@link DeviceAdminReceiver} that is the device
	 *                        owner.
	 * @param deviceOwnerType The device owner type is set to. Use
	 *                        {@link #DEVICE_OWNER_TYPE_DEFAULT} for the default
	 *                        device owner type. Use
	 *                        {@link #DEVICE_OWNER_TYPE_FINANCED} for the financed
	 *                        device owner type.
	 *
	 * @throws IllegalStateException When admin is not the device owner, or there is
	 *                               no device
	 *                               owner, or attempting to set the device owner
	 *                               type again for the same admin.
	 ...
	 * {@link #setDeviceOwnerType(ComponentName, int)}.
	 * {@link #DEVICE_OWNER_TYPE_DEFAULT}
	 * would be returned when the device owner type is not set for the device owner
	 * admin.
	 *
	...
	public boolean isFinancedDevice() {
		return isDeviceManaged()
				&& getDeviceOwnerType(getDeviceOwnerComponentOnAnyUser()) == DEVICE_OWNER_TYPE_FINANCED;
	}

	/**
	 * Called by a device owner or profile owner of an organization-owned managed
	 * profile to enable
	 * or disable USB data signaling for the device. When disabled, USB data
	 * connections
	 * (except from charging functions) are prohibited.
	 *
	 * <p>
	 * This API is not supported on all devices, the caller should call
	 * {@link #canUsbDataSignalingBeDisabled()} to check whether enabling or
	 * disabling USB data
	 * signaling is supported on the device.
	 *
	 * @param enabled whether USB data signaling should be enabled or not.
	 * @throws SecurityException     if the caller is not permitted to set this
	 *                               policy
	 * @throws IllegalStateException if disabling USB data signaling is not
	 *                               supported or
	 *                               if USB data signaling fails to be
	 *                               enabled/disabled.
	 */
	 ...
	 *
	 * <p>
	 * When called by a device owner or profile owner of an organization-owned
	 * managed profile,
	 * this API returns whether USB data signaling is currently enabled by that
	 * admin. When called
	 * by any other app, returns whether USB data signaling is currently enabled on
	 * the device.
	 *
	 * @return {@code true} if USB data signaling is enabled, {@code false}
	 *         otherwise.
	 */
	...
	/**
	 * Called by the system to check whether USB data signaling is currently enabled
	 * for this user.
	 *
	 * @param userId which user to check for.
	 * @return {@code true} if USB data signaling is enabled, {@code false}
	...
	/**
	 * Returns whether enabling or disabling USB data signaling is supported on the
	 * device.
	 *
	 * @return {@code true} if the device supports enabling and disabling USB data
	 *         signaling.
	 */
	...
	/**
	 * Gets the list of {@link #isAffiliatedUser() affiliated} users running on
	 * foreground.
	 *
	 * @return list of {@link #isAffiliatedUser() affiliated} users running on
	 *         foreground.
	 *
	 ...
	 *
	 * <ul>
	 * <li>{@code nfcIntent}'s action is
	 * {@link NfcAdapter#ACTION_NDEF_DISCOVERED}</li>
	 ...
	 * </ul>
	 *
	 * This method returns {@code null} if the prerequisites are not met or if an
	 * error occurs
	 * when reading the NFC properties.
	 *
	 * @param nfcIntent the nfc intent generated from scanning a NFC tag
	 * @return a {@link #ACTION_PROVISION_MANAGED_DEVICE_FROM_TRUSTED_SOURCE} intent
	 *         with
	 *         intent extras as read by {@code nfcIntent}'s NFC properties or
	 *         {@code null} if the
	 *         prerequisites are not met or if an error occurs when reading the NFC
	 *         properties.
	 *
	 * @hide
	 */
	 ...
	 * The device may not connect to networks that do not meet the minimum security
	 * level.
	 * If the current network does not meet the minimum security level set, it will
	 * be disconnected.
	 *
	 * The following shows the Wi-Fi security levels from the lowest to the highest
	 * security level:
	 ...
	 * in order to be eligible for a connection. Providing a null policy results in
	 * the
	 * deactivation of the SSID restriction
	 *
	 * @param policy Wi-Fi SSID policy
	 * @throws SecurityException if the caller is not permitted to manage wifi
	 *                           policy
	 */
	@RequiresPermission(value = MANAGE_DEVICE_POLICY_WIFI, conditional = true)
	 ...
	 * @throws SecurityException if the caller is not a device owner or a profile
	 *                           owner on
	...
	/**
	 * Returns a {@link DevicePolicyResourcesManager} containing the required APIs
	 * to set, reset,
	 ...
	 *
	 * <p>
	 * If true is returned, then any attempts to begin setup again should result in
	 * factory reset
	 *
	 * @hide
	 */
	...
	/**
	 * Indicates that the DPC (Device Policy Controller, the agent responsible for
	 * enforcing policy)
	 * has or has not been downloaded during provisioning.
	 *
	 * @param downloaded {@code true} if the dpc has been downloaded during
	 *                   provisioning.
	 *                   {@code false} otherwise.
	 *
	 * @hide
	 ...
	 *
	 * <p>
	 * If the device policy management role holder is not configured for this
	 * device, returns
	 * {@code null}.
	 */
	@Nullable
	 ...
	 *
	 * <p>
	 * If the device policy management role holder updater is not configured for
	 * this device,
	 * returns {@code null}.
	 *
	 * @hide
	...
	/**
	 * Returns a {@link List} of managed profiles managed by some profile owner
	 * within the profile
	 * group of the given user, or an empty {@link List} if there is not one.
	 *
	 * @param user the user whose profile group to look within to return managed
	 *             profiles
	 *
	 * @hide
	 */
	 ...
	 *
	 * <p>
	 * If the supplied {@code deviceManagerConfig} is {@code null} or empty, returns
	 * {@code null}.
	 */
	@Nullable
	...
	/**
	 * Reset cache for
	 * {@link #shouldAllowBypassingDevicePolicyManagementRoleQualification}.
	 *
	 * @hide
	 */
	@TestApi
	...
	/**
	 * @return {@code true} if bypassing the device policy management role
	...
	/**
	 * Returns a {@link DevicePolicyState} object containing information about the
	 * current state
	 * of device policies (e.g. values set by different admins, info about the
	 * enforcing admins,
	 * resolved policy, etc).
	 *
	 * @hide
	...
	/**
	 * Overrides the effective cached value of enable_keep_profiles_running for
	 * testing purposes.
	 *
	 * @hide
	 */
	...
	/**
	 * Triggers the data migration of device policies for existing DPCs to the
	 * Device Policy Engine.
	 * If {@code forceMigration} is set to {@code true} it skips the prerequisite
	 * checks before
	 * triggering the migration.
	 *
	 * <p>
	 * Returns {@code true} if migration was completed successfully, {@code false}
	 * otherwise.
	 *
	 * @hide
	 */
	 ...
	 * {@link #setLockTaskPackages})
	 * by the holder of the role
	 * {@link android.app.role.RoleManager#ROLE_FINANCED_DEVICE_KIOSK}.
	 * If this occurs, Device Owners and Profile Owners that have set lock task
	 * packages or
	 * features, or that attempt to set lock task packages or features, will receive
	 * a callback
	 * indicating that it could not be set. See
	 ...
	 *
	 * @throws SecurityException if the caller is not a device owner, profile owner
	 *                           of an
	 *                           organization-owned managed profile, profile owner
	 *                           on the primary user or holder of one of the
	 *                           following roles:
	 *                           {@link android.app.role.RoleManager.ROLE_DEVICE_POLICY_MANAGEMENT},
	 ...
	 * {@link android.app.role.RoleManager#ROLE_FINANCED_DEVICE_KIOSK}.
	 *
	 * @return the package name of the application holding the role or {@code null}
	 *         if the role is
